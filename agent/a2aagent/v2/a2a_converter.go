...
func appendImagePart(parts []*protocol.Part, cp model.ContentPart) []*protocol.Part {
	...
	if len(cp.Image.Data) > 0 {
		fp := protocol.NewRawPart(cp.Image.Data, cp.Image.Format)
	...
	if cp.Image.URL != "" {
		fp := protocol.NewFilePart(cp.Image.URL, "image", cp.Image.Format)
		fp.Metadata = map[string]any{
			ia2a.FilePartMetadataContentTypeKey: ia2a.FilePartMetadataContentTypeImage,
		}
		return append(parts, fp)
...
func appendAudioPart(parts []*protocol.Part, cp model.ContentPart) []*protocol.Part {
	...
	fp := protocol.NewRawPart(cp.Audio.Data, cp.Audio.Format)
	fp.Metadata = map[string]any{
...
func appendFilePart(parts []*protocol.Part, cp model.ContentPart) []*protocol.Part {
	...
	if len(cp.File.Data) > 0 {
		fp := protocol.NewRawPart(cp.File.Data, cp.File.MimeType)
...
func processTextPart(part *protocol.Part) (text string, isThought bool) {
	...
	textContent, ok := part.Content.(protocol.Text)
	if !ok {
		log.Warnf("unexpected part content type: %T", part.Content)
		return "", false
	}
...
func processExecutableCode(d *protocol.Part) string {
	data, ok := d.DataContent().(map[string]any)
	if !ok {
		return ""
	}
...
func processCodeExecutionResult(d *protocol.Part) string {
	data, ok := d.DataContent().(map[string]any)
	if !ok {
		return ""
	}
...
func markTerminalStructuredErrorEvent(evt *event.Event, result protocol.StreamEvent) {
	if evt == nil || evt.Response == nil || evt.Response.Error == nil {
		return
	}
	switch v := result.(type) {
	case *protocol.TaskStatusUpdateEvent:
		// Use Terminal() rather than IsFinal(): v2's IsFinal() also reports
		// input-required, which must stay recoverable (v0 servers only set the
		// wire Final flag for genuinely terminal states).
