...
func TestA2AAgentExample_WithCustomHandler(t *testing.T) {
	...
	difyAgent2, err := New(
		WithName("test-streaming-agent"),
		WithBaseUrl(fmt.Sprintf("http://%s", host)),
	...
	// Run agent
	sessionService := inmemory.NewSessionService()
	testRunner := runner.NewRunner("test", difyAgent2, runner.WithSessionService(sessionService))
...
func TestA2AAgentExample_ErrorHandling(t *testing.T) {
	...
	// Run agent
	sessionService := inmemory.NewSessionService()
	testRunner := runner.NewRunner("test", difyAgent, runner.WithSessionService(sessionService))
