...
func (p *Planner) buildPlannerInstruction() string {
	highLevelPreamble := strings.Join([]string{
		...
		"2. EXECUTE: For each planned step that needs a tool, output " + ActionTag + " with a brief description, then CALL the tool",
	...
	criticalRules := strings.Join([]string{
		...
		"   - Only output " + ActionTag + " when you will issue an actual tool call in the same response.",
