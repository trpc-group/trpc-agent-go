...
func (s *Server) handleStructure(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	compiled, err := s.exportStructure(ctx)
	if err != nil {
	...
	s.respondJSON(w, r, http.StatusOK, structureResponse{Structure: compiled.Snapshot})
}
...
func (s *Server) exportStructure(ctx context.Context) (*profilecompiler.Structure, error) {
	snapshot, err := astructure.Export(ctx, s.agent)
	if err != nil {
		return nil, err
	}
	compiled, err := profilecompiler.NewStructure(snapshot)
	if err != nil {
		return nil, err
	}
	return compiled, nil
