...
func (c *defaultA2AMessageToAgentMessage) ConvertToAgentMessage(
	...
	for _, part := range message.Parts {
		...
		case protocol.Raw, protocol.URL:
			// Convert file content to model.ContentPart.
...
func (c *defaultEventToA2AMessage) setPartTypeMetadata(dataPart *protocol.Part, typeValue string) {
...
func (c *defaultEventToA2AMessage) setThoughtMetadata(textPart *protocol.Part) {
...
// convertFilePart converts a file Part (Raw or URL content) to one or more
// model.ContentPart values.
//
// Content type resolution order (highest to lowest priority):
//  1. Part.Metadata["content_type"] — set explicitly by trpc-agent-go clients
//  2. MediaType or common format value — "image/*"/"png" → ContentTypeImage,
//     "audio/*"/"mp3" → ContentTypeAudio
//  3. Part.Filename — legacy fallback for older clients that used name="image"/"audio"
//
// For Raw content, the bytes are already decoded by the protocol layer.
func convertFilePart(filePart *protocol.Part) []model.ContentPart {
	// Resolve content type using metadata > mediaType > name (legacy).
	...
	case protocol.Raw:
		// Bytes are already decoded by the protocol layer.
		data := []byte(content)
		switch contentType {
...
// resolveFilePartContentType determines the logical content type of a file Part.
//
// Priority:
//  1. Metadata["content_type"] (set by trpc-agent-go clients, unambiguous)
//  2. MediaType or common format value ("image/*", "audio/*", "png", "mp3")
//  3. Filename field (legacy: older clients used name="image"/"audio" as a type hint)
func resolveFilePartContentType(filePart *protocol.Part) string {
	...
	// 2. Infer from MediaType or common format value
	if inferred := inferContentTypeFromMimeType(filePart.MediaType); inferred != "" {
		return inferred
	}

	// 3. Legacy name-based fallback (older clients that used name="image"/"audio")
	switch filePart.Filename {
