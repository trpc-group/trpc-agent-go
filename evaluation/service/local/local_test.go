...
func TestLocalInferenceTraceModeUsesConfiguredActualConversation(t *testing.T) {
	...
	executionTrace := &agenttrace.Trace{RootAgentName: "candidate", RootInvocationID: "trace-root"}
	...
	if assert.Len(t, results[0].ExecutionTraces, 1) {
		...
		assert.Equal(t, executionTrace.RootAgentName, results[0].ExecutionTraces[0].RootAgentName)
	}
