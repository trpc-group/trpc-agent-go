...
func (t *ToolTrajectoryCriterion) Match(actual, expected *evalset.Invocation) error {
	if t.Compare != nil {
	...
	actualTools, err := t.getToolComparers(
	...
	expectedTools, err := t.getToolComparers(
	...
	for i := range len(actualTools) {
		strategy := t.getStrategy(actualTools[i], expectedTools[i])
		if err := strategy.Match(actualTools[i], expectedTools[i]); err != nil {
...
func (t *ToolTrajectoryCriterion) getToolComparers(toolUses []*genai.FunctionCall,
...
func (t *ToolTrajectoryCriterion) getStrategy(actualTool, expectedTool *toolComparer) *ToolTrajectoryStrategy {
...
func (t *ToolTrajectoryStrategy) Match(actual, expected *toolComparer) error {
	if t.Name != nil {
		if err := t.Name.Match(actual.name, expected.name); err != nil {
			return fmt.Errorf("name mismatch: %w", err)
		}
	}
	if t.Arguments != nil {
		if err := t.Arguments.Match(actual.args, expected.args); err != nil {
			return fmt.Errorf("arguments mismatch: %w", err)
		}
	}
	if t.Response != nil {
		if err := t.Response.Match(actual.response, expected.response); err != nil {
