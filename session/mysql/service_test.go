...
func TestCreateSession_Success(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(
		"SELECT expires_at FROM session_states WHERE app_name = ? AND user_id = ? AND session_id = ? AND deleted_at IS NULL FOR UPDATE",
	)).
		WithArgs(key.AppName, key.UserID, key.SessionID).
...
func TestCleanupExpiredSessions_DeduplicatesActiveRowsBeforeSoftDelete(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
