...
func TestCreateSessionSummaryWithRevision(t *testing.T) {
	...
	mock.ExpectQuery("SELECT updated_at FROM session_summaries").
		WithArgs(sess.AppName, sess.UserID, sess.ID, "all").
...
func TestTurnStartUsesRollingProjectionWithoutHistoryScan(t *testing.T) {
	...
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM session_events").
		WithArgs(key.AppName, key.UserID, key.SessionID, sqlmock.AnyArg()).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(0))
	mock.ExpectQuery("SELECT track, COUNT\\(\\*\\) FROM session_track_events").
		WithArgs(key.AppName, key.UserID, key.SessionID, sqlmock.AnyArg()).
		WillReturnRows(sqlmock.NewRows([]string{"track", "count"}))
