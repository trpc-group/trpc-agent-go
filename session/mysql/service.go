...
func (s *Service) deleteDuplicateSessionStates(
	...
	whereClause = strings.ReplaceAll(whereClause, "app_name", "duplicate.app_name")
	whereClause = strings.ReplaceAll(whereClause, "user_id", "duplicate.user_id")
	...
	deleteArgs := make([]any, 0, len(args)+2)
	deleteArgs = append(deleteArgs, now)
	deleteArgs = append(deleteArgs, args...)
	deleteArgs = append(deleteArgs, now)
	if _, err := tx.ExecContext(ctx, query, deleteArgs...); err != nil {
		return fmt.Errorf("delete duplicate session states: %w", err)
	}
	return nil
}
