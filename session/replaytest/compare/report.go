...
// 按 memory ID 比较 read/search 列表中的每条记忆。
func compareMemoryList(
	...
	for _, id := range unionKeys(left, right) {
		...
		if !leftOK || !rightOK {
			report.add(DomainMemory, prefix, locator, valueOrNil(l, leftOK), valueOrNil(r, rightOK), rules)
			continue
		}
...
func memoryByID(entries []normalize.MemoryEntry) map[string]normalize.MemoryEntry {
	out := make(map[string]normalize.MemoryEntry, len(entries))
