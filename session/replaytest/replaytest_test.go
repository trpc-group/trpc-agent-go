...
func TestNormalizerPreservesResponseIdentity(t *testing.T) {
	normalize := func(responseIDs []string) Snapshot {
		events := make([]event.Event, len(responseIDs))
		for i, responseID := range responseIDs {
			events[i] = event.Event{
				ID: fmt.Sprintf("event-%d", i), Author: "assistant",
				Response: &model.Response{ID: responseID},
			}
		}
		sess := session.NewSession("app", "user", "response-identity",
	...
	responseID := func(snapshot Snapshot, index int) any {
		return snapshot.Events[index]["response"].(map[string]any)["id"]
	}

	baseline := normalize([]string{"response-a", "response-b"})
	require.Equal(t, "response-000", responseID(baseline, 0))
	require.Equal(t, "response-001", responseID(baseline, 1))
	require.NotContains(t, baseline.Events[0]["response"], "timestamp")
	...
	tests := []struct {
		...
		{name: "duplicate response IDs", ids: []string{"response-x", "response-x"}, want: "response-000"},
		{name: "missing response ID", ids: []string{"response-x", ""}, want: ""},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			candidate := normalize(tt.ids)
			require.Equal(t, tt.want, responseID(candidate, 1))
			diffs, err := Compare("response identity", "baseline", "candidate", baseline, candidate, nil)
			require.NoError(t, err)
			requireDiffAtPath(t, diffs, "$.events[1].response.id")
		})
	}
}

func TestPublicCasesOwnsStandardCaseMetadata(t *testing.T) {
	names := []string{
		"single_turn",
		"multi_turn",
	...
	runners := make(map[string]PublicCaseRunner, len(names))
	for _, name := range names {
		runners[name] = func(context.Context, Backend) error { return nil }
	}
	cases, err := PublicCases(runners)
	require.NoError(t, err)
	require.Len(t, cases, len(names))
	for i := range cases {
		require.Equal(t, names[i], cases[i].Name)
		require.NotNil(t, cases[i].Run)
	}
	require.True(t, cases[10].OrderEventsByTimestamp)
	require.ElementsMatch(t, []string{
		CapabilityEvents, CapabilityState, CapabilityMemory, CapabilitySummary,
	}, cases[11].RequiredCapabilities)

	delete(runners, "single_turn")
	_, err = PublicCases(runners)
	require.ErrorContains(t, err, `public replay case "single_turn" has no runner`)
}
