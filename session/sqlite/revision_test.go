...
func TestRewindRestoresSQLiteProjection(t *testing.T) {
...
func TestReplacementPreservesEarlierSoftDeletedProjection(t *testing.T) {
  ...
  app_name, user_id, session_id, filter_key, summary, updated_at, deleted_at
	...
	_, err = service.Rewind(ctx, sessionrevision.RewindRequest{
		Key: key, TargetRequestID: "latest-request", ExpectedHeadRequestID: "latest-request", IdempotencyKey: "replacement",
	})
	require.NoError(t, err)
...
ORDER BY created_at ASC, id ASC`,
	...
	_, err = service.Rewind(
		ctx,
		sessionrevision.RewindRequest{
			Key: key, TargetRequestID: "latest-request", ExpectedHeadRequestID: "latest-request", IdempotencyKey: "edit",
		},
	)
	require.NoError(t, err)
	after := activeIDs()
...
WHERE app_name = ? AND user_id = ? AND session_id = ?`,
	...
	tests := []struct {
	...
	}{
		{
			...
			is:   sessionrevision.ErrRewindUnavailable,
		},
		{
			...
			is: sessionrevision.ErrRewindConflict,
		},
		{
		...
		},
		{
			...
			record: sessionrevision.PersistedRecord{Replays: map[string]sessionrevision.PersistedReplay{
				"replacement": {TargetRequestID: "other", ExpectedHeadRequestID: "other"},
			}},
			is: sessionrevision.ErrRewindConflict,
		},
		{
			...
			record: sessionrevision.PersistedRecord{
				...
				Replays: map[string]sessionrevision.PersistedReplay{
					"replacement": {TargetRequestID: "request", ExpectedHeadRequestID: "request", Generation: 1},
				},
			},
			is: sessionrevision.ErrRewindConflict,
		},
	...
	setRecord(sessionrevision.PersistedRecord{
		...
		Head:       1,
		Replays: map[string]sessionrevision.PersistedReplay{
			"replacement": {TargetRequestID: "request", ExpectedHeadRequestID: "request", Generation: 1, Head: 1},
		},
	})
	_, err = service.db.ExecContext(
		ctx,
...
func TestReplacementResultScopedStateFailures(t *testing.T) {
	...
	for name, table := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, err)
			_, err = service.rewindResultWithScopedState(
...
BEGIN SELECT RAISE(ABORT, 'fail'); END`, service.tableSessionTracks,
  ...
  app_name, user_id, session_id, track, event, created_at, updated_at
	...
	t.Run("summary update", func(t *testing.T) {
		service := newService(t)
		current, err := json.Marshal(&session.Summary{
			Summary: "current", UpdatedAt: time.Now(),
		})
		require.NoError(t, err)
		_, err = service.db.ExecContext(context.Background(), fmt.Sprintf(
			`INSERT INTO %s (
  app_name, user_id, session_id, filter_key, summary, updated_at
		...
		), key.AppName, key.UserID, key.SessionID, "all", current,
			time.Now().UTC().UnixNano())
		require.NoError(t, err)
		_, err = service.db.ExecContext(context.Background(), fmt.Sprintf(
			`CREATE TRIGGER fail_summary BEFORE UPDATE ON %s
...
func TestRewindDatabaseFailures(t *testing.T) {
	key := session.Key{AppName: "app", UserID: "user", SessionID: "session"}
	req := sessionrevision.RewindRequest{
		Key: key, TargetRequestID: "request", ExpectedHeadRequestID: "request", IdempotencyKey: "replacement",
	}
	...
	t.Run("expired session", func(t *testing.T) {
		...
		_, err := service.db.ExecContext(
			context.Background(),
			fmt.Sprintf(`UPDATE %s SET expires_at = ?`, service.tableSessionStates),
			time.Now().Add(-time.Second).UTC().UnixNano(),
		)
		require.NoError(t, err)
		expiredReq := req
		expiredReq.ExpectedHeadRequestID = "stale-observation"
		_, err = service.Rewind(context.Background(), expiredReq)
		assert.ErrorIs(t, err, sessionrevision.ErrRewindUnavailable)
		assert.NotErrorIs(t, err, sessionrevision.ErrRewindConflict)
	})
	...
	t.Run("load active state", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = service.Rewind(context.Background(), req)
...
func testSQLiteLatestTurnReplacement(t *testing.T, service *Service, async bool) {
	...
	require.NoError(t, service.flushRevisionPersistence(ctx, key))
  ...
  app_name, user_id, session_id, filter_key, summary, updated_at, deleted_at
	...
	assert.Equal(t, "before", result.Session.Events[0].ID)
	phase, ok := result.Session.GetState("phase")
	require.True(t, ok)
	assert.Equal(t, []byte("before"), phase)
	appTheme, ok := result.Session.GetState(session.StateAppPrefix + "theme")
	require.True(t, ok)
	assert.Equal(t, []byte("dark"), appTheme)
	userLocale, ok := result.Session.GetState(session.StateUserPrefix + "locale")
	require.True(t, ok)
	...
	require.Contains(t, result.Session.Summaries, "")
	assert.Equal(t, "before", result.Session.Summaries[""].Summary)
