...
func (m *summarizerManager) Summarize(ctx context.Context, sess *session.Session, force bool) error {
	...
	summaryText, err := m.summarizer.Summarize(ctx, sess, 0) // Use default keepRecent.
	if err != nil {
	...
	if summaryText != "" {
		...
		m.cache[appName][userID][sess.ID] = &SessionSummary{
			...
			CompressedCount: len(sess.Events),
			CreatedAt:       time.Now(),
			Metadata: map[string]any{
				MetadataKeyCompressionRatio: float64(originalEventCount-len(sess.Events)) / float64(originalEventCount) * 100,
			},
		}

		// Update the stored session if base service is available.
		if m.baseService != nil {
			// Create a summary event to append to the session.
			summaryEvent := &event.Event{
				InvocationID: "summary-" + sess.ID,
				Author:       authorSystem,
				Response: &model.Response{
					Choices: []model.Choice{{
						Message: model.Message{
							Role:    authorSystem,
							Content: fmt.Sprintf("Session compressed. Summary: %s", summaryText),
						},
					}},
				},
				Timestamp: time.Now(),
			...
			// Append the summary event to persist the compressed session.
			if err := m.baseService.AppendEvent(ctx, sess, summaryEvent); err != nil {
				// The session has already been compressed in memory.
				// We only log here to avoid disrupting the caller flow, matching the TS behavior.
				// External backends (e.g., Redis) may not persist the synthetic summary event by design.
				log.Warnf("failed to append summary event for session %s: %v", sess.ID, err)
			}
		}
	}
