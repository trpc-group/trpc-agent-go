...
const (
	...
	sqlCreateSessionEventsTable = `
		...
		PARTITION BY (app_name, cityHash64(user_id) % 64)
		ORDER BY (user_id, session_id, event_type, created_at, deleted_at)
		SETTINGS index_granularity = 8192`
	...
	sqlCreateSessionEventsEventTypeIndex = `
		ALTER TABLE {{TABLE_NAME}} ADD INDEX IF NOT EXISTS {{INDEX_NAME}} (event_type) TYPE set(100) GRANULARITY 4`
)
...
var indexDefs = []indexDefinition{
	...
	{sqldb.TableNameSessionEvents, "event_type", sqlCreateSessionEventsEventTypeIndex},
}
