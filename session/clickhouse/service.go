...
// EventTypeMessage is the event type for conversation messages.
const EventTypeMessage = "message"

// EventTypeTrackPrefix is the prefix for track event types.
const EventTypeTrackPrefix = "track:"

// SessionState is the state of a session.
type SessionState struct {
	ID        string           `json:"id"`
	State     session.StateMap `json:"state"`
	Metadata  session.StateMap `json:"metadata,omitempty"`
...
func (s *Service) DeleteAppState(ctx context.Context, appName string, key string) error {
	...
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE app_name = ? AND key = ?`, s.tableAppStates),
		appName, key)

	if err != nil {
		return fmt.Errorf("clickhouse session service delete app state failed: %w", err)
	}
	return nil
}
...
func (s *Service) DeleteUserState(ctx context.Context, userKey session.UserKey, key string) error {
	...
	// Hard delete using ALTER TABLE DELETE (async mutation)
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE app_name = ? AND user_id = ? AND key = ?`, s.tableUserStates),
		userKey.AppName, userKey.UserID, key)

	if err != nil {
		return fmt.Errorf("clickhouse session service delete user state failed: %w", err)
	}
	return nil
}
...
func (s *Service) flushEventBatch(batch []*sessionEventPair) {
	...
	// Group by session for state merging
	groups := make(map[string][]*sessionEventPair)
	for _, pair := range batch {
		keyStr := fmt.Sprintf("%s:%s:%s", pair.key.AppName, pair.key.UserID, pair.key.SessionID)
		groups[keyStr] = append(groups[keyStr], pair)
	}
...
func (s *Service) cleanupExpiredSessions(ctx context.Context, now time.Time) {
	// Hard delete expired sessions using ALTER TABLE DELETE (async mutation)
	// Check updated_at < expires_at to avoid deleting renewed sessions
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE expires_at IS NOT NULL AND expires_at <= ? AND updated_at < expires_at`, s.tableSessionStates),
		now)
	if err != nil {
	...
	err = s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE expires_at IS NOT NULL AND expires_at <= ?`, s.tableSessionEvents),
		now)
	if err != nil {
	...
	err = s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE expires_at IS NOT NULL AND expires_at <= ?`, s.tableSessionSummaries),
		now)
	if err != nil {
...
func (s *Service) cleanupExpiredAppStates(ctx context.Context, now time.Time) {
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE expires_at IS NOT NULL AND expires_at <= ?`, s.tableAppStates),
		now)
	if err != nil {
...
func (s *Service) cleanupExpiredUserStates(ctx context.Context, now time.Time) {
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE expires_at IS NOT NULL AND expires_at <= ?`, s.tableUserStates),
		now)
	if err != nil {
...
func (s *Service) cleanupExpiredForUser(ctx context.Context, userKey session.UserKey) {
	...
	err := s.chClient.Exec(ctx,
		fmt.Sprintf(`ALTER TABLE %s DELETE WHERE app_name = ? AND user_id = ? AND expires_at IS NOT NULL AND expires_at <= ?`, s.tableSessionStates),
