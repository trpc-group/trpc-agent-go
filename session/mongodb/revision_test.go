...
func TestRevisionMutations(t *testing.T) {
	...
	trackEvent := &session.TrackEvent{
		Track: "trace", Payload: json.RawMessage(`{"ok":true}`),
		Timestamp: time.Now(),
	}
...
func TestRewindRestoresCompleteProjection(t *testing.T) {
	key := session.Key{AppName: "app", UserID: "user", SessionID: "session"}
	...
	require.NoError(t, err)
	summaryRaw, err := json.Marshal(restored.Summaries["all"])
	require.NoError(t, err)
	...
	mc.findFn = func(any) (*mongo.Cursor, error) {
		...
		if findCalls == 3 {
			return mongo.NewCursorFromDocuments([]any{sessionSummaryDoc{
				FilterKey: "all", Summary: summaryRaw,
			}}, nil, nil)
		}
		if findCalls == 4 {
			return mongo.NewCursorFromDocuments(eventDocs, nil, nil)
	...
	result, err := s.Rewind(context.Background(), sessionrevision.RewindRequest{
		Key: key, TargetRequestID: "old-request", ExpectedHeadRequestID: "old-request", IdempotencyKey: "new-request",
	})
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, result.Session)
	assert.Equal(t, []byte("turn"), result.Session.State["before"])
	assert.Len(t, result.Session.Events, 1)
	assert.Len(t, result.Session.Tracks["trace"].Events, 1)
	assert.Equal(t, "before turn", result.Session.Summaries["all"].Summary)
	generation, ok := sessionrevision.Generation(result.Session)
	assert.True(t, ok)
...
func TestRewindReplayAndGenerationLimit(t *testing.T) {
	key := session.Key{AppName: "app", UserID: "user", SessionID: "session"}
	...
	replayRecord := &sessionrevision.PersistedRecord{
		...
		Replays: map[string]sessionrevision.PersistedReplay{
			"new-request": {TargetRequestID: "old-request", ExpectedHeadRequestID: "old-request", Generation: 4, Head: 8},
		},
	}
	var err error
	...
	_, err = s.Rewind(context.Background(), sessionrevision.RewindRequest{
		Key: key, TargetRequestID: "old-request", ExpectedHeadRequestID: "old-request", IdempotencyKey: "another-request",
	})
	assert.ErrorIs(t, err, sessionrevision.ErrRewindUnavailable)
}

func TestRewindRequiresUnexpiredSession(t *testing.T) {
	var observed bson.M
	mc := &mockClient{
		findOneFn: func(filter any) *mongo.SingleResult {
			observed = filter.(bson.M)
	...
	s := newServiceForTest(t, mc)
	_, err := s.Rewind(context.Background(), session.RewindRequest{
		Key:             session.Key{AppName: "app", UserID: "user", SessionID: "session"},
		TargetRequestID: "latest", ExpectedHeadRequestID: "latest",
		IdempotencyKey: "edit",
	})
	assert.ErrorIs(t, err, session.ErrRewindUnavailable)
	assert.Contains(t, observed, "$or")
}
...
func TestRevisionWriteFailurePaths(t *testing.T) {
	...
	t.Run("transaction", func(t *testing.T) {
		...
		assert.ErrorContains(t, err, "rewind session: transaction failed")
	})
...
func TestRewindProtocolFailures(t *testing.T) {
	key := session.Key{AppName: "app", UserID: "user", SessionID: "session"}
	...
	t.Run("flush", func(t *testing.T) {
		...
		_, err := s.Rewind(context.Background(), req)
	...
	t.Run("replay conflict", func(t *testing.T) {
		record := &sessionrevision.PersistedRecord{
			Generation: 2, Head: 3,
			Replays: map[string]sessionrevision.PersistedReplay{
				"new-request": {TargetRequestID: "different", ExpectedHeadRequestID: "different", Generation: 2, Head: 3},
			},
		}
		raw, encodeErr := encodeRevision(record)
		require.NoError(t, encodeErr)
	...
	t.Run("checkpoint decode", func(t *testing.T) {
		...
		s, _ := revisionTestService(t, sessionStateDoc{Revision: raw})
		_, err := s.Rewind(context.Background(), req)
		assert.ErrorContains(t, err, "restore rewind boundary")
	})
	...
	for name, failAt := range map[string]int{"app state": 4, "user state": 5} {
		t.Run(name, func(t *testing.T) {
			record := &sessionrevision.PersistedRecord{
				Generation: 1,
				Replays: map[string]sessionrevision.PersistedReplay{
					"new-request": {TargetRequestID: "old-request", ExpectedHeadRequestID: "old-request", Generation: 1},
				},
			}
			raw, encodeErr := encodeRevision(record)
			require.NoError(t, encodeErr)
...
func TestRevisionTailVerificationFailsClosed(t *testing.T) {
	...
	prefix := session.TrackEvent{
		Track: track, Payload: json.RawMessage(`{"step":1}`),
	}
	prefixRaw, err := json.Marshal(prefix)
	require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			activeEvents: []session.TrackEvent{{
				Track: track, Payload: json.RawMessage(`{"step":2}`),
			}},
