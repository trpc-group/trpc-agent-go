...
func TestListSessionsReloadsMetadataAfterGenerationChange(t *testing.T) {
	...
	expectPostgresRevisionGeneration(mock, key, state2)
...
func expectPostgresRevisionGeneration(
...
func TestCreateSessionSummaryWithRevision(t *testing.T) {
	...
	require.NoError(t, err)
	defer db.Close()
	s := createTestService(t, db, WithSummarizer(&mockSummarizerImpl{summaryText: "summary"}))
