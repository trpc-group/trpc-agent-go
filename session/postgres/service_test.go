...
func TestService_ConsecutiveUserMessage_HandlerCalled(t *testing.T) {
	...
	s, mock, db := setupMockService(t, nil)
	s.opts.onConsecutiveUserMsg = handler
	defer db.Close()
	...
	// Call appendEventInternal directly.
...
func TestService_ConsecutiveUserMessage_SkipCurrent(t *testing.T) {
	...
	s, _, db := setupMockService(t, nil)
	s.opts.onConsecutiveUserMsg = skipCurrentHandler
	defer db.Close()
	...
	// Session events should still have only the original event (handler returned false).
