...
func TestIsSingleFilterKey_Caching(t *testing.T) {
	now := time.Now()

	t.Run("caches true result in session state", func(t *testing.T) {
		sess := &session.Session{
			ID:    "s1",
		...
		result := isSingleFilterKey(sess, "app/test")
		require.True(t, result)
		// Verify cache was written.
		cached, ok := sess.GetState(singleFilterKeyCachePrefix + "app/test")
		require.True(t, ok)
		require.Equal(t, "1", string(cached))
	})

	t.Run("caches false result in session state", func(t *testing.T) {
		sess := &session.Session{
			ID:    "s1",
			State: session.StateMap{},
			Events: []event.Event{
				makeEvent("e1", now, "app/test"),
				makeEvent("e2", now, "other/key"),
			},
		}
		// First call should compute and cache.
		result := isSingleFilterKey(sess, "app/test")
		...
		cached, ok := sess.GetState(singleFilterKeyCachePrefix + "app/test")
		require.True(t, ok)
		require.Equal(t, "0", string(cached))
	})

	t.Run("returns cached result without rescanning", func(t *testing.T) {
		sess := &session.Session{
			ID:    "s1",
			State: session.StateMap{},
			Events: []event.Event{
		...
		// Pre-populate cache with false (opposite of actual result).
		sess.SetState(singleFilterKeyCachePrefix+"app/test", []byte("0"))
		// Should return cached false, not recompute.
		result := isSingleFilterKey(sess, "app/test")
		require.False(t, result)
	})

	t.Run("different targetKeys have separate caches", func(t *testing.T) {
		sess := &session.Session{
			ID:    "s1",
			State: session.StateMap{},
			Events: []event.Event{
		...
		// Check for "app/test" - should be true.
		result1 := isSingleFilterKey(sess, "app/test")
		require.True(t, result1)
		// Check for "other/key" - should be false.
		result2 := isSingleFilterKey(sess, "other/key")
		require.False(t, result2)
		// Verify both caches exist.
		cached1, _ := sess.GetState(singleFilterKeyCachePrefix + "app/test")
		cached2, _ := sess.GetState(singleFilterKeyCachePrefix + "other/key")
		require.Equal(t, "1", string(cached1))
		require.Equal(t, "0", string(cached2))
	})
}
