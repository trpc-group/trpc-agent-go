...
func TestRevisionGenerations(t *testing.T) {
	...
	require.NoError(t, rdb.Set(ctx, c.revisionKey(key), raw, 0).Err())
...
func TestRewindPreservesRemainingTTL(t *testing.T) {
	...
	active, applied, err := c.Rewind(ctx, key, "request", "request", "replacement")
	require.NoError(t, err)
	assert.True(t, applied)
	assert.Equal(t, before, mr.TTL(stateKey))
	assert.Equal(t, before, mr.TTL(c.revisionKey(key)))
	replayed, replayApplied, err := c.Rewind(
		ctx,
		key,
		"request",
		"request", "replacement",
...
func TestApplyRemainingTTL(t *testing.T) {
	...
	pipe := rdb.Pipeline()
	applyRemainingTTL(ctx, pipe, key, -1)
	_, err := pipe.Exec(ctx)
	require.NoError(t, err)
	...
	pipe = rdb.Pipeline()
	applyRemainingTTL(ctx, pipe, key, -2)
	_, err = pipe.Exec(ctx)
	require.NoError(t, err)
	ttl, err = rdb.PTTL(ctx, key).Result()
	require.NoError(t, err)
...
func TestRevisionMetadataFailures(t *testing.T) {
	...
	_, _, err = c.Rewind(ctx, key, "request", "request", "replacement")
	assert.ErrorContains(t, err, "decode revision metadata")
...
func TestRewindRejectsInvalidRevisionRecords(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			record: sessionrevision.PersistedRecord{Replays: map[string]sessionrevision.PersistedReplay{
				"replacement": {TargetRequestID: "request", ExpectedHeadRequestID: "request"},
			}},
			is: sessionrevision.ErrRewindUnavailable,
		},
	}
