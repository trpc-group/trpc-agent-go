...
func TestStateInitializationLeaseKeyUsesDigest(t *testing.T) {
	...
	require.Contains(t, leaseKey, "{app}")
...
func TestLoadSessionStateValue(t *testing.T) {
	...
	t.Run("missing session", func(t *testing.T) {
		value, present, generation, exists, err := client.LoadSessionStateValue(
			ctx,
			stateInitializationZSetKey("missing"),
			"state",
		)
	...
	t.Run("loads caller owned value", func(t *testing.T) {
		key := stateInitializationZSetKey("value")
	...
	t.Run("migrates legacy generation and preserves ttl", func(t *testing.T) {
		key := stateInitializationZSetKey("legacy")
		stored := SessionState{
			ID:    key.SessionID,
		...
		require.NoError(t, redisClient.HSet(ctx, stateKey, key.SessionID, encoded).Err())
		require.NoError(t, redisClient.Expire(ctx, stateKey, 5*time.Minute).Err())
		...
		require.NotEmpty(t, generation)
		require.Positive(t, mr.TTL(stateKey))

		storedJSON, err := redisClient.HGet(ctx, stateKey, key.SessionID).Bytes()
		require.NoError(t, err)
	...
	t.Run("rejects malformed records", func(t *testing.T) {
		for _, test := range []struct {
			...
			t.Run(test.name, func(t *testing.T) {
				key := stateInitializationZSetKey("bad-" + test.name)
				require.NoError(t, redisClient.HSet(
					ctx,
					client.sessionStateKey(key),
					key.SessionID,
					test.raw,
				).Err())
...
func TestCommitStateInitialization(t *testing.T) {
	...
	createSession := func(t *testing.T, id string) (session.Key, string) {
		t.Helper()
		key := stateInitializationZSetKey(id)
	...
	t.Run("fences owner generation and missing session", func(t *testing.T) {
		...
		missing := stateInitializationZSetKey("missing-commit")
	...
	t.Run("returns script error", func(t *testing.T) {
		key := stateInitializationZSetKey("bad-commit")
		require.NoError(t, redisClient.HSet(
			ctx,
			client.sessionStateKey(key),
			key.SessionID,
...
func TestStateInitializationRedisErrorsAndClone(t *testing.T) {
	...
	key := stateInitializationZSetKey("closed")
...
func stateInitializationZSetKey(id string) session.Key {
