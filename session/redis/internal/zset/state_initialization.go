...
func (c *Client) LoadSessionStateValue(
	...
	sessionText, err := c.runScript(
		ctx,
		luaLoadStateInitializationValue,
		[]string{c.sessionStateKey(key)},
		key.SessionID,
		uuid.NewString(),
	).Text()
	if err == redis.Nil {
	...
	if err := json.Unmarshal([]byte(sessionText), &stored); err != nil {
	...
	return cloneStateInitializationValue(value), present, stored.Generation, true, nil
}
...
func (c *Client) CommitStateInitialization(
	...
	result, err := c.runScript(
		...
		[]string{leaseKey, c.sessionStateKey(key)},
		ownerToken,
		key.SessionID,
		string(encodedState),
		nilSentinel,
