...
func (c *Client) AppendEventWithRevision(
	...
	rewindExpectedHead := write.HasExpectedHead
	callerExpectedHead := rewindExpectedHead || write.Start != nil
	for attempt := 0; attempt < revisionWriteAttempts; attempt++ {
		...
		args := []any{
			...
			projectionJSON,
			boolToInt(preparedWrite.BoundaryRequiresSummary),
			boolToInt(preparedWrite.Hazard),
		}
