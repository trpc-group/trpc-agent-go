...
func (s *Service) GetSession(
	...
	final := func(c *session.GetSessionContext, next func() (*session.Session, error)) (*session.Session, error) {
		return s.getSession(c.Context, c.Key, c.Options.EventNum, c.Options.EventTime, c.Options.Track)
	}
...
func (s *Service) getSession(
	...
	events, err := s.getEventsList(ctx, []session.Key{key}, limit, afterTime, track)
	if err != nil {
...
func (s *Service) listSessions(
	...
	events, err := s.getEventsList(ctx, sessionKeys, limit, afterTime, track)
	if err != nil {
		return nil, fmt.Errorf("get events failed: %w", err)
	}
...
func (s *Service) addEvent(ctx context.Context, key session.Key, event *event.Event) error {
	...
	// update event list when track exists or event is a completed valid response
