...
func (p *WorkspaceExecRequestProcessor) guidanceText() string {
	...
	b.WriteString("\n")
	b.WriteString("- workspace_exec runs shell commands inside the ")
	b.WriteString("current executor workspace, not on the agent host. ")
	b.WriteString("Treat it as a general shell runner for that ")
	b.WriteString("environment, not only as a local file viewer.\n")
