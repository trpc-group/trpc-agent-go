...
func TestFunctionCallResponseProcessor_AfterToolRoundOncePerMode(t *testing.T) {
	for _, perCall := range []bool{false, true} {
		t.Run(fmt.Sprintf("per_call_%t", perCall), func(t *testing.T) {
			recorder := &afterToolRoundRecorder{}
			manager, err := plugin.NewManager(recorder)
			require.NoError(t, err)
			inv, _ := newExecutionTraceProcessorInvocation(t)
			inv.Plugins = manager
			inv.RunOptions.ToolResultEventPerCallEnabled = perCall
			tools := map[string]tool.Tool{
			...
			calls := []model.ToolCall{
				{ID: "call-first", Function: model.FunctionDefinitionParam{
					Name: "first", Arguments: []byte(`{"value":1}`),
				}},
				{ID: "call-second", Function: model.FunctionDefinitionParam{
					Name: "second", Arguments: []byte(`{"value":2}`),
				}},
			}
			eventChan := make(chan *event.Event, len(calls))
			_, err = NewFunctionCallResponseProcessor(false, nil).
			...
			require.True(t, recorder.complete)
			require.Len(t, recorder.messages, len(calls))
		})
	}
}

type afterToolRoundRecorder struct {
	calls    int
	complete bool
	messages []model.Message
}

func (r *afterToolRoundRecorder) Name() string {
	return "after-tool-round-recorder"
}

func (r *afterToolRoundRecorder) Register(registry *plugin.Registry) {
