...
func TestHandleFunctionCallsAndSendEvent_ResultFormatterPerCallEvents(
	t *testing.T,
) {
	for _, parallel := range []bool{false, true} {
		t.Run(fmt.Sprintf("parallel=%t", parallel), func(t *testing.T) {
			...
			defaultTool := function.NewFunctionTool(
				func(context.Context, struct{}) (resultFormatTestResult, error) {
					return resultFormatTestResult{ExitCode: 0, Output: "default"}, nil
				},
				function.WithName("default"),
			)
			...
			inv := agent.NewInvocation(
				agent.WithInvocationID("inv-result-format-per-call"),
				agent.WithInvocationAgent(&mockAgentWithTools{name: "agent"}),
				agent.WithInvocationSession(session.NewSession(
					"app",
					"user",
					"session",
				)),
			...
			eventsByTool := make(map[string]*event.Event, len(toolCalls))
			stateDeltaEvents := 0
			for range toolCalls {
				evt := <-eventChan
				require.NotNil(t, evt)
				...
				eventsByTool[msg.ToolName] = evt
				assert.Equal(
					t,
					toolCallIDForName(toolCalls, msg.ToolName),
					msg.ToolID,
				)
			...
			require.Contains(t, eventsByTool, "formatted")
			assert.Contains(
				t,
				eventsByTool["formatted"].Choices[0].Message.Content,
				"<observation>",
			)
			require.Contains(t, eventsByTool, "default")
			assert.Equal(
				t,
				`{"exit_code":0,"output":"default"}`,
				eventsByTool["default"].Choices[0].Message.Content,
			)
			assert.Equal(t, 1, stateDeltaEvents)
			assert.Equal(t, []byte("updated"), last.StateDelta["state"])
