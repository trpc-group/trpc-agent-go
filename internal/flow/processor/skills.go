...
func defaultToolingAndWorkspaceGuidance() string {
	...
	b.WriteString("- When the user asks to create, modify, or inspect ")
	b.WriteString("workspace files, use the skill workspace file tools ")
	b.WriteString("(skill_ws_read_file, skill_ws_write_file, ")
	b.WriteString("skill_ws_replace_content, skill_ws_list_dir) instead ")
	b.WriteString("of only describing edits in plain text.\n")
