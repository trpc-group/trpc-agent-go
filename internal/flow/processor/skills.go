...
func defaultToolingAndWorkspaceGuidance() string {
	...
	b.WriteString("- When a tool result is evicted, the tool message ")
	b.WriteString("contains a JSON payload with preview+ref. ")
	b.WriteString("Automatically call read_file with the ref (use ")
	b.WriteString("start_line/num_lines to page) before answering.\n")
