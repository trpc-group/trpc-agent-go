...
func defaultToolingAndWorkspaceGuidance() string {
	...
	b.WriteString("- skill_run is a command runner (usually via bash) ")
	b.WriteString("inside the skill workspace, not a magic capability. ")
	b.WriteString("It does not automatically add the skill directory ")
	b.WriteString("to PATH or install dependencies. Invoke scripts via ")
	b.WriteString("an explicit interpreter and path (e.g., ")
	b.WriteString("python3 scripts/foo.py). Treat non-zero exit_code ")
	b.WriteString("(or timed_out / meaningful stderr) as a failure ")
	b.WriteString("signal and adjust accordingly.\n")
	...
	b.WriteString("executable is on PATH).\n")
	b.WriteString("- When skill_run fails (non-zero exit_code or ")
	b.WriteString("meaningful stderr), do not stop early. Inspect the ")
	b.WriteString("error, verify assumptions (files, PATH, required ")
	b.WriteString("deps), consult SKILL.md/docs, and adjust the ")
	b.WriteString("command before retrying. Avoid repeating the exact ")
	b.WriteString("same failing command; after a couple attempts, ")
