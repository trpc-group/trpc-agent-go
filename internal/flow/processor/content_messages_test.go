...
func TestProcessRequest_IgnoresRunnerCompletionEchoInHistory(t *testing.T) {
	sess := &session.Session{}
	sess.Events = append(sess.Events,
		newSessionEvent("user", model.NewUserMessage("hello")),
		newSessionEvent("test-agent", model.NewAssistantMessage("child reply")),
		event.Event{
			Author: "app",
	...
	inv := &agent.Invocation{
		InvocationID: "inv-runner-completion",
		AgentName:    "test-agent",
		Session:      sess,
...
func TestProcessRequest_GraphSnapshotPreassembledMessages_SkipHistoryAndInvocationMessage(t *testing.T) {
	sess := &session.Session{}
	sess.Events = append(sess.Events,
		newSessionEvent("user", model.NewUserMessage("old user")),
		newSessionEvent("test-agent", model.NewAssistantMessage("old assistant")),
	)

	preassembled := []model.Message{
		model.NewUserMessage("graph user"),
		model.NewAssistantMessage("graph assistant"),
	}

	inv := &agent.Invocation{
		InvocationID: "inv-graph-snapshot",
		AgentName:    "test-agent",
		Session:      sess,
		...
		RunOptions: agent.RunOptions{
			RuntimeState: map[string]any{
				...
				graph.StateKeyMessages:            preassembled,
			},
		},
	}
	...
	require.Len(t, req.Messages, 1+len(preassembled))
	require.True(t, model.MessagesEqual(model.NewSystemMessage("agent system prompt"), req.Messages[0]))
	require.True(t, model.MessagesEqual(model.NewUserMessage("graph user"), req.Messages[1]))
	require.True(t, model.MessagesEqual(model.NewAssistantMessage("graph assistant"), req.Messages[2]))
}

func TestProcessRequest_GraphSnapshotExplicitRequiresTypedMessages(t *testing.T) {
	sess := &session.Session{}

	inv := &agent.Invocation{
		InvocationID: "inv-graph-snapshot-explicit",
		AgentName:    "test-agent",
		Session:      sess,
		Message:      model.NewUserMessage("current should not be appended"),
		RunOptions: agent.RunOptions{
	...
	require.Len(t, req.Messages, 2)
	require.True(t, model.MessagesEqual(model.NewSystemMessage("agent system prompt"), req.Messages[0]))
	require.True(t, model.MessagesEqual(model.NewUserMessage("current should not be appended"), req.Messages[1]))
}

func TestProcessRequest_GraphSnapshotAutoStillDecodesLooseMessages(t *testing.T) {
	sess := &session.Session{}

	inv := &agent.Invocation{
		InvocationID: "inv-graph-snapshot-auto",
		AgentName:    "test-agent",
		Session:      sess,
		Message:      model.NewUserMessage("current should not be appended"),
		RunOptions: agent.RunOptions{
			RuntimeState: map[string]any{
				graph.CfgKeyIncludeContents:               "none",
				graph.CfgKeySubgraphMessageSource:         string(graph.SubgraphMessageSourceGraphSnapshot),
				graph.CfgKeySubgraphMessageSourceExplicit: false,
				graph.StateKeyMessages: []map[string]any{
					{"role": string(model.RoleUser), "content": "graph user"},
					{"role": string(model.RoleAssistant), "content": "graph assistant"},
				},
