...
func TestOutputResponseProcessor_ProcessResponse(t *testing.T) {
	...
	event := events[0]
	if event.Object != "state.update" {
		t.Errorf("Expected object to be 'state.update', got '%s'", event.Object)
	}

	if len(event.StateDelta) != 1 {
		t.Errorf("Expected 1 state delta, got %d", len(event.StateDelta))
		return
	}

	if value, exists := event.StateDelta["test_key"]; !exists {
