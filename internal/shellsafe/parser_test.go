...
func TestPolicy_BlocksSecondaryExecutorArguments(t *testing.T) {
	...
	tests := []string{
		...
		`awk '{ print $1 > output_file }' data.txt`,
		...
		`git clone --upload-pack='rm -rf /' ./repo dst`,
		`git fetch --receive-pack='rm -rf /' ./repo`,
		`git clone -u 'rm -rf /' ./repo dst`,
...
func TestPolicy_AllowsNonExecutingFindAwkGitAndSed(
	...
	tests := []string{
		`find . -name '*.go'`,
		`awk 'BEGIN { print("ok") }'`,
		`awk '/error|warn/ { print $2 }' log.txt`,
		`awk '$3 > 10 { print $1 }' data.txt`,
		`awk '{ print ($3 > 10) }' data.txt`,
