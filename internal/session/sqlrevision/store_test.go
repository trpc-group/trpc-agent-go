...
VALUES (?, ?, ?, ?, ?)`,
	...
	trackBefore := session.TrackEvent{
		Track: "trace", RequestID: "before-request", Timestamp: baseTime,
	}
...
VALUES (?, ?, ?, ?, ?, ?)`,
	...
	trackAfter := &session.TrackEvent{
		Track: "trace", RequestID: next.RequestID,
...
INSERT INTO tracks (
  app_name, user_id, session_id, track, event, created_at, expires_at
	...
	tx, err = db.BeginTx(ctx, nil)
	require.NoError(t, err)
	require.NoError(t, store.replaceSummaries(ctx, tx, key, restored))
	require.NoError(t, tx.Commit())
...
VALUES (?, ?, ?, ?, ?)`,
	...
	latestTrack := session.TrackEvent{
		Track: "trace", RequestID: "latest-request",
...
VALUES (?, ?, ?, 'default', ?, ?)`,
	...
	tx, err := db.BeginTx(ctx, nil)
	require.NoError(t, err)
	result, err := store.ReplaceLatestTurn(
		ctx,
		tx,
