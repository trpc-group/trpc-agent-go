...
func (s *replacementTestService) Rewind(
	context.Context,
	RewindRequest,
...
func TestRewindCapability(t *testing.T) {
	key := session.Key{AppName: "app", UserID: "user", SessionID: "session"}
	active := session.NewSession(key.AppName, key.UserID, key.SessionID)
	valid := &replacementTestService{result: &session.RewindResult{
		Session: active,
	}}

	request := RewindRequest{
		Key:             key,
		TargetRequestID: "old-request", ExpectedHeadRequestID: "old-request",
	...
	tests := []struct {
		...
		},
		{
			...
			service: &replacementTestService{result: &session.RewindResult{}},
			request: request,
			wantErr: "returned no active session",
		},
		{
			...
			service: &replacementTestService{result: &session.RewindResult{
				Session: session.NewSession("other", "user", "session"),
			}},
			request: request,
			wantErr: "returned key",
		},
	}
...
func TestTrackEventsEqualIgnoresJSONStorageNormalization(t *testing.T) {
	...
	assert.True(t, JSONEqual([]byte("invalid"), []byte("invalid")))
	assert.False(t, JSONEqual([]byte("{"), []byte("[")))
	assert.False(t, JSONEqual([]byte("{}"), []byte("[")))
}

func TestRewindReplay(t *testing.T) {
	...
	_, ok, err = RewindReplay(record, "request", "request", "missing")
	require.NoError(t, err)
	assert.False(t, ok)
	_, ok, err = RewindReplay(nil, "request", "request", "replacement")
	require.NoError(t, err)
	assert.False(t, ok)
	...
	for _, mutate := range []func(*PersistedRecord){
		...
		assert.True(t, ok)
		assert.ErrorIs(t, err, ErrRewindConflict)
	}
}
...
func TestRevisionContextAndGeneration(t *testing.T) {
	...
	writeCtx = ContextWithRequestID(writeCtx, "request")
	...
	assert.Equal(t, "request", write.RequestID)
...
func TestRewindWriteFence(t *testing.T) {
	AttachRewindFence(nil, &PersistedRecord{})
	sess := session.NewSession("app", "user", "session")
	AttachRewindFence(sess, nil)
	_, ok := RewindHeadFence(sess)
	assert.False(t, ok)

	record := &PersistedRecord{Generation: 3, Head: 7}
	AttachRewindFence(sess, record)
	head, ok := RewindHeadFence(sess)
	assert.True(t, ok)
	assert.Equal(t, uint64(7), head)

	ordinary := NewWrite(context.Background(), sess)
	assert.False(t, ordinary.HasExpectedHead)
	eventWrite, err := NewEventWrite(
		context.Background(),
		sess,
		revisionTestEvent("edit", "edit-invocation", false),
	)
	require.NoError(t, err)
	assert.True(t, eventWrite.HasExpectedHead)
	assert.Equal(t, uint64(7), eventWrite.ExpectedHead)
	_, err = NewEventWrite(context.Background(), sess, &event.Event{})
	assert.ErrorIs(t, err, errRewindEventNotPersistable)
	_, ok = RewindHeadFence(sess)
	assert.True(t, ok)
	startCtx := ContextWithTurnStart(context.Background(), TurnStart{
		RequestID: "edit", InvocationID: "edit-invocation",
	})
	write := NewWrite(startCtx, sess)
	assert.True(t, write.HasExpectedHead)
	assert.Equal(t, uint64(7), write.ExpectedHead)
	assert.NoError(t, CheckWrite(record, write))
	assert.ErrorIs(t, CheckWrite(&PersistedRecord{
		Generation: 3, Head: 8,
	}, write), ErrRewindConflict)
	assert.ErrorIs(t, CheckWrite(&PersistedRecord{
		Generation: 4, Head: 7,
	}, write), ErrStaleGeneration)

	require.Error(t, CompleteWrite(sess, write, errors.New("write failed")))
	_, ok = RewindHeadFence(sess)
	assert.True(t, ok)
	require.NoError(t, CompleteWrite(sess, write, nil))
	_, ok = RewindHeadFence(sess)
	assert.False(t, ok)
	ClearRewindHeadFence(nil)
}
...
func TestBoundaryRestore(t *testing.T) {
	...
	require.ErrorIs(t, err, ErrRewindUnavailable)
	...
	current.Tracks["trace"].Events = append(
		...
		session.TrackEvent{Track: "trace"},
	)
...
func TestRollingProjectionMatchesAuthoritativeBoundary(t *testing.T) {
	...
	before.Tracks = map[session.Track]*session.TrackEvents{
		"trace": {
			Track:  "trace",
			Events: []session.TrackEvent{{Track: "trace"}},
		},
	}
	record := &PersistedRecord{}
	...
	nextTrack := session.TrackEvent{Track: "trace"}
...
func TestApplyEventWriteMakesReusedRequestIDUnavailable(t *testing.T) {
	record := terminalRevisionRecord(t)
	require.Equal(t, "request", record.HeadRequestID)
	start := Write{
		HasExpectedGeneration: true,
		Start: &TurnStart{
			RequestID: "request", InvocationID: "second-invocation",
		},
		Boundary: []byte(`{"id":"session"}`),
	}
	ApplyEventWrite(
		record,
		start,
		revisionTestEvent("request", "second-invocation", false),
		true,
	)
	require.NotNil(t, record.Checkpoint)
	assert.True(t, record.Checkpoint.Hazard)
...
func TestApplyTrackWriteRequiresOpenRequest(t *testing.T) {
	record := runningRevisionRecord(t)
	ApplyTrackWrite(record, Write{
		HasExpectedGeneration: true, RequestID: "request",
	}, &session.TrackEvent{})
	assert.False(t, record.Checkpoint.Hazard)
	...
	ApplyTrackWrite(record, Write{
		HasExpectedGeneration: true, RequestID: "other-request",
	}, &session.TrackEvent{})
	assert.True(t, record.Checkpoint.Hazard)
}
...
func TestRecordRewindReplayIsBounded(t *testing.T) {
	record := &PersistedRecord{}
	RecordRewindReplay(nil, "ignored", PersistedReplay{})
	RecordRewindReplay(record, "", PersistedReplay{})
	for i := 0; i <= maxPersistedReplays; i++ {
		key := fmt.Sprintf("replacement-%02d", i)
		RecordRewindReplay(record, key, PersistedReplay{
			TargetRequestID:       key,
			ExpectedHeadRequestID: key,
			Generation:            uint64(i),
			Head:                  uint64(i),
		})
	}
