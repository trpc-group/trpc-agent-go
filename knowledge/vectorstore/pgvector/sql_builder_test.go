...
// TestDeleteQueryBuilder tests the deleteQueryBuilder functionality
func (suite *SQLBuilderTestSuite) TestDeleteQueryBuilder() {
	suite.Run("basic delete query", func() {
		builder := newDeleteQueryBuilder("test_table")

		sql, args := builder.buildDeleteQuery()

		expectedSQL := "DELETE FROM test_table WHERE 1=1"
		suite.Equal(expectedSQL, sql)
		suite.Empty(args)
	})

	suite.Run("delete with single metadata filter", func() {
		builder := newDeleteQueryBuilder("documents")
		metadata := map[string]interface{}{
			"category": "programming",
		}

		builder.addMetadataFilter(metadata)
		sql, args := builder.buildDeleteQuery()

		expectedSQL := "DELETE FROM documents WHERE 1=1 AND metadata ->> 'category' = $1"
		suite.Equal(expectedSQL, sql)
		suite.Require().Len(args, 1)
		suite.Equal("programming", args[0])
	})

	suite.Run("delete with multiple metadata filters", func() {
		builder := newDeleteQueryBuilder("documents")
		metadata := map[string]interface{}{
			"category": "programming",
			"level":    "beginner",
		...
		// Check that SQL contains all expected conditions
		suite.Contains(sql, "DELETE FROM documents WHERE 1=1")
		suite.Contains(sql, "metadata ->> 'category' = $")
		suite.Contains(sql, "metadata ->> 'level' = $")
		suite.Contains(sql, "metadata ->> 'active' = $")

		// Check arguments
		suite.Require().Len(args, 3)

		// Since map iteration order is not guaranteed, check that all values are present
		values := make(map[interface{}]bool)
		for _, arg := range args {
			values[arg] = true
		}
		suite.True(values["programming"])
		suite.True(values["beginner"])
		suite.True(values[true])
	})

	suite.Run("delete with empty metadata filter", func() {
		builder := newDeleteQueryBuilder("documents")
		metadata := map[string]interface{}{}
		...
		// Should be same as basic delete query when no metadata filters are added
		expectedSQL := "DELETE FROM documents WHERE 1=1"
		suite.Equal(expectedSQL, sql)
		suite.Empty(args)
	})

	suite.Run("parameter indexing", func() {
		builder := newDeleteQueryBuilder("test_table")

		// Add first filter
		builder.addMetadataFilter(map[string]interface{}{
			"key1": "value1",
		})
		...
		suite.Contains(sql, "$1")
		suite.Contains(sql, "$2")
		suite.Require().Len(args, 2)
		suite.Equal("value1", args[0])
		suite.Equal("value2", args[1])
	})

	suite.Run("special characters in metadata values", func() {
		builder := newDeleteQueryBuilder("documents")
		metadata := map[string]interface{}{
		...
		// SQL should be properly constructed
		suite.Contains(sql, "DELETE FROM documents WHERE 1=1")
		suite.Contains(sql, "metadata ->> 'description' = $")
		suite.Contains(sql, "metadata ->> 'path' = $")

		// Arguments should contain the exact values
		suite.Require().Len(args, 2)
		values := make(map[interface{}]bool)
		for _, arg := range args {
			values[arg] = true
		}
		suite.True(values["It's a test with 'quotes' and \"double quotes\""])
		suite.True(values["/home/user/file.txt"])
	})

	suite.Run("numeric and boolean metadata values", func() {
		builder := newDeleteQueryBuilder("documents")
		metadata := map[string]interface{}{
			"count":   42,
			"score":   3.14,
		...
		suite.True(values[42])
		suite.True(values[3.14])
		suite.True(values[false])
	})
}
