...
// New constructs a VectorStore bound to a Chroma 1.5.3+ collection. The context
// controls collection initialization and is not retained. WithAPIKey infers the
// Cloud tenant and database when omitted. WithSparseSearch plus auto-create adds
// a sparse index to newly created collections.
func New(ctx context.Context, opts ...Option) (*VectorStore, error) {
	...
	if opt.sparseSearch {
		...
		// Auto-created schemas declare the built-in Cloud SPLADE embedder;
		// custom embedders keep the default "unknown" declaration.
...
func collectClientBuilderOpts(o options) ([]storage.ClientBuilderOpt, error) {
	if o.baseURL == "" {
		return nil, errors.New("chroma: must specify WithChromaInstance or WithBaseURL")
	}
...
// Update replaces an existing document. An empty embedding keeps the stored
// vector. Because Chroma's /update is a partial update, metadata keys absent
// from doc are sent as null deletion markers to preserve replacement semantics.
...
// deleteAll removes every record while preserving the collection.
...
// forEachGetPage walks Get pages, deduplicating IDs, until an empty page or
// maxRecords unique IDs are reached. maxRecords <= 0 reads every page.
