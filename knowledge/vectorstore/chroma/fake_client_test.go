...
func (f *fakeClient) Delete(_ context.Context, p storage.DeleteParams) error {
	...
	// Mirror the server's delete semantics, verified against Chroma Cloud and
	// self-hosted 1.5.3: when ids are combined with where or where_document,
	// only records matching every selector are deleted (intersection), like
	// the server's Filter(ids, where, where_document).
	ids := make(map[string]struct{}, len(p.IDs))
	for _, id := range p.IDs {
		ids[id] = struct{}{}
	}
