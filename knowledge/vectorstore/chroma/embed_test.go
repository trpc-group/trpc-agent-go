...
// TestSparseVectorValueEmptyEncodesAsArrays verifies that an empty sparse
// vector serializes with array-valued indices and values. nil slices would
// JSON-encode as null, which Chroma rejects because both fields must be
// arrays.
func TestSparseVectorValueEmptyEncodesAsArrays(t *testing.T) {
	got, err := sparseVectorValue(SparseVector{})
	if err != nil {
		t.Fatal(err)
	}
	if got["indices"] == nil || got["values"] == nil {
		t.Fatalf("sparse value = %#v, want non-nil empty slices", got)
	}
	raw, err := json.Marshal(got)
	if err != nil {
		t.Fatal(err)
	}
	if want := `{"#type":"sparse_vector","indices":[],"values":[]}`; string(raw) != want {
		t.Fatalf("empty sparse vector JSON = %s, want %s", raw, want)
	}
}
