...
func sparseVectorValue(vec SparseVector) (map[string]any, error) {
	...
	// Copy into always non-nil slices: make returns a non-nil empty slice for
	// zero-length input, so an empty (but valid) vector JSON-encodes as
	// "[]" rather than "null", which Chroma rejects.
