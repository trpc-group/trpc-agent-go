...
func TestWithSpladeTransport(t *testing.T) {
	// A nil transport is ignored and the default is kept.
	e, err := NewCloudSpladeEmbedder("key", WithSpladeTransport(nil))
	if err != nil {
		t.Fatalf("NewCloudSpladeEmbedder: %v", err)
	}
	if e.httpClient == nil {
	...
	e, err = NewCloudSpladeEmbedder("key",
		WithSpladeBaseURL("http://splade.test"), WithSpladeTransport(custom))
	if err != nil {
		t.Fatalf("NewCloudSpladeEmbedder: %v", err)
	}
	...
	// Redirect control must remain with this package even with an injected
	// transport.
	if e.httpClient.CheckRedirect == nil {
		t.Fatal("redirect policy must stay with the default client")
	}
}
...
// TestSpladeRedirectLoopStopsAtLimit verifies that the default client's
// redirect policy bounds redirect loops even though a custom CheckRedirect
// disables net/http's built-in limit.
func TestSpladeRedirectLoopStopsAtLimit(t *testing.T) {
	hops := 0
	...
	embedder, err := NewCloudSpladeEmbedder("test-key", WithSpladeBaseURL(loop.URL))
	if err != nil {
		t.Fatalf("NewCloudSpladeEmbedder: %v", err)
	}
	if _, err := embedder.EmbedDocument(context.Background(), "text"); err == nil ||
