...
func TestConvertCondition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: map[string]any{"$and": []any{
				...
				map[string]any{"lang": map[string]any{"$eq": "zh"}},
			}},
		},
		{
			...
			want: map[string]any{"a": map[string]any{"$eq": "1"}},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.cond == nil {
				if got.where != nil || got.ids != nil || got.whereDocument != nil || got.idsSet || got.noMatch {
					t.Fatalf("nil cond = %#v", got)
				}
				return
			}
			if tt.want != nil && !reflect.DeepEqual(got.where, tt.want) {
				t.Fatalf("got = %#v, want %#v", got.where, tt.want)
			}
		})
	}
}
...
func TestConvertConditionErrors(t *testing.T) {
	...
	if err != nil || gotPrefixed.where["category"] == nil {
		t.Fatalf("prefixed metadata field = %#v, %v", gotPrefixed.where, err)
	}
	...
	if err != nil || unwrapped.where["a"] == nil {
