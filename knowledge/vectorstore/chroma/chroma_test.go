...
func TestNewNamedInstanceAndMissingCollection(t *testing.T) {
	...
	vs, err := New(context.Background(), WithChromaInstance("named"), WithCollection("col"), WithIndexDimension(3))
	if err != nil {
	...
	if _, err := New(context.Background(), WithChromaInstance("missing"), WithCollection("col"), WithIndexDimension(3)); err == nil || !strings.Contains(err.Error(), "not registered") {
...
func TestNewRequiresURL(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "WithChromaInstance or WithBaseURL") {
		t.Fatalf("err = %v", err)
	}
}
...
// TestDeleteByFilterCombinedSelectors verifies combined ID and metadata
// selectors delete only records matching both, mirroring the server's
// intersection semantics (verified against Chroma Cloud and self-hosted
// 1.5.3).
func TestDeleteByFilterCombinedSelectors(t *testing.T) {
	ctx := context.Background()
	fc := newFakeClient()
	vs := testVectorStore(fc)
	_ = vs.Add(ctx, &document.Document{ID: "a", Content: "a", Metadata: map[string]any{"category": "guide"}}, []float64{1, 0, 0})
	...
	// a is listed but does not match the filter; b matches the filter but is
	// not listed. Only records matching both selectors may be deleted.
	if err := vs.DeleteByFilter(ctx,
		vectorstore.WithDeleteDocumentIDs([]string{"a", "c"}),
		vectorstore.WithDeleteFilter(map[string]any{"category": "other"}),
	); err != nil {
		t.Fatal(err)
	}
	if _, _, err := vs.Get(ctx, "a"); err != nil {
		t.Fatalf("a should survive (listed but does not match filter): %v", err)
	}
	if _, _, err := vs.Get(ctx, "b"); err != nil {
		t.Fatalf("b should survive (not listed): %v", err)
	}
	if _, _, err := vs.Get(ctx, "c"); !errors.Is(err, errNotFound) {
		t.Fatalf("c should be deleted (listed and matches filter): %v", err)
	}
}
