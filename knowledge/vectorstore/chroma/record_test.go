...
// TestDocToRecordRejectsReservedPrefixKeys verifies that '$' and '#' prefixed
// metadata keys are rejected before the write request; Chroma refuses them
// server-side with an error that does not name the offending key.
func TestDocToRecordRejectsReservedPrefixKeys(t *testing.T) {
	vs := testVectorStore(newFakeClient())
	for _, key := range []string{"$and", "$eq", "#document", "#custom"} {
		t.Run(key, func(t *testing.T) {
			_, err := vs.docToRecord(&document.Document{
				ID:       "d",
				Metadata: map[string]any{key: "caller-value"},
			}, []float64{1, 0, 0}, time.Now().Unix())
			if err == nil || !strings.Contains(err.Error(), "uses a reserved prefix") {
				t.Fatalf("docToRecord() error = %v", err)
			}
		})
	}
