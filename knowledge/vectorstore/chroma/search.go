...
//
// SearchModeHybrid is the zero value, so pure vector search requires an
// explicit SearchModeVector. MinScore applies to ranked results. Vector Query
// caps one request at the configured limit because it has no cursor.
