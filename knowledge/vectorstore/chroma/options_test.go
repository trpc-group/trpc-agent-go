...
func TestHybridOptions(t *testing.T) {
	...
	o := testOptions(WithSparseSearch(stub), WithHybridSearchWeights(2, 1))
	...
	o = testOptions(WithSparseSearch(stub), WithHybridSearchWeights(0, 0))
	if err := validateOptions(&o); err == nil {
		t.Fatal("zero weights should fail validation")
	}
	o = testOptions(
		WithSparseSearch(stub),
		WithHybridSearchWeights(-1, 2),
		WithHybridSearchWeights(2, 1),
	)
	if err := validateOptions(&o); err != nil {
