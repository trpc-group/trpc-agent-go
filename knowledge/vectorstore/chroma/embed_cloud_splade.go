...
// crosses origins, mirroring the storage client's checkRedirect. A custom
// CheckRedirect disables net/http's default redirect limit, so the hop count
// is bounded here as well.
...
// WithSpladeTransport sets the transport used for embedding requests. This
// can be used to customize proxies, TLS, timeouts, or tracing. A nil
// transport is ignored and the default transport is kept.
//
// The redirect policy, which strips the API key header on cross-origin
// redirects, always remains with this package.
