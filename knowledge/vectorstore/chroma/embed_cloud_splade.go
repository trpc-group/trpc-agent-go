...
// Default Cloud sparse embedding service, mirroring the official clients'
...
const (
	...
	// spladeMaxRedirects mirrors net/http's default redirect limit. A custom
	// CheckRedirect disables that default, so the limit must be enforced here.
	spladeMaxRedirects = 10
)

// CloudSpladeEmbedder is a SparseEmbedder backed by Chroma Cloud's hosted
// SPLADE API; documents and queries share one vector space. It is immutable and
// safe for concurrent use. Each write and keyword or hybrid search makes one
// hosted call per text, subject to the account's rate limits. The model is
// English-focused; other languages need a custom SparseEmbedder.
...
func NewCloudSpladeEmbedder(apiKey string, opts ...CloudSpladeOption) (*CloudSpladeEmbedder, error) {
	...
	// Redirect handling always stays with this package; only the transport is
	// caller-configurable.
	e.httpClient = defaultSpladeHTTPClient()
	if e.transport != nil {
		e.httpClient.Transport = e.transport
	}
	return e, nil
}
...
// defaultSpladeHTTPClient strips the API key on cross-origin redirects and
// bounds the hop count, which a custom CheckRedirect otherwise disables.
func defaultSpladeHTTPClient() *http.Client {
	return &http.Client{
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			if len(via) >= spladeMaxRedirects {
				return fmt.Errorf("chroma: stopped after %d redirects", spladeMaxRedirects)
			}
			if len(via) > 0 {
				origin := via[0].URL
...
// x-chroma-embedding-model header; the account must have access to it.
...
// WithSpladeTransport sets the transport for embedding requests, for custom
// proxies, TLS, timeouts, or tracing. A nil transport keeps the default. The
// redirect policy always stays with this package.
func WithSpladeTransport(rt http.RoundTripper) CloudSpladeOption {
	return func(e *CloudSpladeEmbedder) {
		if rt != nil {
			e.transport = rt
		}
...
// functionDeclaration reports the registry name and config written into
// auto-created schemas, mirroring the official clients.
