	...
	return function.NewFunctionTool(
		...
		function.WithInputSchema(&tool.Schema{
			...
			Properties: map[string]*tool.Schema{
				...
				"max":   {Type: "number", Description: "Maximum value (exclusive)"},
				"type":  {Type: "string", Description: "Type of random value", Enum: []any{"integer", "float"}},
				"count": {Type: "integer", Description: "Number of random values to generate", Default: 1},
			},
		}),
	)
}
...
func generateRandom(_ context.Context, req randomRequest) (randomResponse, error) {
	...
	for i := 0; i < req.Count; i++ {
		if req.Type == "integer" {
			values[i] = float64(rng.Intn(int(req.Max)-int(req.Min)) + int(req.Min))
		} else {
			values[i] = req.Min + rng.Float64()*(req.Max-req.Min)
		}
	}
