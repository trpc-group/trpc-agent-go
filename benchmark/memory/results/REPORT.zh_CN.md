...
> 注：trpc-agent-go 使用 Auto 提取 + pgvector（第 4 节中表现最佳的配置）作为跨框架对比的代表。ADK 和 AutoGen 数据已更新为含 per-QA token 追踪的重跑结果。
...
ADK 的 Memory 场景遭遇了 **122/1,986 个 QA（6.1%）因 `ContextWindowExceededError` 失败**。ADK 将所有对话轮次存储为 session events，依赖 Agent 调用 `LoadMemoryTool` 进行检索。当 Agent 加载过多记忆内容时，组合上下文（高达 234K tokens）超过 GPT-4o-mini 的 128K token 限制，导致请求失败并返回空预测（F1=0）。这 122 个零分 QA 包含在 ADK 报告的 F1（0.301）中；排除后 F1 为 0.320。
...
| 4 类均值（不含 adv） | **0.380** | 0.255 | 0.181 | 0.246 | 0.084 |
...
>
> **注**：ADK 和 AutoGen 数据来自含 per-QA token 追踪的重跑结果。
> ADK 的 65K tokens/QA 极高，因 ADK 将完整对话历史作为 session
> events 加载，每次 Agent 调用 `LoadMemoryTool` 均消耗大量 context。

### 6.6 Token 效率：F1 per 1M Tokens

**F1/1M Tokens** 是衡量框架 token 使用效率的核心指标：**每消耗 100 万 token 能获得多少 F1 质量**。该指标同时反映了记忆检索的精度和上下文管理的效率。

**表 15b：F1 per 1M Tokens — Memory 场景**
...
> trpc-agent-go 的 F1/1M Tokens（182.6）是 Agno 的 **21 倍**、ADK 的
> **40 倍**、CrewAI 的 **1,826 倍**。与 AutoGen 相比，trpc-agent-go 的
> F1/1M 为其 70.4%——差距主要来自 AutoGen 的单次 top-30 检索注入策略
> 在基准测试中的优势，而 trpc-agent-go 的双次调用（检索+回答）模式
> 提供了更灵活的记忆交互方式。

**表 15c：成本效率分析（GPT-4o-mini 定价）**

| 框架 | F1 | 估算成本（$） | F1/$ | F1/1M Tokens | F1/调用 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
> 按 GPT-4o-mini 定价（prompt $0.15/1M，completion $0.60/1M）估算。
> trpc-agent-go 的总成本（$0.62）仅为 ADK 的 **3.2%**、Agno 的 **4.7%**，
> 在成本约束下提供了极具竞争力的性价比。
...
为全面评估各框架在生产场景中的适用性，我们构建了一个多维度加权综合评分体系。各维度权重反映生产部署中的优先级：**记忆质量（F1，40%）**、**Token 效率（F1/1M，25%）**、**对抗鲁棒性（Adv.F1，20%）**、**可靠性（完成率，15%）**。

**表 17：综合评分 — 多维度排名**

| 框架 | F1 | F1/1M | Adv.F1 | 可靠性 | 综合分 | 排名 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
> trpc-agent-go 综合评分（0.844）仅次于 AutoGen（0.897），以 5.3% 的
> 微小差距位居第二。两者的差距集中在 F1 维度（0.363 vs 0.442），
> 而在对抗鲁棒性（0.771 vs 0.395）和 Token 效率（182.6 vs 259.5）上，
> trpc-agent-go 分别领先 95% 和接近 70% 水平。这表明 trpc-agent-go 是
> **唯一在质量、效率、安全性和可靠性之间取得全面均衡的框架**。

### 6.9 分析

**1. trpc-agent-go 的 Token 效率位居第二，仅次于 AutoGen。** trpc-agent-go 的 F1/1M Tokens（182.6）是 ADK 的 **40 倍**、Agno 的 **21 倍**。这意味着在固定 Token 预算下，trpc-agent-go 能提供远超 ADK 和 Agno 的记忆质量。与 AutoGen（259.5）相比，差距主要来自检索策略而非架构效率——AutoGen 采用单次 top-30 向量检索直接注入（1.0 call/QA），而 trpc-agent-go 使用双次调用（检索+回答，2.0 calls/QA），后者提供了更灵活的 Agent 交互模式。

**2. trpc-agent-go 实现 100% 完成率，零上下文溢出失败。** 与 ADK 因上下文窗口溢出（128K 限制被突破，最高达 234K tokens）导致 6.1% QA 失败不同，trpc-agent-go 的记忆检索架构天然管理上下文预算——检索到的记忆受 top-K 检索限制约束，确保每个问题都能获得有效响应。这种可靠性对于静默失败不可接受的生产部署至关重要。

**3. trpc-agent-go 在可靠框架中展现最强对抗鲁棒性。** adversarial F1 为 0.771，仅次于 CrewAI（0.815），在正确识别不可回答问题方面表现突出。AutoGen（0.395）和 ADK（0.306）在对抗性问题上明显挣扎，当未检索到相关记忆时常产生幻觉。这一鲁棒性对生产安全至关重要——一个对不可回答问题自信地编造答案的 Agent，比承认不确定性的 Agent 更危险。

**4. trpc-agent-go 具有最佳的延迟-质量权衡。** 平均延迟 5,235ms 下达到 F1=0.363，而 ADK 需要 10,487ms 却只有 F1=0.301（且有 6.1% 失败），Agno 需要 19,728ms 才有 F1=0.382。以每秒延迟的 F1 计，trpc-agent-go（0.069 F1/s）比 ADK（0.029 F1/s）高 2.4 倍，比 Agno（0.019 F1/s）高 3.6 倍。

**5. ADK 的 Token 消耗暴露了架构缺陷。** ADK 每 QA 消耗 65,076 tokens——是 trpc-agent-go 的 **32.7 倍**——原因是 ADK 将完整对话历史加载为 session events，Agent 每次调用 `LoadMemoryTool` 都会加载大量上下文。这不仅导致成本极高（$19.42 vs trpc-agent-go 的 $0.62），还是上下文溢出失败（6.1%）的根本原因。

**6. CrewAI 的内部开销不可接受。** CrewAI 每个问题消耗 350 万 tokens——是 trpc-agent-go 的 1,749 倍——原因在于其架构模式（每个问题创建新的 Crew 对象并运行复杂的内部推理链）。尽管底层概念相同（向量搜索 + embedding），但这使其在大规模记忆评估中不实用。

**7. Agno 的 LLM 事实提取以延迟换取边际质量提升。** Agno 对每个对话轮次使用 LLM 调用提取结构化事实，总评估时间达 10.9 小时（trpc-agent-go 仅 2.9 小时）。尽管有 3.8 倍的开销，其 Memory F1（0.382）仅比 trpc-agent-go（0.363）高 5.2%，且 F1/1M Tokens（8.7）仅为 trpc-agent-go（182.6）的 4.8%，表明原始文本 + 向量搜索是比 LLM 事实提取更高效的范式。

**8. 多跳推理是关键差异化因素。** AutoGen 在 multi-hop 上遥遥领先（0.499），其次是 ADK（0.367）和 Agno（0.280）。trpc-agent-go 的 Auto 模式在 memory 场景下 multi-hop 仅得 0.091，说明当前检索策略能获取相关的单条事实，但难以跨对话片段组合推理。这是主要的改进方向。

**9. trpc-agent-go 提供最均衡的性能-可靠性-成本特征。** 综合零失败、强对抗鲁棒性（0.771）、有竞争力的保留率（76.6%）、合理延迟（5.2s）、极低 token 成本（1,988/QA，$0.62 总成本）以及多存储后端选项（pgvector/MySQL），trpc-agent-go 在所有评估框架中为生产部署提供了最均衡的记忆解决方案。在综合评分中排名第二（0.844），仅次于 AutoGen（0.897），但在安全性（对抗鲁棒性领先 95%）和成本可控性上具有明显优势。
