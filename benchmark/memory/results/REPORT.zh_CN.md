...
> 优化版 F1 从 0.399 提升至 **0.458**（+14.8%），已达到
> Long-Context F1 的 **97.7%**（原版为 85.1%）。虽然名义
> Tokens/QA（17,359）较高，但**其中 43.5% 命中 prompt cache**，
> 实际新增 token 成本仅 ~9,815/QA（详见 4.5 节）。
...
| --- | ---: | ---: | ---: |
| 5 类加权 (÷1986) | **0.469** | 0.458 | 0.399 |
| 4 类加权 (÷1540，排除 adversarial) | 0.411 | **0.421** | 0.279 |

> 优化版在 single-hop、multi-hop、temporal、open-domain 四项上
> 均实现提升。其中 multi-hop 从 0.096 提升至 0.445（+364%），
> 是改善最显著的类别。temporal 从 0.088 提升至 0.222（+152%），
> 提升幅度位居第二。adversarial 有所下降（0.814 → 0.585），
...
| **平均** | **199** | **0.469** | **0.458** | **0.399** |

> 优化版在全部 10 个样本上均优于原版，其中 4 个样本甚至超过
> Long-Context。
...
- **Token 特征**：tool-call 模式导致每步重读前序上下文，名义
  prompt token 为 ~17,359/QA。但**其中 43.5% 命中了提供商的
  prompt cache**（OpenAI `cached_tokens`），实际*新增* prompt
  成本仅 ~9,815 tokens/QA——按计费口径（大多数提供商 cache
...
| Tokens/QA | 17,359（有效 9,815†） | **1,943** | 2,839 | 49,224 | 10,436 |

> † 优化版 43.5% 的 prompt tokens 命中提供商 prompt cache，
> 实际*新增* token 成本仅 ~9,815/QA。
>
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| single-hop | 282 | **0.388** | 0.377 | 0.322 | 0.316 | 0.299 | 0.240 |
| multi-hop | 321 | 0.445 | **0.512** | 0.380 | 0.096 | 0.418 | 0.283 |
| temporal | 96 | **0.222** | 0.176 | 0.140 | 0.088 | 0.120 | 0.076 |
| open-domain | 841 | 0.446 | **0.594** | 0.501 | 0.358 | 0.494 | 0.292 |
| adversarial | 446 | 0.585 | 0.272 | 0.448 | **0.814** | 0.163 | 0.556 |

**表 9：加权平均 F1**

| 平均方式 | trpc (优化版) | AutoGen | CrewAI | trpc (原版) | ADK | Agno |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 5 类加权 (÷1986) | **0.458** | 0.457 | 0.427 | 0.399 | 0.362 | 0.332 |
| 4 类加权 (÷1540) | 0.421 | **0.511** | 0.420 | 0.279 | 0.420 | 0.267 |
...
| trpc-agent-go (优化版) | **0.458** | 34,590,410 | 17,359 | **43.5%** | **9,815** | 13.2 |
...
> 优化版的*名义* token 数较高（17,359/QA），原因是多步 tool-call
> 模式中每步需重读前序上下文。然而，**43.5% 的 prompt tokens
> 命中了提供商的 prompt cache**（34.47M prompt tokens 中有 14.98M
> 为 cached），实际*新增* prompt 成本仅约 9,815 tokens/QA。按照
...
**优化版耗时更长的原因分析（4h35m vs 3h40m）：**

优化版消耗 5.7 倍的 tokens/QA（17,359 vs 3,056），单 QA 延迟增长
1.25 倍（8,303ms vs 6,659ms）。根因在于三步 Agent 工作流：
...
与前序步骤共享大量公共前缀。实际运行中，**43.5% 的 prompt tokens
（34.47M 中的 14.98M）命中了提供商的自动 prompt cache**，实际
新增 prompt 量仅为 ~19.49M tokens。按照标准 50% cache 定价，
实际可计费的 prompt 成本等效于 ~26.98M tokens 而非 34.47M——
比名义数字**低约 22%**。

尽管 token 成本更高，优化版的 F1/成本权衡显著更优：以 **5.7 倍
名义 token 成本**（计入 cache 折扣后远低于此）换取 **+14.8% F1
提升**（0.399→0.458），在重视回答质量的生产场景中是值得的。
...
| **平均** | **199** | **0.458** | 0.457 | 0.427 | 0.399 | 0.362 | 0.332 |
...
1. **trpc-agent-go（优化版）在 5 类加权 F1 上排名第一**（0.458），
   ...
   **0.458**（+14.8%），达到 Long-Context 上界的 **97.7%**。
   四项知识类别均有大幅改善，其中 multi-hop 从 0.096 跃升至
   0.445（+364%），temporal 从 0.088 跃升至 0.222（+152%）。
