...
1. **Long-Context 模式**与 A-Mem 处于同一水平（Overall J 0.480 vs 0.484），但显著低于 Mem0（0.669）和 Zep（0.660）。原因在于 Mem0 论文的 Full-context 使用完整 50 样本对话且 Judge 跑 10 次取均值，而本工作 Long-Context 仅用 10 样本且 Judge 跑 1 次

2. **Auto 记忆提取**（0.249）低于大多数框架。Auto 模式的核心劣势在于 temporal 类别极低（0.060 vs Mem0 的 0.489），以及 multi-hop 表现不足（0.088 vs Mem0 的 0.286）

3. **Single-Hop 是亮点**：trpc-agent (LC) 的 single-hop F1（0.330）接近 OpenAI Memory（0.343），说明单跳事实检索能力不弱

4. **Multi-Hop 是 Long-Context 的优势**：trpc-agent (LC) 的 multi-hop F1（0.319）**超越所有记忆框架**（Mem0 最高仅 0.286），这验证了完整对话上下文在多跳推理中的价值

5. **Temporal 是最大短板**：所有 trpc-agent 场景的 temporal F1 均低于 0.1，而 Mem0g 达到 0.516。改进时间推理是首要优化方向

> **可比性说明：**
> - 本工作使用 10 样本子集，Mem0 论文使用完整 LoCoMo（50 样本），数据分布可能不同
> - 两者均使用 GPT-4o-mini 推理；本工作的 LLM Judge 跑 1 次，Mem0 跑 10 次取均值
> - Mem0 记忆通过 API 直接搜索+回答，本工作 Auto/Agentic 通过 Agent 工具调用实现
> - 本工作延迟为包含完整 Agent 工具调用链路的端到端延迟
