...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Long-Context | **0.469** | **0.426** | **0.526** | 18,767 | 1.0 | 2,607ms | 1h26m |
| Auto pgvector (优化版) | 0.458 | 0.422 | 0.527 | 17,359 | 3.0 | 8,303ms | 4h35m |
| Auto pgvector (原版) | 0.351 | 0.326 | 0.384 | 3,842 | 2.0 | 5,587ms | 3h05m |

> 优化版 F1 从 0.351 提升至 **0.458**（+30.6%），已达到
> Long-Context F1 的 **97.7%**（原版为 74.8%）。

**表 2：各类别 F1**

| 类别 | Count | Long-Context | 优化版 | 原版 | 优化提升 |
| --- | ---: | ---: | ---: | ---: | ---: |
| single-hop | 282 | 0.320 | **0.388** | 0.276 | +40.6% |
| multi-hop | 321 | 0.308 | **0.445** | 0.108 | +312.0% |
| temporal | 96 | 0.088 | **0.222** | 0.098 | +126.5% |
| open-domain | 841 | **0.518** | 0.446 | 0.310 | +43.9% |
| adversarial | 446 | **0.667** | 0.585 | 0.704 | -16.9% |

**表 3：加权平均 F1**

| 平均方式 | Long-Context | 优化版 | 原版 |
| --- | ---: | ---: | ---: |
| 5 类加权 (÷1986) | **0.469** | 0.458 | 0.351 |
| 4 类加权 (÷1540，排除 adversarial) | 0.411 | **0.421** | 0.249 |

> 优化版在 single-hop、multi-hop、temporal、open-domain 四项上
> 均实现提升。其中 multi-hop 从 0.108 提升至 0.445（+312%），
> 是改善最显著的类别。temporal 从 0.098 提升至 0.222（+127%），
> 提升幅度位居第二。adversarial 有所下降（0.704 → 0.585），
...
| --- | ---: | ---: | ---: | ---: |
| locomo10_1 | 199 | 0.455 | **0.461** | 0.358 |
| locomo10_2 | 105 | **0.496** | 0.428 | 0.299 |
| locomo10_3 | 193 | **0.527** | 0.481 | 0.401 |
| locomo10_4 | 260 | **0.466** | 0.439 | 0.341 |
| locomo10_5 | 242 | 0.433 | **0.486** | 0.324 |
| locomo10_6 | 158 | **0.511** | 0.474 | 0.248 |
| locomo10_7 | 190 | **0.461** | 0.439 | 0.375 |
| locomo10_8 | 239 | 0.453 | **0.466** | 0.350 |
| locomo10_9 | 196 | 0.450 | **0.456** | 0.384 |
| locomo10_10 | 204 | **0.471** | 0.439 | 0.394 |
| **平均** | **199** | **0.469** | **0.458** | **0.351** |
...
- **问题**：tool-call 模式导致每一步都需重读前序全部上下文，
  prompt 膨胀至 ~17,359 tokens/QA；结构化 JSON 格式增加序列化
  开销
...
- **问题**：**token 消耗灾难性膨胀**（49,224 tokens/QA，
  是优化版的 2.8 倍）；9 个 QA 超过 128K tokens 导致上下文
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| **trpc-agent-go (优化版)** | **0.458** | **0.422** | 0.527 | 17,359 | 3.0 | 8,303ms | 4h35m |
...
| trpc-agent-go (原版) | 0.351 | 0.326 | 0.384 | 3,842 | 2.0 | 5,587ms | 3h05m |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| single-hop | 282 | **0.388** | 0.377 | 0.322 | 0.276 | 0.299 | 0.240 |
| multi-hop | 321 | 0.445 | **0.512** | 0.380 | 0.108 | 0.418 | 0.283 |
| temporal | 96 | **0.222** | 0.176 | 0.140 | 0.098 | 0.120 | 0.076 |
| open-domain | 841 | 0.446 | **0.594** | 0.501 | 0.310 | 0.494 | 0.292 |
| adversarial | 446 | 0.585 | 0.272 | 0.448 | **0.704** | 0.163 | 0.556 |

**表 9：加权平均 F1**

| 平均方式 | trpc (优化版) | AutoGen | CrewAI | trpc (原版) | ADK | Agno |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 5 类加权 (÷1986) | **0.458** | 0.457 | 0.427 | 0.351 | 0.362 | 0.332 |
| 4 类加权 (÷1540) | 0.421 | **0.511** | 0.420 | 0.249 | 0.420 | 0.267 |

> 5 类加权 F1 优化版 **0.458** 排名第一，与 AutoGen（0.457）
> 基本持平。4 类加权 0.421 低于 AutoGen（0.511），差距 0.090。

### 4.5 Token 效率与延迟

**表 10：Token 效率对比**

| 框架 | F1 | Total Tokens | Tokens/QA | F1/十亿 Tokens |
| --- | ---: | ---: | ---: | ---: |
| AutoGen | 0.457 | 3,859,412 | 1,943 | 118.4 |
| CrewAI | 0.427 | 5,639,085 | 2,839 | 75.7 |
| trpc-agent-go (原版) | 0.351 | 7,687,973 | 3,842 | 45.7 |
| trpc-agent-go (优化版) | **0.458** | 34,590,410 | 17,359 | 13.2 |
| Agno | 0.332 | 20,725,728 | 10,436 | 16.0 |
| ADK | 0.362 | 97,759,453 | 49,224 | 3.7 |

> AutoGen 的 token 效率最高（118.4 F1/十亿 Tokens），以极低的
> token 消耗获得了 0.457 的 F1。CrewAI 效率排第二（75.7），以
> 2,839 tokens/QA 达到 0.427 的 F1。优化版以 17,359 tokens/QA
> 的更高 token 成本换取了最高的 F1（0.458），效率为
> 13.2 F1/十亿 Tokens。ADK 效率最低——49,224 tokens/QA 仅获得
> 0.362 的 F1。
...
**优化版耗时更长的原因分析（4h35m vs 3h05m）：**

优化版消耗 4.5 倍的 tokens/QA（17,359 vs 3,842），单 QA 延迟增长
1.49 倍（8,303ms vs 5,587ms）。根因在于三步 Agent 工作流：
...
的 Agent 模式，但每次检索到的记忆条目更少更短（两步总计 ~3,842
tokens），因为原版存储的是原始对话 turn，而非提取后的结构化
fact/episode。

尽管 token 成本更高，优化版的 F1/成本权衡显著更优：以 **4.5 倍 token
成本**换取 **+30.6% F1 提升**（0.351→0.458），在重视回答质量的生产
场景中是值得的。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| conv-26 | 199 | **0.469** | 0.384 | 0.355 | 0.358 | 0.337 | 0.296 |
| conv-30 | 105 | 0.430 | **0.451** | 0.439 | 0.299 | 0.379 | 0.334 |
| conv-41 | 193 | 0.507 | **0.513** | 0.440 | 0.401 | 0.335 | 0.387 |
| conv-42 | 260 | 0.423 | **0.439** | 0.408 | 0.341 | 0.343 | 0.338 |
| conv-43 | 242 | **0.477** | 0.486 | 0.413 | 0.324 | 0.355 | 0.341 |
| conv-44 | 158 | 0.464 | **0.491** | **0.509** | 0.248 | 0.384 | 0.289 |
| conv-47 | 190 | **0.489** | **0.496** | 0.405 | 0.375 | 0.374 | 0.321 |
| conv-48 | 239 | 0.461 | **0.463** | 0.432 | 0.350 | 0.392 | 0.328 |
| conv-49 | 196 | **0.440** | 0.418 | 0.407 | 0.384 | 0.371 | 0.302 |
| conv-50 | 204 | 0.421 | **0.475** | **0.487** | 0.394 | 0.363 | 0.374 |
| **平均** | **199** | **0.458** | 0.457 | 0.427 | 0.351 | 0.362 | 0.332 |

> 优化版在 10 个样本中的 4 个上超过 AutoGen。
...
| **trpc-agent (优化版)** | **0.388** | 0.445 | 0.446 | 0.222 | 0.421 | 本工作 |
...
| trpc-agent (原版) | 0.276 | 0.108 | 0.310 | 0.098 | 0.249 | 本工作 |
...
1. **trpc-agent (优化版)** 4 类加权 F1 达到 **0.421**，超越
   Zep (0.403)、LangMem (0.362)、A-Mem (0.347) 等专用记忆系统，
   与 Mem0 (0.421)、Mem0g (0.422) 基本持平
2. **single-hop 排名第二** (0.388)，仅次于 Mem0 (0.387)
3. **multi-hop 排名第三** (0.445)，仅次于 AutoGen (0.512) 和
   ADK (0.418)，远超 Mem0 (0.286)
4. **temporal** (0.222) 仍是主要差距——Mem0/Mem0g 在此项达到
   0.489/0.516，为后续优化方向
...
1. **trpc-agent-go（优化版）在 5 类加权 F1 上排名第一**（0.458），
   是本次评估中所有框架的最高分。F1 从原版的 0.351 提升至
   **0.458**（+30.6%），达到 Long-Context 上界的 **97.7%**。
   四项知识类别均有大幅改善，其中 multi-hop 从 0.108 跃升至
   0.445（+312%），temporal 从 0.098 跃升至 0.222（+127%）。

2. **全面均衡的类别表现。** 优化版在 temporal 上取得了所有框架
   中的最高分（0.222），同时在 single-hop（0.388）和
   multi-hop（0.445）上保持了有竞争力的表现，adversarial 上
   达到 0.585，远高于其他框架中存在的对抗鲁棒性不足问题。相比
   之下，其他框架普遍存在"偏科"现象——在部分类别表现尚可，
   但在其他类别存在明显短板。

3. **与 Mem0 等专用记忆系统持平。** 4 类加权 F1 达到 0.421，
...
5. **Memory 是生产 Agent 的必需能力。** Long-Context 在单 session
   ...
   接近 Long-Context 质量（97.7%）的同时，提供了持久化、可扩展的
   跨 session 记忆能力。

6. **temporal 是下一步重点优化方向。** 优化版 temporal 从 0.098
...
| Auto pgvec (原版) | 0.276/0.220/0.256 | 0.108/0.092/0.100 | 0.098/0.067/0.215 | 0.310/0.280/0.384 | 0.704/0.704/0.704 |
...
| Auto pgvec (优化版) | 34,474,411 | 115,999 | 34,590,410 | 5,989 | 3.0 |
| Auto pgvec (原版) | 7,630,808 | 57,165 | 7,687,973 | 4,017 | 2.0 |
