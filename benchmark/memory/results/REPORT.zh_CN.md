...
> Token 统计仅含 QA 推理；Agno 的记忆摄入（约 8,700 万额外 token）及
> judge token 已排除。CrewAI 标 ‡ 的值为修正值，详见下方说明。

> **‡ CrewAI token 统计 bug。** CrewAI 的 `TokenProcess` 计数器
> 在 QA 迭代间累积不重置。我们的评测代码复用同一个 `Agent` 对象，
> 导致每次 `crew.kickoff()` 返回的是 agent 创建以来的**累积总量**
> 而非本次增量。将这些累积值作为增量求和产生 **O(N^2) 虚报**：
> 报告值 69 亿 prompt tokens / 41.6 万次调用，实际约 6,600 万
> prompt tokens / 约 4,000 次调用。上表修正值（33,306 tokens/QA、
> $10.00、2.01 calls/QA）通过将报告总量除以 Σ N_i(N_i+1)/2 再乘
> 以 N 得出。日志可验证：单样本（conv-42）内 prompt tokens 从 610
> 线性增长到 128K，仅在 128,034 tokens 处触发一次上下文溢出。
>
> 即使修正 token 统计，CrewAI 仍然 F1 最低：**Memory F1 (0.267)
> < Baseline F1 (0.488)**。内置 `short_term_memory` 将之前所有
> QA 对话线性累积到 prompt 中，随历史增长质量下降。仅 adversarial
> F1 提升（0.815 vs baseline 0.431），因噪声上下文导致模型倾向
> 回答"信息不可用"。
...
> trpc-agent-go 的 F1/1M（182.6）是 Agno/CrewAI 的 **21 倍**、
> ADK 的 **40 倍**。与 AutoGen（259.5）的差距源于检索策略差异：
> AutoGen 使用单次 top-30 注入（1.0 call/QA），trpc-agent-go 使用
> 双次调用模式（检索+回答，2.0 calls/QA），后者在生产 Agent 工作流
> 中更灵活。
...
> 在 conv-42（260 QA）中，152 个 QA 得分 F1=0——大部分是非对抗性问题
> 在记忆丢失后被错误拒答。
>
...
> trpc-agent-go（0.844）仅落后 AutoGen（0.897）5.3%。差距集中在
> F1（0.363 vs 0.442）；trpc-agent-go 在对抗鲁棒性上领先（0.771 vs
> 0.395），成本可比（$0.62 vs $0.51，而 ADK $19.42、Agno $13.31）。
...
3. **trpc-agent-go 在 Agent 框架中综合排名第二**（0.844），仅落后
   AutoGen（0.897）5.3%。
   ...
   **trpc-agent-go 的核心优势：**
   - **Token 效率极高：** F1/1M Tokens 达 182.6，是 Agno 的 21 倍、
     ADK 的 40 倍。完整 1,986 QA 评估仅需 $0.62（ADK $19.42，
     Agno $13.31）
   - **评估速度快：** 总耗时 2h53m，排名第二（仅次于 AutoGen 2h23m），
     远优于 ADK（4h42m）和 Agno（10h53m）
...
4. **LLM-as-Judge 质量与所有框架持平。** 使用相同 positive-only
   ...
   框架中**最高**（AutoGen 0.932, ADK 0.936, Agno 0.934,
   CrewAI 0.920）。报告的低值（0.373）反映的是 Go 实现的全样本
   分母聚合方式，而非回答质量问题。
...
| **平均** | **199** | **0.444** | **0.386** | **0.359** | **0.304** | **0.270** |
...
> Agno 最慢，因记忆摄入阶段的 LLM 事实提取额外消耗约 8,700 万
> tokens。
