...
本报告使用 **LoCoMo**（Long-Context Conversational Memory）基准评估 trpc-agent-go 的记忆能力，在两种存储后端上对比四种不同的记忆范式，分析各自在不同问题类别上的优劣。

**核心发现：**

- **长上下文基线** 取得最高 F1（0.472），在上下文窗口允许的情况下，完整对话仍是最优方案
- **自动记忆提取**（F1=0.357 pgvector）是最强的记忆方案，达到长上下文基线的 75.6%
- **RAG 记忆**（F1=0.325 pgvector）提供强大的对抗鲁棒性（0.955），但事实召回能力较弱
- **Agentic 记忆**（F1=0.294 pgvector）显示 LLM 驱动的记忆提取面临信息密度挑战
- **pgvector 始终优于 MySQL** 2-7% F1，验证了向量相似度搜索的价值
...
| **RAG** | 预计算的 observation 存为记忆 | 批量写入 | Top-K 检索 |
| **Agentic** | LLM Agent 通过工具调用决定存储内容 | LLM 工具调用 (memory_add) | Top-K 检索 |
| **Auto** | 后台提取器自动生成记忆 | 异步提取 | Top-K 检索 |
...
> **已知问题**：RAG 和 Auto 场景中 locomo10\_2/5/6/7/8 五个样本的 observation **解析失败**导致记忆为空。详见 4.5 节

### 3.2 各类别 F1 分数

| 类别 | Long-Context | RAG pgvec | RAG MySQL | Agentic pgvec | Agentic MySQL | Auto pgvec |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| single-hop | 0.330 | 0.166 | 0.127 | 0.146 | 0.168 | 0.272 |
| multi-hop | 0.319 | 0.086 | 0.098 | 0.178 | 0.135 | 0.088 |
| temporal | 0.088 | 0.075 | 0.057 | 0.091 | 0.043 | 0.060 |
| open-domain | 0.518 | 0.163 | 0.140 | 0.126 | 0.146 | 0.302 |
| adversarial | 0.668 | **0.955** | **0.944** | 0.830 | 0.787 | 0.771 |

### 3.3 各样本 F1 分数

| 样本 | QA 数 | Long-Context | RAG pgvec | Auto pgvec | Agentic pgvec |
| --- | ---: | ---: | ---: | ---: | ---: |
...
| **平均** | **199** | **0.472** | **0.325** | **0.357** | **0.294** |
...
#### 4.1.2 自动提取优于 RAG 和 Agentic

自动记忆提取（F1=0.357）优于 RAG（0.325）和 Agentic（0.294）：

- **vs. RAG**：自动提取生成更高密度、语义更丰富的记忆。单跳 F1 从 0.166 提升至 0.272（+64%）
- **vs. Agentic**：自动提取更系统化——处理所有对话内容，而非依赖 LLM Agent 的选择性工具调用

#### 4.1.3 对抗鲁棒性与召回能力呈反相关

RAG 取得最高对抗 F1（0.955），而长上下文仅 0.668。这是因为基于记忆的方法在未检索到相关记忆时自然返回"信息不可用"，这恰好是对抗性问题的正确答案。而长上下文有完整文本可用时，更容易产生看似合理但实际错误的回答。
...
Agentic（pgvector）在记忆方案中取得最高多跳 F1（0.178），超越 RAG（0.086）和 Auto（0.088）。这表明 Agentic 方法虽然总体提取的记忆较少，但创建了更具关联性的知识，有助于多跳推理。`datePrefixMemoryService` 注入的 `[DATE:]` 前缀对此有贡献。
...
- **RAG 场景差距最大**：RAG 存储较长的 observation 文本，语义检索优势更明显
...
- **RAG pgvector**：0.202 - 0.450（范围 0.248），方差较大
- **Auto pgvector**：0.204 - 0.441（范围 0.237），方差较大

记忆方案的高方差表明，某些对话结构本质上更难进行记忆提取和检索。话题交错更复杂的样本（如 locomo10_6, locomo10_8）得分往往更低。

### 4.5 Observation 解析 Bug（严重）

**影响**：RAG 和 Auto 场景中 10 个样本里有 5 个（locomo10\_2/5/6/7/8）**observation 写入为零**，导致这些样本所有非对抗性问题全部返回 "The information is not available."（F1=0）

**根因**：Go 类型 `locomo10Observation` 定义为 `map[string]map[string][][]string`，要求最内层的 evidence 引用为 `string`。但 LoCoMo-10 部分条目的 evidence 引用是**嵌套列表**（如 `["D15:3", "D15:5"]` 而非 `"D15:3"`），导致 `json.Unmarshal` 对整个 `observation` 字段解析失败，`parseObservations()` 返回 `nil`
...
**后果**：RAG 场景的 F1 分数被严重拉低——5 个正常样本的 F1 实际更高（0.399-0.450），但因另外 5 个零分样本拉低了整体均值。Agentic 和 Long-Context 场景**不受影响**，因为它们不依赖预解析的 observation

**修复方案**：将 observation entry 类型从 `[]string` 改为 `[]interface{}`（或 `[]json.RawMessage`），仅提取第一个元素作为 observation 文本，后续元素作为 evidence 引用（不论类型）
...
| **trpc-agent-go (RAG pgvector)** | gpt-4o-mini | **0.325** | 本工作 |
| RAG-Observation (LoCoMo) | GPT-3.5 | 0.414 | LoCoMo 论文 |
...
2. **强对抗鲁棒性**：记忆方案达到 0.77-0.96 的对抗 F1，能正确识别不可回答问题
...
核心结论：
1. **Auto > RAG > Agentic**：在 gpt-4o-mini 下的记忆方案有效性排序
