#!/usr/bin/env python3
"""
Memory evaluation benchmark for Microsoft AutoGen framework.
...
# ---------------------------------------------------------------------------
# Scenario: Memory (AutoGen ChromaDBVectorMemory).
# ---------------------------------------------------------------------------
async def _ingest_memories_for_sample(
    sample: dataset.LoCoMoSample,
    memory: ChromaDBVectorMemory,
) -> int:
    """Feed each conversation turn into ChromaDB as a
    MemoryContent entry. Returns the number of entries added.
...
async def evaluate_memory(
    ...
    """Evaluate using AutoGen ChromaDBVectorMemory.

    Phase 1: Ingest conversation turns into ChromaDB
             (embedding only, no LLM calls).
    Phase 2: Create an AssistantAgent with the memory.
             For each question the agent retrieves relevant
             memories via vector search and answers.
    """
    # Prepare a per-sample ChromaDB directory.
    db_dir = os.path.join(
    ...
    entry_count = await _ingest_memories_for_sample(
        sample, chroma_memory,
    )
    ingest_time_ms = int(
    ...
    # Phase 2: QA using AssistantAgent with memory.
    qa_results: list[QAResult] = []
    total_usage = TokenUsage()
    ...
    qa_agent = AssistantAgent(
        ...
        system_message=_QA_INSTRUCTIONS.format(
            not_available=NOT_AVAILABLE,
        ),
    )
    ...
    for qa in sample.qa:
        ...
        try:
            ...
            # Extract the final text from messages.
...
def print_summary(result: dict) -> None:
    ...
    print("=" * 60)
    print(
        f"Memory Evaluation Results - {meta['scenario']}"
    )
    print("=" * 60)
    ...
    print(
        f"BLEU Score: {summary['overall_bleu']:.4f}"
    )
    if summary["overall_llm_score"] > 0:
        print(
            f"LLM Score:  "
            f"{summary['overall_llm_score']:.4f}"
        )
    ...
    if summary["total_llm_calls"] > 0:
        ...
        print(
            ...
            f" (avg "
            f"{summary['total_prompt_tokens'] / qc:.0f}"
            f"/QA)"
        )
        print(
            f"Completion Tokens: "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "AutoGen Python Memory Evaluation Benchmark"
        ),
    )
    parser.add_argument(
        "--model", default="",
    ...
    parser.add_argument(
        "--output", default="../results/autogen_python",
...
async def main_async() -> None:
    ...
    try:
        ...
        log.info(
            "=== AutoGen Python Memory Evaluation "
            "(LoCoMo) ==="
        )
        ...
        if args.max_tasks > 0:
            ...
            log.info(
                "Limited to %d samples", len(samples),
            )
