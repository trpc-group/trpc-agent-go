...
- **`ParameterSchema`**
  ...
  - Important fields:
    - `name` – parameter name (e.g., `"messages"`, `"result"`, `"model_spec"`).
...
- **`builtin.llm`**
  - `inputs`:
    - Primarily `messages` (`type_id: graph.messages, kind: array`) plus model configuration parameters (`model_spec`, `instruction`, `temperature`, `max_tokens`, ...).
  - `outputs`:
...
- **`builtin.llmagent`**
  ...
  - `config_schema`:
    - `model_spec`, `instruction`, `tools`, `tool_sets`, `mcp_tools`, `output_format` (object with `type: "text" | "json"` and `schema` containing the JSON Schema for structured output when `type == "json"`), sampling parameters, etc.
