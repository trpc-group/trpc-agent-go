...
func publicReplayCases(t *testing.T) []replaytest.Case {
	t.Helper()
	cases, err := replaytest.PublicCases(map[string]replaytest.PublicCaseRunner{
		"single_turn": func(ctx context.Context, backend replaytest.Backend) error {
		...
		"multi_turn": func(ctx context.Context, backend replaytest.Backend) error {
			sess, err := createReplaySession(ctx, backend, nil)
			if err != nil {
				return err
			}
		...
		"tool_call_and_response":              replayToolCall,
		"state_update_overwrite_delete":       replayStateLifecycle,
		"session_state_direct_round_trip":     replayDirectStateRoundTrip,
		"memory_search_order_and_score":       replayMemoryKinds,
		"memory_update_and_delete":            replayMemoryLifecycle,
		"summary_filter_and_update":           replaySummaryUpdate,
		"summary_event_window_recovery":       replaySummaryFollowup,
		"track_status_and_error":              replayTracks,
		"concurrent_tool_event_interleaving":  replayConcurrentToolEvents,
		"failure_recovery_without_duplicates": replayFailureRecovery,
	})
	require.NoError(t, err)
	return cases
}
