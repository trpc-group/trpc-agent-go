...
Keyword search and the sparse half of hybrid search rely on the `/search` API provided by [Chroma Cloud](https://docs.trychroma.com/cloud/getting-started). Self-hosted servers support vector and filter search; hybrid search degrades to dense vector search when sparse search is not configured.
...
`WithAPIKey` sends `X-Chroma-Token`. A Cloud API key can be scoped to a database, in which case `WithTenant` and `WithDatabase` may be omitted because both are resolved from the Cloud identity. Inference requires the identity to report exactly one database; keys that report no database or several databases must set both options explicitly.
...
| `WithChromaInstance(name)` | Use a client registered with `storage.RegisterChromaInstance` | - |
| `WithTenant(tenant)` / `WithDatabase(database)` | Tenant and database. Empty uses the server default; with `WithAPIKey` a database-scoped key resolves both from the Cloud identity. Keys that report no database or several databases must set both explicitly. | Server default |
...
| `WithExtraOptions(opts...)` | Pass opaque values to a custom ClientBuilder. The default HTTP builder ignores them. | - |
...
| `WithMaxUpdateRecords(n)` | Maximum records one `UpdateByFilter` call may match | `100000` |
...
| `WithHybridSearchWeights(dense, sparse)` | Dense/sparse RRF weights; normalized to sum to 1 | `0.5, 0.5` |
...
- `WithSpladeBaseURL` and `WithSpladeModel` customize the embedding service address and model when constructing `NewCloudSpladeEmbedder` explicitly; `WithSpladeTransport` customizes the transport (proxy, TLS, timeouts, tracing) used for embedding requests while the redirect policy stays with the package.
...
| Keyword | ⚠️ | Sparse KNN via Cloud `/search`. Errors when `WithSparseSearch` is not configured. |
...
Simple metadata map filters (`SearchFilter.Metadata`, `WithDeleteFilter`, `WithCountFilter`, `WithGetMetadataFilter`) accept user metadata keys only. Passing one of the reserved keys above is rejected with an error; filter the document `name`, `created_at`, or `updated_at` through `FilterCondition` instead.
