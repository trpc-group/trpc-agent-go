...
- `WithToolResultCompactionConfig(...)` also supports tool-name policy: `ForceCleanToolNames` forces selected tool results to be cleaned, while `KeepToolNames` preserves selected tool results and has higher priority.
