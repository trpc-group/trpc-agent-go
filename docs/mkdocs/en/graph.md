...
graphAgent, err := graphagent.New(
    ...
    // Execution engine: BSP (default) or DAG (eager, no checkpoint/interrupt yet).
...
Current MVP limitations (DAG engine):

- **No checkpoints / time travel / resume**: DAG execution does not currently
  support atomic checkpointing. `WithCheckpointSaver` is rejected.
- **No interrupts (HITL)**: static interrupts and external interrupts are not
  supported yet.
...
Reference example: `examples/graph/dag_engine`.
...
DAG‑like pipelines and currently trades off features like checkpoints and HITL
...
- **DAG engine (opt‑in)**: eager scheduling without a global superstep barrier.
  This engine is designed for DAG‑like pipelines and currently does not support
  checkpoints or interrupts (see the “DAG engine” section above).
...
Note: DAG mode currently rejects `WithCheckpointSaver` and does not support
interrupts or `WithStepTimeout`.
