...
The current Agent can manage the visibility of messages generated by other Agents and historical session messages based on different scenarios. This can be configured through related options.
When interacting with the model, only visible content is input to the model. (invocation.Messageis always visible in any scenario.)
