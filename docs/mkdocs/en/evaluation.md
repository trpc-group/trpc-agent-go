...
- **EvalSet** describes covered scenarios and provides evaluation input. Each case organizes Invocations per turn, including user input and expected `tools` traces or `finalResponse` for comparison. Expected traces can be written statically in EvalSet or generated during evaluation via ExpectedRunner.
...
EvalSet describes the set of scenarios covered and provides evaluation input. Each scenario corresponds to an EvalCase, and EvalCase organizes Invocations per turn. In default mode, Runner is driven by `conversation` to produce actual traces. Expected traces come from `conversation` by default; when a case enables `expectedRunnerEnabled`, expected traces are generated during evaluation via ExpectedRunner. In trace mode, inference is skipped and `actualConversation` is used as actual traces. During evaluation, Service passes actual and expected traces to Evaluator for comparison and scoring.
...
type EvalCase struct {
	...
	ExpectedRunnerEnabled bool             // ExpectedRunnerEnabled indicates whether expected outputs are generated by ExpectedRunner during evaluation, optional.
...
The evaluation phase is handled by `Evaluate`. It takes `InferenceResult` as input, loads the corresponding EvalCase, and constructs actuals and expecteds. Expecteds come from `conversation` by default, or are generated via ExpectedRunner when the EvalCase enables `expectedRunnerEnabled`. It then executes evaluators according to `EvaluateConfig.EvalMetrics`.
...
When `evalMode` is `trace`, inference is skipped, actual traces come from `actualConversation`, and expected traces are provided by `conversation` by default, or generated by ExpectedRunner when `expectedRunnerEnabled` is enabled.
...
In some evaluation tasks, you may want dynamic expected outputs rather than static ones. For example, reference answers may need to be generated on the fly by a reference Runner based on input samples. In this case, you can enable `expectedRunnerEnabled` for an EvalCase and inject an ExpectedRunner when creating AgentEvaluator, so the evaluation phase generates expecteds turn by turn.

When `expectedRunnerEnabled=true`, the evaluation phase runs ExpectedRunner turn by turn on the same `userContent` to build expecteds. In default mode, `userContent` comes from `conversation`. In trace mode, `userContent` comes from `actualConversation`. The generated expecteds are aligned by turn with actuals and then passed to Evaluator. In this mode, expected output fields in EvalSet can be omitted as long as `userContent` is present per turn.
