...
对 `AddAgentNode(...)` 的场景，还有一条重要提醒：

- 把 `WithSubgraphInputMapper` 视为传递**结构化 RuntimeState** 的方式，而不是
  手工拼接 prompt 历史的入口。
- 在 `Auto` 模式下，只有当 RuntimeState 同时带有 graph 预组装快照标记和
  `graph.StateKeyMessages`，并且 child agent 默认声明自己偏好 graph-owned
  message snapshot 时，框架才会切换到这套语义。当前这主要面向普通
  `LLMAgent` 子节点；包装过的 child 或 `GraphAgent` 子节点默认仍保持旧的
  self-history 语义，除非你显式 opt-in。
- 单纯通过 `WithSubgraphInputMapper` 透传 `graph.StateKeyMessages`，不会再自行改变
  child 默认的消息归属语义。
- 如果你就是想让 graph 接管 child 的消息视图，请显式设置
