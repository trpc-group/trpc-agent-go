...
- 图的 state（例如 `last_response`、`node_responses[nodeID]`）只会在节点完成
  后提交；下游节点无法从这些 key 读取到“部分的流式内容”。
- 如果你希望下游节点在上游 LLM（或 Agent 节点）仍在 streaming 时就开始处理，可
  以使用 StreamHub：
  - 生产者节点：`graph.WithStreamOutput("name")`
  - 消费者节点：`graph.OpenStreamReader(ctx, "name")`
- Streams 是内存态的临时通道，不参与 checkpoint。要实时消费，通常需要让 consumer
  节点与 producer 节点并行执行，并在后续通过 join 汇合。
- 参考示例：`examples/graph/streaming_node_consumer`。
