...
对 `AddAgentNode(...)` 且子节点是 `LLMAgent` 的场景，还有一条重要提醒：

- 把 `WithSubgraphInputMapper` 视为传递**结构化 RuntimeState** 的方式，而不是
  手工拼接 prompt 历史的入口。
- 如果子 RuntimeState 中已经包含 `graph.StateKeyMessages`，框架可能会把它视为
  graph 为该子节点预组装好的消息快照。此时子 `LLMAgent` 会直接使用这份快照，
  而不是再从 session 重新装填历史。
- 因此，除非你就是想让 graph 接管 child 的消息视图，否则不要通过
  `WithSubgraphInputMapper` 透传 `graph.StateKeyMessages`。
- 如果你需要显式控制这件事，请使用
