...
<a id="synthetic-error-content-in-session-history"></a>
...
该选项只影响 GraphAgent 从 Session 历史进行的消息投影，不会删除错误事件，也不
会改写图节点直接写入 `graph.StateKeyMessages` 的消息。
