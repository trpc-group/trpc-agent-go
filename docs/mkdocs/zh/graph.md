...
默认情况下，会话摘要作为独立的系统消息注入。在某些场景下，你可能希望将它们合并到已有的系统消息中，以减少消息数量并保持更清晰的提示结构。

**为什么需要此功能：**

部分 LLM 提供商对系统消息的位置和数量有严格要求。例如：

- **Qwen3.5 系列**等模型要求系统消息必须位于对话开头，且不支持多条系统消息。若不启用此选项，可能会遇到如下错误：
  ```
  System message must be at the beginning
  ```
- **API 限制**：某些 API 会拒绝包含多条系统消息，或系统消息位于用户/助手消息之后的请求
- **模型行为**：部分模型会优先处理第一条系统消息，而忽略后续的系统消息
...
ga := graphagent.New(
    ...
    graphagent.WithInitialState(initialState),
    graphagent.WithAddSessionSummary(true),
    graphagent.WithSingleSystemMessage(true), // 合并到已有的系统消息
)
```

**工作机制：**

- 当设置 `WithSingleSystemMessage(true)` 且存在已有系统消息时，摘要内容会追加到该系统消息中，而不是创建新的系统消息
- 如果不存在系统消息，摘要仍会作为独立的系统消息注入到开头
- `WithSummaryFormatter` 仍会在合并前应用

**适用场景：**

- **兼容性保障**：确保与要求单条系统消息开头的模型兼容（如 Qwen3.5、部分阿里云模型）
- **减少消息数量**：最小化 prompt 中的系统消息数量
- **统一提示结构**：保持包含指令和上下文的单一连贯系统提示
- **API 合规**：满足限制系统消息位置的 API 要求

**重要注意事项：**

- 此选项需要设置 `WithAddSessionSummary(true)`
- 当 `WithSingleSystemMessage(false)`（默认值）时，摘要始终作为独立的系统消息注入
- 如果你的模型不支持多条系统消息，强烈建议启用此选项
