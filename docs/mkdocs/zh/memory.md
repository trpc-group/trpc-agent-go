...
DeepSearch 是一个可选的记忆检索增强能力。它从已有的 `memory.Entry`
构建 cue/tag 索引，并为 `LLMAgent` 暴露渐进式的第二阶段检索工具。
原始 memory entry 仍然是唯一权威内容；DeepSearch 不会把 raw session
作为独立记忆内容存储。

DeepSearch 默认关闭，需要两层显式开启：

1. 通过 `WithDeepSearch(indexModel)` 开启后端索引能力。
2. 通过 `llmagent.WithMemoryDeepSearch()` 开启渐进式 Agent 工具面。

缺少任意一个配置时，普通 memory 工具和检索行为都保持不变。
`memory_search` 始终调用原始 memory service search。`memory_deepsearch`
只有在当前 invocation 成功调用过 `memory_search` 后才能使用；调用成功后，
DeepSearch 子工具只在当前 invocation 内激活。
...
当前实验性实现支持 `memory/inmemory` 和 `memory/pgvector`。其他 memory
后端保持现有行为，不暴露 DeepSearch 后端选项。
