...
DeepSearch 是长期记忆的可选第二阶段检索能力。它不会替换
`memory.Service`，不会改变记忆写入方式，也不会引入另一份权威记忆。
DeepSearch 从已有的 `memory.Entry` 派生 cue/tag 二级索引，让 Agent 在普通
语义检索证据不足时，可以沿线索继续查找相关记忆。

DeepSearch 遵循以下设计原则：

1. **memory entry 是唯一权威内容**：每个 DeepSearch content 节点都引用已有
   的 `memory.Entry`，不会把 raw session 作为独立 DeepSearch 记忆保存。
2. **普通检索保持不变**：`memory_search` 始终调用原有
   `memory.Service.SearchMemories`，不会静默融合 cue/tag 结果。
3. **能力必须显式开启**：memory service 和 Agent 需要分别选择开启。
4. **工具渐进式披露**：Agent 初始只看到原有 memory 工具和
   `memory_deepsearch`；只有先尝试 `memory_search` 并显式要求深度检索后，
   详细的 DeepSearch 工具才会出现。
...
memory_search
    ...
    +-- 证据缺失或不确定
       ...
       memory_deepsearch
            |
            +-- 验证 memory_search 已成功调用
            +-- 构建或校验当前用户的 cue/tag 索引
            +-- 在当前 invocation 激活 DeepSearch 工具
            |
            v
       cue 检索 -> tag/路径扩展 -> content 加载
            |
            v
           回答
```

`memory_deepsearch` 是能力开关，本身不返回记忆证据。调用成功后，它会在
当前 invocation 内激活详细检索工具，由模型根据问题选择检索路径。激活状态
不会泄漏到后续 invocation。
...
| `Cue` | 未来问题中可能出现的短检索线索，例如“毕业学位”或“京都酒店”。 |
| `Tag` | 将 cue 与 content 连接起来的稳定实体、主题、关系、日期、人物、地点或个人信息方面。 |
| `Content` | 可检索的索引节点，包含 entry 文本和指向源 memory entry 的强类型引用。 |
| `ContentRef` | 通过引用类型、app name、user ID 和 memory entry ID 标识源对象。 |
| `Metadata` | 保存 source fingerprint、记忆类型、topics、event time、participants 和 location，用于结构化过滤。 |
| `Path` | DeepSearch 查询返回的带分数 `Cue -> Tag -> Content` 遍历路径。 |

索引模型按批次从持久记忆生成 cues 和 tags。生成过程采用严格校验：
LLM 调用失败、JSON 非法、返回缺少输入条目、cue/tag 为空都会导致构建失败。
DeepSearch 不会推断替代关键词，也不会在失败时写入部分索引。

每个 content 节点保存 source fingerprint，它由 entry ID、更新时间和 topics
生成。使用索引前，DeepSearch 会将 fingerprint 与当前 memory entries
比较；索引缺失或过期时，会为该用户完整重建。

#### 索引生命周期

索引与长期记忆使用相同的 `<appName, userID>` 隔离范围，并跟随 memory
生命周期维护：

| 事件 | 行为 |
| ---- | ---- |
| 首次成功调用 `memory_deepsearch` | 读取当前 entries 并校验索引；索引缺失或过期时进行构建，并在发布前确认生成期间 entries 未变化。 |
| 后续调用且 fingerprints 一致 | 直接复用现有索引，不调用索引模型。 |
| 后续调用发现 fingerprints 过期 | 暴露 DeepSearch 工具前完整重建当前用户索引。 |
| 索引激活后 Add 或 Update | 同步生成并替换受影响 entry 的索引文档。 |
| 索引激活后 Delete | 删除对应 content 及其 cue/tag 边。 |
| 索引激活后 Clear | 与 memories 一起清空当前用户的 DeepSearch 索引。 |

同一用户首次使用时的并发构建会被合并。新索引提交前会再次校验 source
fingerprints；如果生成过程中 entries 发生变化，构建会直接失败，不会提交
基于不一致快照生成的索引。

#### 渐进式工具面

`llmagent.WithMemoryDeepSearch()` 只会在 Agent 初始工具面中增加
`memory_deepsearch`，不会删除或替换 `memoryService.Tools()` 返回的任何
原有工具。成功激活后，当前 invocation 会获得以下工具：

| 工具 | 作用 |
| ---- | ---- |
| `memory_deepsearch_cue_search` | 根据用户问题查找可能的检索线索。 |
| `memory_deepsearch_tag_expand` | 将 cue ID 或 cue 文本扩展为带分数的 tag/content 路径。 |
| `memory_deepsearch_content_load` | 按 DeepSearch content ID 或源引用加载内容。 |
| `memory_deepsearch_edges_by_tag` | 按 tag 文本或自由文本 query 遍历索引路径。 |
| `memory_deepsearch_conversation_time` | 按时间范围检索 episode memories。 |
| `memory_deepsearch_event_keywords` | 按关键词和可选时间范围检索 episode memories。 |
| `memory_deepsearch_event_context` | 加载与已命中的 cue、tag 或 content 节点相关的内容。 |
| `memory_deepsearch_personal_information` | 按 query 或 aspect 检索稳定个人事实。 |
| `memory_deepsearch_personal_aspect` | 检索某一具体个人信息方面的记忆。 |
| `memory_deepsearch_topic_events` | 按主题和可选时间范围检索 episode memories。 |

模型预期按以下顺序使用：

1. 首先调用 `memory_search`。
2. 返回的 memories 足够时直接回答。
3. 证据缺失、冲突或不完整时调用 `memory_deepsearch`。
4. 使用激活后的工具追踪 cues、扩展关系并加载源记忆证据，再生成回答。

#### 配置方式

DeepSearch 默认关闭，需要两个相互独立的显式配置：

1. 在 memory service 上配置 `WithDeepSearch(indexModel)`。
2. 在 `LLMAgent` 上配置 `llmagent.WithMemoryDeepSearch()`。

同一个 memory service 还必须注册到 Agent tools，并注入 Runner，确保工具
调用可以从 invocation context 中获取该 service。
...
func main() {
    ...
    // index model 负责生成 cues 和 tags，可以与回答问题的模型独立配置。
    ...
    memoryAgent := llmagent.New(
        ...
        llmagent.WithInstruction(
            "回答用户相关问题前先检索记忆；如果第一次检索仍缺少重要证据"+
                "或答案不确定，使用 memory_deepsearch。",
        ),
        // 保留原有 memory 工具面。
        llmagent.WithTools(memoryService.Tools()),
        // 增加渐进式 DeepSearch 能力开关。
    ...
    appRunner := runner.NewRunner(
        ...
        // 让 memory tool calls 从 invocation 获取同一个 service。
    ...
    events, err := appRunner.Run(
        ...
        "user-1",
        "session-1",
        model.NewUserMessage("我大学毕业时获得了什么学位？"),
    )
    if err != nil {
        panic(err)
    }
...
如果只配置 memory service，Agent 的原有工具面不会发生变化，但已配置的
索引可能在 memories 变化时调用 index model 进行维护。如果只配置 Agent
option，由于 invocation 中的 memory service 不提供索引能力，
`memory_deepsearch` 会返回错误。未开启任意配置的应用保持原有 memory
行为，也不会产生 cue/tag LLM 调用或索引维护开销。

生产环境需要关注索引模型的延迟和 token 成本、监控索引生成失败，并保证
用户范围内的 memory 操作一致性。DeepSearch 会显式返回生成和存储错误，
由调用方决定重试或报告运行故障，而不是静默退化为低质量检索。
