...
DeepSearch 是 Memory 的可选第二阶段检索能力。它不会替代 `memory.Entry`，
也不会创建另一份权威记忆来源。DeepSearch 会从已有 memory entry 派生隐藏的
cue/tag 索引，并把这些派生字段依附在同一条 memory row 上。

DeepSearch 保持原有 Memory 模型不变：

- `memory.Entry.Memory.Memory` 仍然是 Agent 可以引用的唯一权威记忆内容。
- cue 和 tag 是派生检索元数据。它们用于在第一轮语义或关键词检索不完整时
  帮助 Agent 找到相关记忆，但不会在公开读取 memory entry 时暴露。
- 默认行为不变。未配置 DeepSearch 时，Memory 不会调用索引模型，不会生成
  DeepSearch 派生字段，`memory_search` 也不会暴露 `search_mode`。

#### 工作原理

当 memory backend 配置 `WithDeepSearch(indexModel)` 后，backend 可以为当前
用户的每条 memory entry 构建隐藏的 row-attached 索引。索引包含：

| 字段 | 作用 |
| ---- | ---- |
| `content` | 引用原始 `memory.Entry` 文本和元数据。 |
| `cues` | 未来用户问题中可能出现的短检索短语。 |
| `tags` | 用于扩展检索的稳定实体、主题、关系、日期、人物、地点或方面。 |
| `source_fingerprint` | 由源 memory ID、更新时间和 topics 计算的摘要，用于判断索引是否过期。 |
| `version` / `indexed_at` | 索引格式版本和创建时间。 |

backend 在使用索引前会检查 fingerprint。memory 被新增、更新、删除或清空后，
派生索引会失效或从当前 memory entries 重建。DeepSearch 失败会显式返回错误；
不会使用确定性关键词 fallback 静默写入部分索引。

#### 启用方式

DeepSearch 需要在 memory backend 上显式启用：
...
启用后，`memoryService.Tools()` 返回的 `memory_search` 声明会支持
DeepSearch。工具默认仍然是普通搜索，只有启用后才会暴露可选的 `search_mode`
参数：
...
如果希望 LLMAgent 使用第二阶段的 cue/tag/content 工具，需要把 DeepSearch
工具注册为可激活工具集，并根据 `memory_search` 返回的
`deepsearch_activated=true` 激活：
...
推荐先使用标准搜索。如果结果缺失、冲突或不足以确定答案，再使用
`search_mode="deepsearch"` 调用同一个 `memory_search`。DeepSearch 调用成功
后会返回普通搜索结果，并带上 `deepsearch_activated=true`；随后 Agent 可以在
当前 invocation 中使用被激活的 cue/tag/content 工具继续查找证据。如果没有
配置上面的 LLMAgent 激活规则，`memory_search` 仍会执行 DeepSearch 索引检查，
但不会向当前工具面额外加入 cue/tag/content 工具。

#### 兼容性

DeepSearch 对存量用户保持安全：

- 未配置 `WithDeepSearch` 时，工具 schema 和运行时行为不变。
- 公开的 `memory.Entry` JSON 不包含隐藏 DeepSearch 字段。
- 普通 `ReadMemories` 和标准 `SearchMemories` 不会把 cue/tag 文本拼接进权威
  memory content。
- DeepSearch index 是派生数据，可以从已有 memory entries 重新构建。
