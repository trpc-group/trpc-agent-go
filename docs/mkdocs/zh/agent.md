...
- 持久化上下文，写入 session transcript，并排在 **当前用户消息之前**（适合作为需要进入历史的业务上下文、检索结果、状态快照）：
...
4. Session history（会话的 canonical transcript；如果本轮使用 `WithSessionContextMessages`，这些消息会先写入 transcript，并排在当前用户消息之前）
...
- `WithSessionContextMessages` 会 **持久化** 到 session transcript；后续回合会继续从历史中看到它，直到 summary/裁剪等历史构建策略不再包含这段历史。
- 如果只是想在当前用户消息前持久加上下文，优先使用 `WithSessionContextMessages`；如果要改写/展开当前用户输入，再使用 `WithUserMessageRewriter`。
...
- `examples/prompt/session_context_messages`：演示持久化 session context，并打印最终模型请求消息。
