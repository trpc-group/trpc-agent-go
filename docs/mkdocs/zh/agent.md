...
llmAgent := llmagent.New(
    ...
    llmagent.WithMessageTimelineFilterMode(llmagent.BranchFilterModeAll),                                      // 设置传给模型的消息按分支时间维度的过滤模式，可选值：all、request、invocation, 默认值: all
    llmagent.WithMessageBranchFilterMode(llmagent.TimelineFilterAll),                                     // 设置传给模型的消息按分支维度的过滤模式, 可选值: all、prefix、exact
)
```
