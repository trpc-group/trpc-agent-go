...
- 持久化上下文，写入 session transcript，并排在 **当前用户消息之前**：
  - `agent.WithSessionContextMessages([]model.Message{...})`
  - `agent.WithSessionContextMessagesFunc(...)`
- 带稳定名字、由 source 自己维护 state 的持久上下文 producer。适合让框架记住
  某个 source 上一次 materialize 到历史里的内容，并在需要时提醒 source 重新输出完整快照：
  - `agent.WithSessionContextSource("name", func(ctx, args) (*agent.SessionContextSourceResult, error) { ... })`
- 需要完全控制最终消息序列：
...
4. Session history（会话的 canonical transcript；`WithSessionContextMessages` 或 `WithSessionContextSource` 返回的消息会先写入 transcript，并排在当前用户消息之前）
...
- `WithSessionContextMessages` 与 `WithSessionContextSource` 会持久化到 session transcript；后续回合会继续通过正常历史构建看到它们。
- `WithSessionContextSource` 不只服务“状态上下文”；它适用于任何带名字的 durable context producer，例如 rules、workspace policy、persona、channel runtime context。模型看到的仍然只是普通 messages。
- 在 multi-agent 场景中，这些选项属于 `RunOptions`，会随 invocation clone 传播到子调用；如果需要“只对某个 agent 生效”，推荐用回调按 `invocation.AgentName` 过滤实现。

可运行示例：

- `examples/prompt/late_context_messages`：演示非持久化 late context。
- `examples/prompt/append_only_diff`：演示持久化 append-only context diff。
