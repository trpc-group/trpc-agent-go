...
LLM Agent 默认不启用图片 URL 失败后的会话继续能力。需要让同一 session 的后续请求继续推进时，业务可以显式开启。开启后，当模型调用返回图片 URL 拉取、访问或解码类错误时，框架会把匹配到的 URL-backed image input 记录到 session state。后续同一 session 的请求在构造模型请求前，会把这些 image part 替换为文本占位，但不会修改原始持久化的用户事件。失败的本次模型调用不会被重试；这个选项只影响后续轮次。
