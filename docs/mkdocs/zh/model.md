...
逻辑一致。混合或冲突索引会被分配到不同的非负索引，并在整个流中保留服务方
原始索引的映射。因此，即使合法索引晚于负索引出现，省略 ID 的续片也能归属
到正确的工具调用。映射按 choice 隔离；合法索引不与已分配索引冲突时会保留。
缺失或为 null 的索引不会建立服务方索引映射。没有对应映射时，身份兼容的
元数据或参数续片仍可沿用已分配的非负索引，包括延迟到达的名称、ID 和分片
传输的函数名。
对于交错的多个工具调用，服务方仍需提供能明确区分调用的索引或 ID；缺失 ID
且多个调用共用同一索引的分片没有足够信息来还原其所属调用。
