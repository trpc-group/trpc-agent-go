...
本文从使用者的角度介绍 A2A 协议、`trpc-a2a-go` 与 tRPC-Agent-Go 的接入架构，然后给出推荐使用的 A2A v1.0 接入方式、Task 与多节点部署边界、旧版 A2A v0.2.x 接入和迁移指南。
...
- **按需使用完整 A2A Task API：** `A2AAgent` 只封装普通 Agent invocation；需要 Task 查询、列表、取消、重新订阅或 push notification 时，直接使用 `trpc-a2a-go/v2/client` 提供的 `GetTasks`、`ListTasks`、`CancelTasks`、`ResubscribeTask` 等方法。
...
需要在多个服务副本之间共享 A2A Task 状态时，可以为 v1 Server 配置 Redis TaskManager。各副本连接同一个 Redis 后，可以共享 Task 状态、历史、查询和列表；具体配置请参考 `trpc-a2a-go` 的 Redis TaskManager 文档和示例。

A2A Task 与 Runner session 是两套独立状态。Redis TaskManager 只解决 Task 状态共享；如果对话也需要跨节点连续，Runner 仍需使用共享 session service 或配置会话亲和性。
...
调用方 session 中的 user ID 默认通过 `X-User-ID` 发送，服务端将其作为 Runner user ID；两端都可以使用 `WithUserIDHeader` 修改 header 名。缺少 user ID 时，旧版服务端会根据 context ID 生成稳定的 `A2A_USER_<contextID>`。这个 header 只负责身份传递，不等于认证和授权。
...
工具调用、代码执行、reasoning、metadata 和 `state_delta` 的 v0.2.x 编码约定见 [A2A v0.2.x 协议交互规范](a2a-interaction.md)。该文档描述的是旧版 wire model，不应作为 v1.0 Part 和事件结构的参考。
