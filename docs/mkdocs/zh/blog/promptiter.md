# tRPC-Agent-Go PromptIter：AI Agent 提示词自动迭代与工程实践全解析
...
tRPC-Agent-Go 框架在 Evaluation 的评估能力基础上提供 PromptIter，用于把 Agent 提示词优化从人工改写推进为受评估约束的自动化迭代流程。PromptIter 以训练集、验证集和评估指标为核心，将训练集暴露出的失败信号沿执行轨迹做反向传播归因，再通过文本梯度聚合和优化补丁生成形成候选提示词，并由验证集评估与接受策略决定候选提示词是否进入后续轮次。每次运行都会沉淀候选提示词、验证结果、接受判定和停止原因，使不同提示词版本可以在既定评估标准下被比较、审查和追踪，支持同步运行、异步运行和 HTTP 迭代服务。
...
// Handler 返回 HTTP 处理器，供 HTTP 服务挂载。
...
// spromptiter.New 创建只暴露结构查询和同步迭代的 PromptIter 服务。
...
// 启动 HTTP 服务。
if err := http.ListenAndServe("127.0.0.1:8080", serverInstance.Handler()); err != nil {
	return err
}
```
...
// spromptiter.New 创建同时暴露同步迭代接口和异步迭代接口的 PromptIter 服务。
serverInstance, err := spromptiter.New(
	// WithAppName 指定服务处理的应用名。
	spromptiter.WithAppName(appName),
	// WithEngine 提供同步迭代和结构查询能力。
...
HTTP 服务把结构查询、同步迭代和异步迭代暴露为远程接口，适合平台页面、任务系统或其他后端服务调用。HTTP 层接收 `Engine` 和可选 `Manager`，并将请求转为本地 `Describe`、`Run`、`Start`、`Get` 和 `Cancel` 调用。完整代码见 [examples/evaluation/promptiter/server](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/evaluation/promptiter/server)。
...
本文介绍了 tRPC-Agent-Go 的 PromptIter 能力，给出了从同步迭代到异步运行、HTTP 服务与多节点 Agent 的使用路径。PromptIter 以训练集、验证集与评估指标为核心，将失败信号提取、反向传播归因、文本梯度聚合、优化补丁生成与接受判定统一为可重复执行的提示词迭代流程，并沉淀结构化运行结果，记录候选提示词、验证结果、接受判定与停止原因。

框架复用 Evaluation 的评估资产与运行体系，结合结构快照、执行轨迹、迭代目标约束、接受策略与停止策略，使候选提示词的生成、验证、接受和拒绝都保留明确依据。将关键场景沉淀为训练集和验证集，并配合可信评估指标、必要的裁判上下文与人工审查，可将提示词优化纳入本地调试、测试环境验证和服务化迭代，持续收敛单点手工改写带来的行为漂移风险。
...
- [PromptIter 使用文档](https://github.com/trpc-group/trpc-agent-go/blob/master/docs/mkdocs/zh/promptiter.md)
- [PromptIter syncrun 示例](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/evaluation/promptiter/syncrun)
- [PromptIter asyncrun 示例](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/evaluation/promptiter/asyncrun)
- [PromptIter server 示例](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/evaluation/promptiter/server)
- [PromptIter multinode 示例](https://github.com/trpc-group/trpc-agent-go/tree/main/examples/evaluation/promptiter/multinode)
