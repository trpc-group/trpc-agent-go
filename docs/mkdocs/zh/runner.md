...
`agent.WithUserMessageRewriter(...)` 用于在当前轮运行开始前改写用户消息。改写结果会直接作为本轮实际生效的输入写入 Session，并在后续轮次继续参与会话构建。该能力适用于整理用户表达、合成 tool transcript，或将一条输入拆成多条更适合模型处理的消息。如果只是想在当前用户消息前持久追加上下文，而不改变用户输入，优先使用 `agent.WithSessionContextMessages(...)`。
...
如果你希望在**当前用户消息之前**追加一段上下文，并且让这段上下文写入 session transcript，可以使用：

- `agent.WithSessionContextMessages(...)`：传入静态消息列表。
- `agent.WithSessionContextMessagesFunc(...)`：在当前 run 开始时动态构造消息列表。

这些消息会在 Runner 持久化当前用户输入前写入 Session。它们不会改变 `invocation.Message`；当前用户输入仍然保持原样。如果同一次 run 同时使用 `WithUserMessageRewriter(...)`，持久化顺序为：
...
eventChan, err := r.Run(
    ...
    agent.WithSessionContextMessages([]model.Message{
        model.NewUserMessage("业务上下文：当前工单优先级=P0，处理策略=先止血再根因分析。"),
    }),
)
```
...
eventChan, err := r.Run(
    ...
    agent.WithSessionContextMessagesFunc(func(
        ...
        return []model.Message{
            model.NewUserMessage("业务上下文：\n" + ctxText),
        }, nil
    }),
)
```

可运行示例：`examples/prompt/session_context_messages`。该示例会打印最终模型请求消息，用于确认上下文是否真实进入模型输入。
