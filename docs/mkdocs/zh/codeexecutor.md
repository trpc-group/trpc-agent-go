...
- **Deny 与内置 deny** 在所有 OS 上都做大小写折叠。`deny: ["curl"]`
  ...
  大写同名二进制的纵深防御。
- **Allow** 在 Windows / macOS 上折叠，在 Linux 上保持精确大小写。
  Linux 文件系统大小写敏感，`./safe` 和 `./SAFE` 是两个文件；如果
  allow 也折叠，`WithWorkspaceExecAllowedCommands("./safe")` 就会
  把工作区里被注入写入的 `./SAFE` 一起放过。Linux 上若确实需要两种
  写法，请同时列出。
