...
**所有 OS 上匹配都做大小写折叠**。`deny: ["curl"]` 同时拦下 `curl`、
`Curl`、`CURL`；内置 deny 也能拦下 `SH -c`、`Sh`、`Bash` 等大小写
变体。macOS 默认 APFS 大小写不敏感（`CURL` 解析到 `/usr/bin/curl`），
Windows 也一样；Linux 上折叠主要是对工作区里可能塞进大写同名二进制的
纵深防御。
