...
- 开启时，实际工具调用数量可以比预期更多，评估会按照预期序列的顺序在实际序列中寻找匹配；开启 OrderInsensitive 时，会在排序后的序列上做的子集匹配。

假设 `A`、`B`、`C` 和 `D` 各自是一组工具调用，匹配情况示例如下。

| SubsetMatching | OrderInsensitive | 预期序列 | 实际序列 | 结果 | 说明 |
| --- | --- | --- | --- | --- | --- |
| 关 | 关 | `[A]` | `[A, B]` | 不通过 | 数量不等 |
| 开 | 关 | `[A]` | `[A, B]` | 通过 | 预期是子集 |
| 开 | 关 | `[A, C]` | `[A, B, C]` | 通过 | 预期是子集且顺序可匹配 |
| 开 | 关 | `[C, A]` | `[A, B, C]` | 不通过 | 顺序不匹配 |
| 开 | 开 | `[C, A]` | `[A, B, C]` | 通过 | 排序后匹配 |
| 开 | 开 | `[C, D]` | `[A, B, C]` | 不通过 | 缺少 D |
