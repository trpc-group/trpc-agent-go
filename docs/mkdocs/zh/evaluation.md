# tRPC-Agent-Go Evaluation 评估能力


Agent 行为存在随机性。同一输入在不同温度、不同上下文下可能得到不同结果。提示词、工具参数、检索策略频繁迭代时，仅靠人工验收很难形成稳定的回归信号。

对工具型 Agent 来说，回归更常发生在过程而非最终回答。工具选择错误、参数结构变化、结果字段变动、调用顺序漂移，都需要用结构化轨迹来对照与定位。

Evaluation 通过 EvalSet 与 Metric 把期望行为与判定规则配置化，每次运行后输出结果文件，便于在 CI 中留存与对比。

## 快速开始

本节给出一个最小使用示例，帮助你快速感受 tRPC-Agent-Go 评估的使用方法。

完整代码见 [examples/evaluation/local](https://git.woa.com/trpc-go/trpc-agent-go/tree/master/examples/evaluation/local)。
...
评估集文件路径为 `data/math-eval-app/math-basic.evalset.json`。

（需要文字解释评估集文件的作用）

（需要文字解释这个评估集文件内容的意义，简单讲下字段的意义，让用户快速了解）
...
评估指标文件路径为 `data/math-eval-app/math-basic.metrics.json`。
...
结果文件会同时保留实际轨迹与预期轨迹，只要工具轨迹满足指标要求，评估结果仍判定为通过。
...
![](../docs/mkdocs/assets/img/evaluation/evaluation.png)

Evaluation 通过统一的评估流程将 Agent 运行过程规范化。评估输入由评估集 EvalSet 与评估指标 Metric 组成，评估输出为评估结果 EvalResult。

- 评估集 **EvalSet** 用于描述评估覆盖的场景，每个用例按轮组织 Invocation，包含用户输入，以及作为预期的 `tools` 轨迹或 `finalResponse`。
- 评估指标 **Metric**。用于定义评估指标配置，包含 `metricName`、`criterion`、`threshold`。`metricName` 用来选择评估器实现，`criterion` 用来描述评估准则，`threshold` 用来定义阈值。
- 评估器 **Evaluator**。它读取实际轨迹与预期轨迹，按 `criterion` 计算 `score`，再与 `threshold` 对比得到通过或失败。
- 评估器注册中心**Registry**。它维护 `metricName` 与 Evaluator 的映射关系，内置评估器和自定义评估器都通过它接入。
- 评估服务 **Service**。它负责执行用例、采集轨迹、调用评估器打分，并将结果交由 EvalResultManager 保存。
- **AgentEvaluator**。`evaluation.New` 创建它并注入 Runner、Managers、Registry 等依赖，对上层只暴露 `Evaluate`。
...
4. Service 汇总分数与状态，生成 EvalResult，结构体名称为 `EvalSetResult`
5. EvalResultManager 保存结果，local 模式写入目录，inmemory 模式驻留内存

## 使用方法

### 评估集 EvalSet

EvalSet 用于描述评估覆盖的场景集合。每个场景对应一个 EvalCase，EvalCase 再按轮组织 Invocation。评估运行时，Service 会按 EvalCase 的 `conversation` 驱动 Runner 推理，并将推理得到的实际轨迹与 EvalSet 中的预期轨迹交给 Evaluator 对比打分。
...
如果你希望从数据库、对象存储或配置平台读取 EvalSet，可以实现该接口并在创建 AgentEvaluator 时注入。
...
当你希望复用既有目录结构时，可以自定义 `Locator` 并在创建 EvalSetManager 时注入。
...
`metricName` 用于从 Registry 选择评估器实现，默认内置 `tool_trajectory_avg_score`、`final_response_avg_score`、`llm_final_response`、`llm_rubric_response`、`llm_rubric_knowledge_recall`。
...
Criterion 用于描述评估准则，不同评估器只会读取自己关心的子准则。
...
`matchStrategy` 支持 `exact`、`contains`、`regex` 三种策略，默认值为 `exact`。对比时 `source` 是实际字符串，`target` 是预期字符串。`exact` 要求 `source` 与 `target` 完全一致，`contains` 要求 `source` 包含 `target`，`regex` 会将 `target` 视为正则表达式并匹配 `source`。
...
type ToolTrajectoryStrategy struct {
	Name      *text.TextCriterion // Name 用于对比工具名
	Arguments *json.JSONCriterion // Arguments 用于对比工具参数
	Result    *json.JSONCriterion // Result 用于对比工具结果
}
```

工具轨迹对比默认只关注工具名、参数与结果，不会对比工具 `id`。

`orderSensitive` 默认为 false，此时会做无序匹配。实现上会将预期工具调用视为左节点，实际工具调用视为右节点。只要某个预期工具与某个实际工具满足匹配策略，就在两者之间建立一条连边，再用 Kuhn 算法求解二分图最大匹配，得到一组一对一配对。若所有预期工具都能找到且仅能找到一个不冲突的匹配，则认为通过，否则会返回无法匹配的预期工具。
...
type FinalResponseCriterion struct {
	Text    *text.TextCriterion                                      // Text 用于对比最终响应文本
	JSON    *json.JSONCriterion                                      // JSON 用于对比最终响应 JSON
	Compare func(actual, expected *evalset.Invocation) (bool, error) // Compare 自定义比较逻辑
}
```
...
`judgeModel` 支持在 `providerName`、`modelName`、`baseURL`、`apiKey` 中引用环境变量，运行时会自动展开。`numSamples` 用于控制每轮的采样次数，采样次数越大越能抵御裁判波动，但开销也会相应增加。`providerName` 表示裁判模型的供应商，对应框架的 Model Provider。框架会按 `providerName` 与 `modelName` 创建裁判模型实例，常见取值有 `openai`、`anthropic`。Provider 的概念与接入方式可参考 [Provider](../docs/mkdocs/zh/model.md#provider)。
...
如果你希望从数据库、对象存储或配置平台读取 Metric，可以实现该接口并在创建 AgentEvaluator 时注入。
...
内置最终响应评估器名称为 `final_response_avg_score`，相应评估准则为 `criterion.finalResponse`，并在每一轮对比 `finalResponse`。

该评估器采用二值打分，并按逐轮平均值聚合整体分数。若你希望对比最终回答的结论或关键字段，优先通过 `FinalResponseCriterion` 的 `text` 与 `json` 配置调整匹配策略，再考虑使用 `Compare` 扩展点覆盖对比逻辑。
...
type MessagesConstructor interface {
	...
	ConstructMessages(ctx context.Context, actuals, expecteds []*evalset.Invocation, evalMetric *metric.EvalMetric) ([]model.Message, error)
}
```
...
type ResponseScorer interface {
	...
	ScoreBasedOnResponse(ctx context.Context, resp *model.Response, evalMetric *metric.EvalMetric) (*evaluator.ScoreResult, error)
}
```
...
type SamplesAggregator interface {
	...
	AggregateSamples(ctx context.Context, samples []*evaluator.PerInvocationResult, evalMetric *metric.EvalMetric) (*evaluator.PerInvocationResult, error)
}
```
...
type InvocationsAggregator interface {
	...
	AggregateInvocations(ctx context.Context, results []*evaluator.PerInvocationResult, evalMetric *metric.EvalMetric) (*evaluator.EvaluateResult, error)
}
```
...
内置 LLM Judge 类评估器包括 `llm_final_response`、`llm_rubric_response`、`llm_rubric_knowledge_recall`。
...
Registry 用于管理评估器注册关系，评估执行会用 `metricName` 从 Registry 获取对应 Evaluator。框架默认 Registry 已注册全部内置评估器。

可以注册自定义评估器并在创建 AgentEvaluator 时注入自定义 Registry。

```go
reg := registry.New()
reg.Register("my_metric", myevaluator.New())
...
如果你希望将结果写入对象存储或数据库，可以实现该接口并在创建 AgentEvaluator 时注入。
...
evalResultManager := evalresultlocal.New(
	...
	func(o *evalresult.Options) {
		o.Locator = &customResultLocator{}
	},
)
```
...
type Service interface {
	...
	Evaluate(ctx context.Context, request *EvaluateRequest) (*evalresult.EvalSetResult, error) // Evaluate 执行评估阶段
...
框架提供了 Service 的本地实现，依赖 Runner 执行推理，EvalSetManager 读取 EvalSet，Registry 定位评估器实现，EvalResultManager 保存评估结果。
...
评估完成后会生成 `EvalSetResult` 并交给 EvalResultManager 保存。

### AgentEvaluator

AgentEvaluator 是面向使用方的评估入口。它负责按 `evalSetID` 组织一次评估运行，读取评估集与评估指标，驱动评估服务完成推理与打分，并对多次运行的结果做聚合。
...
默认情况下，`evaluation.New` 会创建 AgentEvaluator 并使用 InMemory 的 EvalSetManager、MetricManager、EvalResultManager 与默认 Registry，同时创建本地 Service。若你希望从本地文件读取 EvalSet 与指标配置，并将结果写入文件，需要显式注入 Local Manager。
...
`contextMessages` 用于为某个 EvalCase 配置一组额外上下文消息。推理阶段会在每轮调用 Runner 时注入这些消息，用于稳定提示词、补充背景或设置系统指令。

在 EvalSet 文件中为用例设置 `contextMessages`。
...
`contextMessages` 会对该用例的每一轮推理生效，并随每次评估运行重复注入。它不替代 `sessionInput.state`，`state` 用于注入会话初始化状态，`contextMessages` 用于注入模型输入上下文，两者可以同时使用。

如果 `contextMessages` 中存在空元素，推理阶段会直接失败并写入错误信息，建议将其作为评估资产的一部分纳入版本管理。
