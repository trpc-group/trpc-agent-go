...
除“消息快照续传”一节所述的初始尽力刷新外，实时对话的后续 SSE 输出不会等待历史事件写入完成，前端仍会即时收到模型输出。`/history` 读取的是已经成功写入会话存储的内容，因此在对话仍在运行时，请求消息快照可能只能看到上一次刷新成功时的状态。开启消息快照续传后，服务端会持续追踪历史事件，后续刷新成功的事件会继续推送给客户端。
