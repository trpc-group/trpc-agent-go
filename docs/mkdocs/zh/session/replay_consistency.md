...
每个 memory query 都声明 `ExpectedContents`。每条结果的 `AppName` 和 `UserID` 必须与查询 user key 完全一致；归属不匹配属于 runner error，不能通过 `allowed_diff` 放行。通过归属校验后，查询结果再按照无序的精确内容多重集合比较，因此忽略后端特有的 ID、score 和排序，同时仍能发现缺失、无关、额外和重复结果。
...
包含非空 `Case.InitialState`，或直接 `Case.AppState`、`Case.UserState`、`Case.SessionState` 更新的 case，都会把这些 API 已明确区分的作用域作为后端契约直接校验。作用域由 case 字段和对应的 service 方法决定，不根据裸 key 猜测。`InitialState` 只属于主 session，绝不并入 app/user expected state。`AppState` / `UserState` 接受无前缀或一层匹配前缀；已知跨作用域前缀仍由 harness 统一拒绝，未知前缀按普通 key 保留。原始 accepted map 会传给后端以覆盖真实 API 输入行为，prepared expected map 只剥离一个对应 App/User 前缀；`List*States` 返回值按已存储 key 形式复制，不会再次剥离。`flag` 与 `app:flag`、`locale` 与 `user:locale` 等 canonical collision 会在任何后端调用前报告，并包含 canonical key 和排序后的原始 key。`SessionState` 继续只拒绝 `app:` 与 `user:`，`temp:` 保留为 session-local key。只有 `InitialState`、`AppState`、`UserState` 和 `SessionState` 全部为空时，才跳过 ListApp/ListUser 与 peer 作用域校验；仅有 `Event.StateDelta` 不会触发该校验。非法直接 state fixture 会在任何后端调用前返回；`InitialState` 与 `Event.StateDelta` 保持既有 key 语义。Runner 使用 `ListAppStates` / `ListUserStates` 比较直接 app/user 更新，随后在同一 app/user 下创建临时 peer session，并要求它只继承这些 app/user 值，绝不能继承主 session 的 InitialState。expected 与 observed app/user/peer value 使用同一套 StateMap 字节精确比较。每次 peer 创建尝试后都会使用脱离调用方取消信号的限时 context 尝试删除，包括已经落盘但返回错误的 ambiguous fail-after-write。字节漂移、app/user 传播缺失、session/temp/initial state 泄漏和 peer 清理失败都是 runner error，不属于 snapshot diff，也不能通过 `allowed_diff` 放行。
