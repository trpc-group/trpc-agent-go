...
当上游业务已经维护完整对话历史，或者每次请求都应相互独立时，可以使用 `session/noop`。Noop 不会跨请求保存 Session、事件或状态，也不会随着 Session 数量增加而持续占用存储空间。
...
- 上游业务服务已经负责保存完整对话历史
- 无状态 API，每次请求都显式携带完整上下文
- 不需要跨请求 Session 状态、摘要、历史快照或恢复能力
- 希望保留 Graph、Chain、Tool 在单次运行内对临时 Session 的依赖

如果需要 Runner 自动恢复上一轮历史，或者需要跨请求状态与摘要，请使用[内存存储](inmemory.md)或其他持久化后端。
