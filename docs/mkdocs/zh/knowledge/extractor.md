...
- **OCR**：从图片或扫描页中识别文字；旧接口曾作为 PDF Reader 的内部增强能力，现在推荐放在 Extractor 路径中完成
...
对于扫描 PDF、图片型 PDF、图片、结构复杂的 HTML / Office 文档，**推荐优先使用 Docling 作为主处理路径**。

`Docling` 本身已经内置 OCR 能力，因此在这类链路里通常**不需要再额外配置独立的 OCR 能力**。`docling.New()` 默认开启 OCR，需要关闭时可以显式传 `docling.WithOCR(false)`。
...
旧的独立 `OCR` 接口已经标记为 Deprecated。新代码需要 OCR 时，优先选择带 OCR 能力的 `Extractor`。
