...
Knowledge 提供两种加载模式：**默认模式（不启用同步）** 和 **同步模式（启用 enableSourceSync）**。

### 默认模式（不启用同步）

默认情况下，`enableSourceSync` 为 `false`，Knowledge 采用**追加式加载**：
...
- **适用场景**：一次性导入、数据只增不减的场景

### 同步模式（启用 enableSourceSync）

启用 `enableSourceSync` 后，Knowledge 会**保持向量存储与配置的 Source 完全一致**：
