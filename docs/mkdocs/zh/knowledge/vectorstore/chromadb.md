...
关键词检索以及混合检索中的稀疏分支依赖 [Chroma Cloud](https://docs.trychroma.com/cloud/getting-started) 的 `/search` 接口；自托管服务支持向量与过滤检索，未配置稀疏检索时混合检索会退化为纯稠密向量检索。
...
`WithAPIKey` 发送 `X-Chroma-Token`。Cloud API key 可以绑定到某个 database，这种情况下可以省略 `WithTenant` 和 `WithDatabase`，两者都会从 Cloud identity 推断。自动推断要求 identity 返回唯一的 database；如果该 key 没有 database 或返回多个 database，则必须显式设置这两个选项。
...
| `WithChromaInstance(name)` | 使用 `storage.RegisterChromaInstance` 注册的客户端 | - |
| `WithTenant(tenant)` / `WithDatabase(database)` | 租户和数据库。空值使用服务端默认值；配置 `WithAPIKey` 时，绑定 database 的 key 会从 Cloud identity 自动推断两者。如果 key 没有 database 或返回多个 database，则必须显式设置。 | 服务端默认值 |
...
| `WithExtraOptions(opts...)` | 注入自定义 ClientBuilder 配置，默认无需关心 | - |
...
| `WithMaxUpdateRecords(n)` | 单次 `UpdateByFilter` 最多匹配的记录数 | `100000` |
...
| `WithHybridSearchWeights(dense, sparse)` | 稠密/稀疏 RRF 权重，内部归一化为总和 1 | `0.5, 0.5` |
...
- 显式构造 `NewCloudSpladeEmbedder` 时，可用 `WithSpladeBaseURL` 和 `WithSpladeModel` 自定义嵌入服务地址与模型；可用 `WithSpladeTransport` 自定义嵌入请求使用的传输层（代理、TLS、超时、链路追踪），重定向策略始终由包内控制。
...
| Keyword | ⚠️ | 通过 Cloud `/search` 做稀疏 KNN。未配置 `WithSparseSearch` 时返回错误。 |
...
简易 metadata map 过滤（`SearchFilter.Metadata`、`WithDeleteFilter`、`WithCountFilter`、`WithGetMetadataFilter`）只接受用户 metadata 键；传入上面的保留键会直接报错。要过滤文档的 `name`、`created_at`、`updated_at`，请使用 `FilterCondition`。
