...
- **注意**：`pattern` 内容同样受逗号分隔限制；如果正则本身需要包含逗号，建议使用 `function.WithInputSchema(customInputSchema)` 自定义 schema。
- **运行时兼容性**：工具调用清理阶段使用 Go `regexp` 校验 `pattern`，不是严格的 JSON Schema ECMA-262 正则语法；无法被 Go `regexp` 编译的 pattern 在运行时会按“不强制约束”处理。
...
- `HistoryScopeIsolated`（默认）：子 Agent 使用独立的 `FilterKey`，例如
  `math-specialist-<uuid>`（每次调用都会生成新的子 key）。在正常 Runner 生成的事件下，
  子 Agent 只会看到本次工具调用参数，不会继承父 Agent 的历史。子 Agent 的事件仍会写入
  同一个 session，但位于独立视图下。
  - 如果你希望“同一个 AgentTool 多次调用时，子 Agent 能看到自己上一次的执行历史”，可以
    在 `HistoryScopeIsolated` 下开启 `WithPersistentHistory*`（见下文选项说明）。此时子 key
    会改为稳定值（例如 `agenttool:math-specialist:default`），从而让子 Agent 的上下文连续。
...
- WithPersistentHistory() / WithPersistentHistoryKey(string) / WithPersistentHistoryKeyFunc(...)：
  ...
  - 作用：在 `HistoryScopeIsolated` 下使用**稳定的子 `FilterKey`**，让子 Agent 能在同一个 session 内跨多次
    AgentTool 调用读取自己的历史（而不是每次都从“全新子 key”开始）。
  - 默认 key：`agenttool:<toolName>:default`（刻意不含 `/`，避免和父 key 形成前缀关系导致父侧误入上下文）。
  - 高级用法：用 `WithPersistentHistoryKey(...)` 或 `WithPersistentHistoryKeyFunc(...)` 把不同任务分到不同 key，
    防止多个并发/多任务把历史混在一起。
  - 注意：
    - 这不是权限边界；如果父 Agent 侧配置了 `BranchFilterModeAll` / 空 key，或你把子 key 设计成 `parent/...` 这类
      前缀关系，父 Agent 仍可能在后续请求里看到子事件。
    - 子 Agent 是否真的能看到“更早历史”，还取决于子 Agent 自身的 `MessageFilterMode` / timeline 过滤（默认会包含历史；
...
任何工具（无论延迟还是 preset）拒绝与其他调用同轮执行，`call_tool` 就会以代为
拒绝的形式被声明（参见“拒绝与其他调用同轮执行”）；没有任何工具拒绝时，它不作
任何声明——与一个两个并发接口都未实现的工具完全一样：照常放行，也不会替被隐藏
的目标工具作出任何可重入承诺。所以除非注册了拒绝并发的工具，dispatch 模式的一轮
仍保持并行。
...
返回 `true` 是一项承诺：该工具可以与本轮中的任何其他调用同时执行，包括对它
自身的其他调用，因此 `tool.MetadataOf` 会把它读作 `ConcurrencySafe`。干脆不实现
该接口则表示不提出拒绝、也不作任何承诺：这是默认行为，`tool.MetadataOf` 对此
...
启用并行工具且存在拒绝并发的可用工具时，**LLMAgent** 还会把这一点告诉模型：在
system prompt 末尾追加一段简短说明，列出这些工具，并指出包含它们的一轮会逐个
执行其中的每个调用。否则模型把一个独占工具和三个廉价读取放进同一轮时，四个调用
都会被静默串行，而模型无从得知原因。

这段说明只由 LLMAgent 添加。Graph Tools 节点同样应用执行门控——只要有调用提出
拒绝，该批次一律串行——但它不会去改写产生这些调用的模型请求，因此单独搭建模型
步骤的 graph 只会得到这一行为，而看不到对应的说明。如果需要让模型知晓，请在该
步骤自己的 system prompt 中补充同样的指引。
