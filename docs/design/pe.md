...
`GET /trpc-agent/v1/apps/{appName}/structure` 返回 agent 静态结构图。`structure` 复用框架已有 `agent/structure.Snapshot`，用于平台识别可优化 surface，并构造执行期自包含的 initial profile。
...
// 读取或加载平台保存的用户服务 structure snapshot，并在本次迭代中固定使用它。
...
engineInstance, err := promptiterengine.New(
	ctx,
	// 注入本次迭代固定使用的 structure snapshot。
...
runRequest := &promptiterengine.RunRequest{
	...
	MaxRounds:        maxRounds,
	// 必填：限定本次允许优化的 surface，平台可以用它控制优化范围。
...
PR 按 `core -> server -> runner -> examples/docs` 的顺序提交。前三个 PR 是框架能力，第四个 PR 只放示例、跨框架 adapter 和文档验证材料，避免把用户可见协议、平台 runner 和示例跑分混在一起审。

### PR 1：PromptIter Core / Profile Compiler

Branch：`evaluation/promptiter-compile-structured-profiles`

PR title：`evaluation/promptiter: compile structured profiles`

文件范围：

- `internal/profilecompiler/**`
- `evaluation/workflow/promptiter/**`
- `server/promptiter/**`
- 现有 PromptIter 示例的必要适配：`examples/evaluation/promptiter/{asyncrun,syncrun,server,tooldesc,multinode}/**`

该 PR 负责把 profile 校验、structure surface 索引和 `profile -> runtime patch` 编译逻辑收敛到 `internal/profilecompiler`，并让 PromptIter engine、manager、store 和 `server/promptiter` 复用同一套结构化 profile 语义。现有本地 PromptIter 示例只做必要适配，确保 core 行为变化可以被单独 review。

该 PR 需要重点说明兼容性语义：`TargetSurfaceIDs` 是否允许为空、`RunResult` 是否继续暴露 structure、异步 manager 是否承诺 clone 安全，以及 `server/promptiter` 对 unknown surface 的错误返回时机。上述问题属于 PromptIter core 行为，不应藏在 remote example PR 中。

### PR 2：用户侧 tRPC-Agent API Server
...
服务端接收结构化 profile 后复用 PR 1 的 profile compiler，把 profile 应用到真实业务 runner。本 PR 只审用户侧 HTTP 服务面和协议响应形态，不混入 PromptIter engine 行为调整。
...
新增平台侧 `runner/trpcagent`，实现远程 structure describe 和 remote run。它遵循 PR 2 中已经落地的 tRPC-Agent API wire 形态，把 `runner.Run` 转成 tRPC-Agent API 请求，并把响应恢复为 evaluation 可消费的 runner events。
...
### PR 4：Remote Examples and Docs

Branch：`examples/promptiter-add-remote-trpcagent`

PR title：`examples/promptiter: add remote tRPC-Agent flow`

文件范围：

- `examples/evaluation/promptiter/remote/**`
- `examples/evaluation/promptiter/README.md`
- `docs/design/pe.md`

补齐 PromptIter remote driver、Go reference candidate server、sports recap data、ADK adapter、LangGraph adapter、运行说明和实际迭代结果。协议契约测试应在 PR 2 和 PR 3 内完成，PR 4 只负责端到端示例、跨框架接入方式和文档验证材料。

该 PR 中 README 可以记录 Go reference、ADK 和 LangGraph 的运行结果，但不提交临时 `*.log` 和 evaluation output 目录。由于 ADK 和 LangGraph 依赖外部 Python 包和 OpenAI-compatible endpoint，示例 README 需要明确模型、base URL、API key 和默认迭代参数的对齐关系。

不应混审的边界：`server/promptiter/**` 只属于 PR 1，`server/trpcagent/**` 只属于 PR 2，`runner/trpcagent/**` 只属于 PR 3，`examples/evaluation/promptiter/remote/servers/adk-python/**` 和 `examples/evaluation/promptiter/remote/servers/langgraph/**` 只属于 PR 4。

PR 依赖顺序为 PR 1 -> PR 2 -> PR 3 -> PR 4。PR 1 先稳定 PromptIter core 和 profile compiler；PR 2 落地用户侧服务；PR 3 接入平台侧 runner；PR 4 最后补端到端示例、跨框架 adapter 和文档。
