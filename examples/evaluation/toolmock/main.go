...
// Package main demonstrates evaluation with invocation-level tool mocks.
package main
...
var (
	...
	evalSetID = flag.String("eval-set", "toolmock-weather", "Evaluation set identifier to execute")
	numRuns   = flag.Int("runs", 1, "Number of times to repeat the evaluation loop per case")
)

const appName = "toolmock-app"

func main() {
	flag.Parse()
	ctx := context.Background()
	r := runner.NewRunner(appName, newWeatherAgent(*modelName, *streaming))
	defer r.Close()
...
func printSummary(result *evaluation.EvaluationResult, outDir string) {
	fmt.Println("✅ Evaluation completed with tool mock example")
	fmt.Printf("App: %s\n", result.AppName)
