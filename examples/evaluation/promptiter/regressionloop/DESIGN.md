...
other。每个失败 case 都携带原始 `reason` 作为可解释依据，并按 P0–P3 汇总训练集终端
损失的严重度分布，供人工审阅优先级排序。
...
接受候选的 surface 投影、每轮 validation 分与接受/拒绝理由，以及成本：耗时、按角色模型
调用数、evaluated case 数）与 `optimization_report.md`。确定性配置（`deterministic`、
`randomSeed`、fake 模型摘要）随报告落盘；evaluated case 数与模型调用数分列、互不混淆。
