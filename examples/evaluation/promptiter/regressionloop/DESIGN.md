...
（机器真相源：baseline / candidate 分数、逐 case delta、gate 决策与理由、失败归因统计、
每轮候选与接受/拒绝理由、成本估算）与 `optimization_report.md`（人读结论）。每轮产物、
随机无关的确定性配置、成本口径均可回溯；成本因引擎不带 token 计费而标注为估算。
