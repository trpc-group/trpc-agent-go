...
`optimization.maxRoundsWithoutRelease` limits consecutive candidates rejected by the release Gate; it does not count PromptIter search rejection. `evaluation.traceMode` must be `true` because trace evidence is part of this example's auditable failure attribution contract.
...
Artifacts are atomically replaced through a filesystem root handle that prevents traversal and symbolic-link escapes. Reports retain per-case deltas, attribution summaries, trace surfaces, actual/expected tool trajectories, Gate checks, model configuration, seed, latency, usage, estimated cost, and references to each persisted snapshot. When round artifacts are disabled, an accepted profile is still persisted separately so the write-back reference remains valid. Round 1 generalizes and is released; Round 2 improves training only and is rejected; Round 3 improves training while regressing validation and the P0 tool case, so the workflow retains Round 1. Public examples and boundary tests exercise these behaviors; hidden-set acceptance and attribution accuracy remain external acceptance measurements and are not claimed by this example.
