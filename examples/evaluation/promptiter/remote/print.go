...
func printSummary(
	...
	initialInstruction = initialInstructionText(result, initialInstruction, targetSurfaceID)
	...
	fmt.Println("PromptIter remote sports commentary example completed")
	...
	for _, round := range result.Rounds {
		...
		fmt.Printf("Round %d -> train %.2f, validation %.2f, accepted %t, delta %.2f, stop=%t (%s)\n", round.Round, trainScore, validationScore, round.Acceptance.Accepted, round.Acceptance.ScoreDelta, round.Stop.ShouldStop, round.Stop.Reason)
...
func initialInstructionText(result *engine.RunResult, fallback string, targetSurfaceID string) string {
	for _, surface := range result.Structure.Surfaces {
		if surface.SurfaceID != targetSurfaceID || surface.Value.Text == nil {
			continue
		}
		return *surface.Value.Text
	}
	return fallback
}

func acceptedInstructionText(result *engine.RunResult, initialInstruction string, targetSurfaceID string) string {
