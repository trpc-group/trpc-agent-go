...
var (
	mode                       = flag.String("mode", "run", "Execution mode: run or serve-go-candidate")
	addr                       = flag.String("addr", ":8081", "Listen address for serve-go-candidate mode")
	serverBasePath             = flag.String("server-base-path", "/trpc-agent/v1/apps", "Base path exposed by serve-go-candidate mode")
	...
	dataDir                    = flag.String("data-dir", "../syncrun/data", "Directory containing evaluation set and metric files")
	outputDir                  = flag.String("output-dir", "./output", "Directory where evaluation results will be stored")
	modelName                  = flag.String("model", "deepseek-v3.2", "Model identifier used by the Go candidate server")
...
func main() {
	...
	switch *mode {
	case "serve-go-candidate":
		if err := runGoCandidateServer(ctx, candidateServerConfig{
			Addr:                 *addr,
