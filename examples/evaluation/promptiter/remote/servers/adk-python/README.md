# ADK Python tRPC-Agent Service

This example exposes a Google ADK sports recap workflow through the `server/trpcagent` HTTP protocol so PromptIter can optimize the same stage instruction surfaces as the Go reference server.
...
```bash
cd examples/evaluation/promptiter/remote/servers/adk-python
python3 -m venv .venv
. .venv/bin/activate
pip install -r requirements.txt
export OPENAI_API_KEY="your-openai-compatible-key"
export OPENAI_BASE_URL="https://your-openai-compatible-endpoint/v1"
export ADK_MODEL="deepseek-v3.2"
python server.py
```
