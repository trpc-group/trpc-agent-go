...
- 运行 ID：`run-20260729-231250.471085000`（mode=fake，seed=20260705，耗时 21ms）
...
baseline 失败 1 例，候选失败 1 例。
train baseline 失败 3 例，是优化的输入，候选侧无对应归因，
单列在下表末列，不参与对比。

| 类别 | validation baseline | validation 候选 | train baseline |
|---|---|---|---|
| final_response_mismatch | 0 | 1 | 1 |
| tool_call_error | 1 | 0 | 1 |
| tool_argument_error | 0 | 0 | 1 |

因果链明细：

- [baseline/validation] validation/val_01_generalize_tool_and_format：root: tool_call_error, cascaded to final_response_mismatch
...
- [候选/validation] validation/val_02_protected_format：root: final_response_mismatch
...
- [baseline/train] train/train_01_response_completeness：root: final_response_mismatch
...
- [baseline/train] train/train_02_wrong_tool_choice：root: tool_call_error, cascaded to final_response_mismatch
...
- [baseline/train] train/train_04_wrong_tool_argument：root: tool_argument_error, cascaded to final_response_mismatch
...
| max_wall_clock | 21ms | \<= 3m0s | 通过 | 整个 pipeline 运行的墙钟预算 |

## 轮次时间线

| 轮次 | train | validation | 引擎内层判定 | 模型调用 | 耗时 |
|---|---|---|---|---|---|
| 1 | 0.3750 | 0.8333 | 接受 | 11 | 4ms |
| 2 | 0.5000 | 0.8333 | 拒绝（停止：max rounds reached） | 11 | 8ms |
...
分阶段耗时：s1_baseline_train=4ms s2_attribution=0s s3_optimization=14ms s4_delta=0s s5_gate=0s 
