...
func reportToMarkdown(r *RegressionReport) string {
	...
	if r.Attribution == nil || len(r.Attribution.Failures) == 0 {
		...
		fmt.Fprintf(&b, "- 总 case 数: %d，失败 case 数: %d\n", r.Attribution.TotalCases, r.Attribution.FailedCases)
		b.WriteString("- 按类别分布：")
	...
	b.WriteString("\n## 逐 case 分数变化\n\n")
	b.WriteString("| 评测集 | Case | 基线 | 候选 | 变化 | 趋势 |\n")
	b.WriteString("| --- | --- | --- | --- | --- | --- |\n")
	for _, d := range r.PerCaseDelta {
		trend := map[string]string{"up": "上升", "down": "下降", "flat": "持平"}[d.Trend]
		if trend == "" {
			trend = d.Trend
		}
		fmt.Fprintf(&b, "| %s | %s | %.4f | %.4f | %+.4f | %s |\n",
			d.EvalSetID, d.EvalCaseID, d.BaselineScore, d.CandidateScore, d.Delta, trend)
	}
	b.WriteString("\n## 成本\n\n")
	if r.Cost == nil {
		b.WriteString("未记录成本。\n")
