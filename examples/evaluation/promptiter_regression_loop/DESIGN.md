...
The JSON and Markdown reports are audit artifacts. They record deterministic seed `0`, prompt and configuration sources and hashes, resolved PromptIter policies, fake model generation settings, target surfaces, round patches, scores, deltas, gate reasons, attribution evidence, latency, model calls, and deterministic zero cost. Normal reports retain measured wall-clock latency, while explicit sample-report generation normalizes latency to `0ms` before building either the report or final gate so checked-in JSON and Markdown remain byte-stable. An omitted critical-case list retains the example default, while an explicit empty list disables that check without changing other gate rules. This makes the example reproducible without an API key while leaving a clear path for future real-model extensions.
