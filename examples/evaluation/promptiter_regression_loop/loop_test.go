...
func TestPerCaseDeltas(t *testing.T) {
	baseline := newResult(0.5, []promptiterengine.CaseResult{
		{EvalSetID: validationEvalSetID, EvalCaseID: "a", Metrics: []promptiterengine.MetricResult{{MetricName: "m", Score: 0.5}}},
	})
	candidate := newResult(0.8, []promptiterengine.CaseResult{
		{EvalSetID: validationEvalSetID, EvalCaseID: "a", Metrics: []promptiterengine.MetricResult{{MetricName: "m", Score: 0.8}}},
	})
	deltas := perCaseDeltas(baseline, candidate)
	if len(deltas) != 1 {
		t.Fatalf("expected 1 delta, got %d", len(deltas))
	}
	if deltas[0].Delta < 0.29 || deltas[0].Delta > 0.31 {
		t.Fatalf("expected delta ~0.3, got %v", deltas[0].Delta)
	}
	if deltas[0].Trend != "up" {
		t.Fatalf("expected up trend, got %s", deltas[0].Trend)
	}
}
