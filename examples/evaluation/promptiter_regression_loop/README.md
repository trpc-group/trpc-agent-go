...
- **聚类去重 + 可观测性**：失败 case 按 `(类别, 归一化理由)` 去重为少量 `Clusters`，报告展示
  ...
  调用与失败情况，但不影响接受/拒绝门禁。
- **逐 case delta 分类 + 随机种子审计**：`逐 case 分数变化` 表在分数之外新增"基线通过 /
  候选通过 / 变化类别"三列，明确区分 **新增通过（new_pass）**、**新增失败（new_fail）**、
  **分数提升（score_up）**、**分数下降（score_down）** 与 **持平（flat）** 五类结果，满足
  issue #2003 对候选验证"区分新增通过、新增失败、分数提升、分数下降"的要求；运行配置快照
  (`ConfigSnapshot`) 新增 `seed` 字段并在报告 `## 配置` 中输出随机种子，保证产物可完整复现。
