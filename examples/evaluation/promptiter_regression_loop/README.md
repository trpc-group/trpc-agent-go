...
The optimizer has a sub-budget inside the global gate. Its model and base URL inherit the evaluation model by default but can be overridden independently, along with temperature, output-token limit, timeout, and retries. Before the optimizer runs, the pipeline reserves enough global calls and estimated tokens and cost for the remaining candidate training evaluation and repeated validation runs. Optimizer retries consume both budgets.

The sample configuration allows 165 total HTTP attempts, 200,000 total tokens, and 20 CNY. Within that total, the one-round optimizer is limited to three attempts, 16,384 tokens, and 1 CNY. The optimizer token allowance includes the official structured-output schema as well as the baseline and training gradients.
...
PromptIter only receives the training eval set. Failure attribution, backward gradients, and aggregation remain deterministic and auditable in both modes. Fake mode uses a deterministic optimizer; live mode gives the aggregated training gradients and baseline surface to the official LLM-backed PromptIter optimizer. A tracking wrapper captures the returned patch and reason without changing framework APIs. The normal PromptIter engine performs its internal selection using the same training data, alongside training loss extraction, patch application, and its built-in score check. Reports call this value `innerTrainScore`, not validation score.
...
The tests cover attribution precedence, negated-fact and natural-language credential-disclosure counterexamples, paired deltas, Pass^k, bootstrap reproducibility, hard-failure and critical-case vetoes, provider-error rejection, overfitting rejection, strict configuration decoding, configuration inheritance, HTTP retry accounting, optimizer and global budgets, validation-capacity reservation, validation-set leakage, fail-closed live reports, atomic report replacement, and end-to-end deterministic replay.
