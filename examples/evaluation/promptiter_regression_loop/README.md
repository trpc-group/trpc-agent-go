...
It uses the repository's canonical `evalset.EvalSet` / `evalset.EvalCase` ownership through the explicitly restricted `RegressionEvalSet` adapter, plus the standard tool-trajectory and PromptIter `Profile` / `PatchSet` contracts. The offline `DeterministicPromptIter` and local evaluator make the workflow deterministic without weakening the validation gate. Training failure categories generate concrete remediation rules in each proposal. The optimizer returns only public `PromptProposal` content; the pipeline owns candidate IDs, candidate-and-seed markers, target surfaces, profiles, and patch sets. Every explicit candidate output must carry the exact SHA-256 of the prompt's semantic content, so reusing a candidate ID or seed with different prompt text fails before scoring. A production integration can implement `Optimizer` and `Evaluator` directly: `Evaluator.RuntimeMode` is a compile-time requirement, and adapters never need to reproduce a private candidate protocol.

The config and report schema versions are implementation-owned constants. This version accepts `promptiter.json` schema `1.1` and always emits report schema `1.1`; unsupported input versions fail during loading instead of relabeling the current report layout.
