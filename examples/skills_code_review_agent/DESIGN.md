...
| 控制项 | 实现 |
|--------|------|
| 默认生产 runtime | `container`（`docker/Dockerfile`：Go + python3）或 `e2b`；CLI 无 repo 时默认 `local` 便于无 Docker 测试 |
| 超时 | 30s（`RunProgramSpec.Timeout` / context） |
| 输出上限 | 64KB stdout/stderr 截断（Writer 契约：短写仍 ack 全长） |
| 环境变量 | CleanEnv + PATH/LANG 白名单；Go 检查 GOPROXY=off |
| 依赖 | vendor 或可离线 `go list`；否则 skip + `deps_unavailable` |
| 失败处理 | 记录 `sandbox_runs.error_type`，不中断 review task；ready workspace 失败不回退宿主机 |

## Permission / Filter 策略

执行前对每个**管线真实提出的**命令调用 PermissionPolicy（不做合成探针）：
...
高风险探针仅出现在 `checkPermission` 单元测试中。
...
- 去重键：`file:line:category:rule_id`
- 低置信度不进 confirmed findings
- 脱敏在最终写入前执行（含 sandbox stdout/stderr），检出率目标 ≥ 95%
- LLM finding 必须锚定到 diff 新增行，且强制 `source=llm`
