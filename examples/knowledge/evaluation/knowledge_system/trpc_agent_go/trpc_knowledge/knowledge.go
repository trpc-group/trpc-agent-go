...
func NewKnowledgeService(storeType VectorStoreType, modelName string) (*KnowledgeService, error) {
	...
	svc.emb = openai.New(openai.WithModel(embeddingModel))
...
func (s *KnowledgeService) Load(ctx context.Context, filePaths []string) error {
	...
	log.Infof("[Load] Clearing vector store...")
	if err := s.vs.DeleteByFilter(ctx, vectorstore.WithDeleteAll(true)); err != nil {
		log.Warnf("[Load] Failed to clear vector store: %v", err)
	}

	// Verify deletion by checking count
	countResult, err := s.vs.Count(ctx)
	if err != nil {
		log.Warnf("[Load] Failed to verify vector store count: %v", err)
	} else {
		log.Infof("[Load] Vector store count after deletion: %d", countResult)
		if countResult > 0 {
			log.Warnf("[Load] Warning: Vector store still contains %d documents after deletion", countResult)
		}
	}
	...
	if err := s.kb.Load(ctx, knowledge.WithShowProgress(true)); err != nil {
	...
	agent := llmagent.New(
		...
		llmagent.WithInstruction(
			"Answer the following question using the available tools. "+
				"Base your final answer STRICTLY on the information retrieved from the tools. "+
				"Do not include external knowledge you already know.",
		),
