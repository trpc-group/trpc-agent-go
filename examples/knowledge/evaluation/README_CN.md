...
### 4. 垂直评测：Reciprocal Rank Fusion (RRF) 融合模式
...
| **Weighted** (v100_t0, 纯向量) | 1.0000 | 0.8787 | 0.7648 | 0.7493 | 0.7665 | 1.0000 | 0.5500 |
| **Weighted** (v90_t10) | 1.0000 | 0.8544 | 0.6543 | 0.7232 | 0.7257 | 0.9000 | 0.5750 |
| **Weighted** (v50_t50, 对半) | 0.8667 | 0.7296 | 0.5921 | 0.6817 | 0.5795 | 0.8000 | 0.5500 |

**分析：**

1. **RRF 在当前场景下表现不及纯向量加权融合**：RRF 的 Answer Correctness (0.5439) 和 Context Precision (0.5957) 均显著低于纯向量模式 (0.7648 / 0.7665)，Context Recall 也从 1.0 降至 0.8。
2. **RRF 与中等权重加权融合接近**：RRF 的各项指标与 v50_t50 相近，说明在当前数据集下，RRF 的排名融合效果大致等价于向量和文本各占一半权重的加权融合。
3. **原因分析**：在当前的 HuggingFace Markdown 文档 + BGE-M3 高质量 Embedding 场景下，向量检索通道的质量远高于文本稀疏检索通道。RRF 对两个通道的排名给予了相对均等的权重（都是 `1/(k+rank)`），这实际上**拉高了低质量文本通道的影响力**，稀释了高质量向量通道的优势。而加权融合可以通过设置极高的向量权重（如 0.9 或 1.0）来压制文本通道的噪声。

**结论**：RRF 更适合**两个检索通道质量相当**的场景（例如向量检索和高质量 BM25 检索并存时）。当其中一个通道明显优于另一个时，加权融合配合适当的权重调优是更好的选择。
