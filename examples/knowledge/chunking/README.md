...
|--------|-------------------|
| [`sample.md`](./sample.md) | Markdown headings, a table, mixed-language boundaries, a long fenced Go block, emoji, and a long token |
| [`sample-edge.md`](./sample-edge.md) | Sparse heading levels, nested blocks, a wide table, an oversized code block, combining characters, and fallback boundaries |
| [`sample-catalog.md`](./sample-catalog.md) | A realistic reference-document shape with grouped rows, several short tables, and a long table cell |
| [`sample.txt`](./sample.txt) | Plain-text paragraphs, punctuation, whitespace, logs, CJK text, emoji, and an unbroken token |
| [`sample.csv`](./sample.csv) | Multilingual records, empty fields, long values, URLs, and CSVReader normalization |
| [`sample.json`](./sample.json) | Nested objects and arrays, empty values, multilingual strings, and properties that compete for a byte budget |
