...
//   - CHROMA_URL: (Optional) Chroma HTTP address, defaults to
//     https://api.trychroma.com when CHROMA_API_KEY is set, otherwise
//     http://localhost:8000. The value must include a scheme, for example
...
//   - CHROMA_TENANT: (Optional) Chroma tenant, resolved from a database-scoped
//     CHROMA_API_KEY when omitted
//   - CHROMA_DATABASE: (Optional) Chroma database, resolved from a
//     database-scoped CHROMA_API_KEY when omitted. Set it explicitly when the
//     key reports no database or several
//
// Example usage:
//
...
//	go run main.go
//
// For Chroma Cloud, export CHROMA_API_KEY instead of CHROMA_URL. A key scoped
// to one database lets WithAPIKey resolve tenant and database from the Cloud
// identity endpoint; keys that report no database or several databases need
// CHROMA_TENANT and CHROMA_DATABASE set explicitly.
package main
...
var (
	...
	baseURL    = util.GetEnvOrDefault("CHROMA_URL", defaultChromaURL(apiKey))
...
// defaultChromaURL returns the Cloud endpoint when an API key is present, so
// the documented "export CHROMA_API_KEY instead of CHROMA_URL" flow reaches
// Chroma Cloud rather than sending the key to a local server.
func defaultChromaURL(apiKey string) string {
	if apiKey != "" {
		return "https://api.trychroma.com"
	}
...
func main() {
	...
	if apiKey != "" {
		fmt.Printf("📊 Connecting to Chroma Cloud: %s (tenant: %q, database: %q)\n", baseURL, tenant, database)
	} else {
