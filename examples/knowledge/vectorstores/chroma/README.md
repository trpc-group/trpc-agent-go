...
export CHROMA_TENANT=xxxx           # optional; resolved from a database-scoped key
export CHROMA_DATABASE=xxxx         # optional; required only when the key reports no database or several
```

With an API key set and `CHROMA_URL` omitted, the example defaults to
`https://api.trychroma.com`. Without an API key, it defaults to
`http://localhost:8000`. An explicit `CHROMA_URL` takes precedence and must
include a scheme (`http://` or `https://`), for example
`https://api.trychroma.com`. The collection defaults to `trpc_example`; the
`chroma.New` API itself does not default to any base URL.
...
authentication. For Chroma Cloud, a database-scoped `WithAPIKey` resolves tenant
and database from identity when they are omitted, so both options are optional.
Keys that report no database or several databases must pass `chroma.WithTenant`
and `chroma.WithDatabase` explicitly.
...
`WithHybridSearchWeights` to override the field and RRF weights. The server must
