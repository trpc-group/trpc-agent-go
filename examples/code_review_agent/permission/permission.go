...
const (
	// DecisionAllow records that command execution was allowed.
	DecisionAllow = "allow"
	// DecisionDeny records that command execution was denied.
	DecisionDeny = "deny"
	// DecisionNeedsHumanReview records that command execution needs approval.
...
func checkCommand(command string) tool.PermissionDecision {
	...
	if strings.ContainsAny(lower, "|;&><`$\r\n") {
		return tool.DenyPermission("shell operators are not allowed")
	}
	denyNeedles := []string{
	...
	allowedScripts := map[string]bool{
		"bash skills/code-review/scripts/diff_summary.sh -":             true,
		"bash skills/code-review/scripts/secret_scan.sh -":              true,
		"bash skills/code-review/scripts/go_static_checks.sh work/repo": true,
	}
	if allowedScripts[lower] {
		return tool.PermissionDecision{
			Action: tool.PermissionActionAllow,
			Reason: "command is allow-listed for code review checks",
		}
