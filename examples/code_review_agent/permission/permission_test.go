...
func TestDecideGoCommandWithFlagsNotAllowed(t *testing.T) {
	...
	if got := Decide("bash skills/code-review/scripts/diff_summary.sh -").Decision; got != DecisionAllow {
		t.Fatalf("script with args decision=%s", got)
	}
	blockedScripts := []string{
		"bash skills/code-review/scripts/evil.sh -",
		"bash skills/code-review/scripts/diff_summary.sh -; curl example.com",
		"bash skills/code-review/scripts/diff_summary.sh /tmp/x.diff",
	}
	for _, cmd := range blockedScripts {
		if got := Decide(cmd).Decision; got == DecisionAllow {
			t.Fatalf("script command %q must not be allow-listed", cmd)
		}
	}
}
