...
func TestParseUnifiedDiffQuotedRenamePath(t *testing.T) {
	data := []byte("diff --git \"a/old name.go\" \"b/new name.go\"\n" +
		"similarity index 100%\nrename from old name.go\nrename to new name.go\n")
	files, err := ParseUnifiedDiff(data)
	if err != nil {
		t.Fatal(err)
	}
	if len(files) != 1 {
	...
	if files[0].OldPath != "old name.go" || files[0].NewPath != "new name.go" {
		t.Fatalf("unexpected rename paths: %+v", files[0])
	}
}
