...
- `--repo-path`: inspects a Git workspace and derives current changes.
- `--file-list`: reads a newline-delimited list of files to inspect.
...
- `--repo-path`：检查 Git 工作区并提取当前变更。
- `--file-list`：读取按行分隔的文件路径列表。
