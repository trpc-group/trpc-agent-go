...
| --- | --- | --- | --- |
| [SI-001](SI-001-hardcoded-secret.md) | Critical | Security | Detects hardcoded secrets, API keys and tokens in source. |
...
| [RL-001](RL-001-resource-not-closed.md) | High | Reliability | Flags `os.Open`/`os.Create`/`os.OpenFile` handles not closed via `defer`. |
| [EH-001](EH-001-error-unchecked.md) | Medium | Correctness | Detects function calls whose returned `error` is discarded. |
| [TM-001](TM-001-missing-tests.md) | Low | Quality | Flags new non-test `.go` source files added in a diff. |
...
- **Critical**: Confirmed security vulnerability or data-loss risk. Block
  merge immediately.
