...
func publish(report Report, outputDir string) (Report, ReportPaths, error) {
	dir := filepath.Join(outputDir, report.Task.ID)
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return Report{}, ReportPaths{}, err
	}
	jsonPath := filepath.Join(dir, "review_report.json")
	markdownPath := filepath.Join(dir, "review_report.md")
	...
	for attempts := 0; attempts < 8; attempts++ {
		...
		if len(jsonData) > maxArtifactBytes || len(markdown) > maxArtifactBytes {
			return Report{}, ReportPaths{}, fmt.Errorf("report exceeds artifact limit")
		}
		...
		if report.Artifacts[jsonIndex].SizeBytes == int64(len(jsonData)) && report.Artifacts[markdownIndex].SizeBytes == int64(len(markdown)) {
			...
			if err := atomicWrite(markdownPath, markdown); err != nil {
				return Report{}, ReportPaths{}, err
			}
			return report, ReportPaths{JSON: jsonPath, Markdown: markdownPath}, nil
		}
		report.Artifacts[jsonIndex].SizeBytes = int64(len(jsonData))
		report.Artifacts[markdownIndex].SizeBytes = int64(len(markdown))
	}
	return Report{}, ReportPaths{}, errors.New("report artifact sizes did not converge")
}
...
func renderMarkdown(report Report) []byte {
	var b bytes.Buffer
	fmt.Fprintf(&b, "# Code Review Report\n\nTask: `%s`\n\nStatus: `%s`\n\nMode: `%s`\n\n", report.Task.ID, report.Task.Status, report.Mode)
	...
	for _, item := range report.PermissionDecisions {
		fmt.Fprintf(&b, "- `%s`: **%s**", item.Command, item.Action)
		if item.Reason != "" {
	...
	for _, item := range report.FilterDecisions {
		fmt.Fprintf(&b, "- `%s`: **%s** to `%s` — %s\n", item.Fingerprint, item.Action, item.TargetBucket, item.Reason)
	}
	b.WriteString("\n## Sandbox summary\n\n")
	for _, run := range report.SandboxRuns {
		fmt.Fprintf(&b, "- `%s %s`: %s; exit=%d; timeout=%t; duration=%dms; error=%s\n", run.Command, stringsJoin(run.Args), run.Status, run.ExitCode, run.TimedOut, run.DurationMS, run.ErrorType)
	}
	b.WriteString("\n## Monitoring\n\n")
	...
	for _, key := range keys {
		fmt.Fprintf(&b, "- Severity %s: %d\n", key, report.Metrics.SeverityDistribution[key])
	}
	fmt.Fprintf(&b, "\n## Conclusion\n\n%s\n", report.Conclusion)
	return b.Bytes()
}
...
func renderFindingTable(b *bytes.Buffer, title string, values []Finding) {
	...
	for _, item := range values {
		fmt.Fprintf(b, "| %s | %s | `%s:%d` | %s — `%s` | %s |\n", item.Severity, item.Category, item.File, item.Line, item.Title, escapeCell(item.Evidence), item.Recommendation)
	}
	b.WriteString("\n")
}

func escapeCell(value string) string { return redact(bytes.NewBufferString(value).String()) }
