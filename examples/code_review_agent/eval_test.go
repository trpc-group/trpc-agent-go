...
func TestEvalHelpers(t *testing.T) {
	...
	v, ok := measuredRatio(0, 0)
	require.Zero(t, v)
	require.False(t, ok)
	v, ok = measuredRatio(1, 2)
	...
	require.Equal(t, "n/a", formatMeasuredMetric(0, false))
	require.Contains(t, secretLeakID(0, "secret"), "secret-01-")
}

func TestRunEvaluationZeroDenominatorsDoNotPass(t *testing.T) {
	dir := t.TempDir()
	labels := filepath.Join(dir, "labels.json")
	require.NoError(t, os.WriteFile(labels, []byte(`{
  "fixtures": [{"name":"clean","expected_rules":[],"expected_high_rules":[]}],
  "secret_values": []
}`), 0o644))
	report, _, _, err := RunEvaluation(context.Background(), ReviewOptions{
		FixtureDir:     "testdata/fixtures",
	...
	require.NoError(t, err)
	require.False(t, report.HighRiskRecallMeasured)
