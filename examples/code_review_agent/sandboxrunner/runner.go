...
func runLocal(ctx context.Context, repoPath string, command string, timeout time.Duration) review.SandboxRun {
	...
	// #nosec G204 -- command is an exact PermissionPolicy allow-list match.
	cmd := exec.CommandContext(runCtx, parts[0], parts[1:]...)
	cmd.Dir = repoPath
	cmd.Env = allowedLocalEnv()
	var output limitedBuffer
	output.limit = outputExcerptLimit
	cmd.Stdout = &output
	cmd.Stderr = &output
	err := cmd.Run()
	exitCode := -1
	...
	return runFromOutput(command, start, output.Bytes(), exitCode, runCtx.Err(), err)
}
...
func runContainer(ctx context.Context, cfg Config, command string) review.SandboxRun {
	...
	exec, err := containerexec.New(
		containerexec.WithHostConfig(dockercontainer.HostConfig{
			...
			Tmpfs:          map[string]string{"/tmp": "rw,nosuid,nodev,size=536870912"},
			Resources: dockercontainer.Resources{
				Memory: 512 << 20, NanoCPUs: 1_000_000_000, PidsLimit: &pidsLimit,
			},
		}),
...
func allowedLocalEnv() []string {
	allowed := map[string]bool{
		"PATH": true, "SYSTEMROOT": true, "WINDIR": true,
		"TMP": true, "TEMP": true, "HOME": true, "USERPROFILE": true,
