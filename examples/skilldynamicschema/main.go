...
type setOutputSchemaInput struct {
	// Schema is the JSON schema object, or a JSON string.
...
func (t *setOutputSchemaTool) Declaration() *tool.Declaration {
	return &tool.Declaration{
		...
		Description: "Set structured output JSON schema for subsequent model calls. Always pass `schema`.",
...
func (t *setOutputSchemaTool) Call(ctx context.Context, jsonArgs []byte) (any, error) {
	...
	if err := json.Unmarshal(in.Schema, &schema); err != nil {
		...
		if errText := json.Unmarshal(in.Schema, &schemaText); errText != nil {
			return map[string]any{"ok": false, "error": fmt.Sprintf("schema must be an object or a JSON string: %v", err)}, nil
		}
		if errParse := json.Unmarshal([]byte(schemaText), &schema); errParse != nil {
			return map[string]any{"ok": false, "error": fmt.Sprintf("invalid schema JSON string: %v", errParse)}, nil
		}
	}
	if len(schema) == 0 {
