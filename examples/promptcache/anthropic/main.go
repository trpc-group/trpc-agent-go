...
func main() {
	...
	fmt.Println("    A turn that runs tools also marks its newest tool results in the")
	fmt.Println("    request that carries them, when one of the four cache_control")
	fmt.Println("    slots is still free, so tool output is cached as it is sent.")
	fmt.Println()
	...
	fmt.Println("           Tool results are marked in the request that carries them")
	fmt.Println("           when a cache_control slot is still free.")
	fmt.Println()
