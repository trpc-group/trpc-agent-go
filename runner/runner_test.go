...
func TestWithSessionMultimodalExternalization(t *testing.T) {
	t.Run("sets config in options", func(t *testing.T) {
		opts := &Options{}

		option := WithSessionMultimodalExternalization(
			SessionMultimodalExternalizationConfig{Enabled: true},
		)
		option(opts)

		assert.True(t, opts.sessionMultimodalExternalization.Enabled)
	})
}

func TestWrapSessionMultimodalExternalization(t *testing.T) {
	t.Run("disabled returns original service", func(t *testing.T) {
		inner := sessioninmemory.NewSessionService()
		opts := Options{sessionService: inner}

		wrapped := wrapSessionMultimodalExternalization(opts)

		assert.Equal(t, inner, wrapped)
	})

	t.Run("enabled wraps service and externalizes events", func(t *testing.T) {
		ctx := context.Background()
		...
		opts := Options{
			...
			artifactService:                  artifactinmemory.NewService(),
			sessionMultimodalExternalization: SessionMultimodalExternalizationConfig{Enabled: true},
		}
