...
type startErrorAgent struct {
	*mockAgent
}

func (a *startErrorAgent) Run(
	context.Context,
	*agent.Invocation,
...
func TestRunnerRewindReplayDoesNotDuplicateStartedTurn(t *testing.T) {
	ctx := context.Background()
	service := sessioninmemory.NewSessionService()
	t.Cleanup(func() { require.NoError(t, service.Close()) })
	...
	r := NewRunner(
		"app",
		&startErrorAgent{mockAgent: &mockAgent{name: "agent"}},
		WithSessionService(service),
	)
	events, err = r.Run(
		ctx,
	...
	require.Nil(t, events)
	require.ErrorContains(t, err, "agent start failed")
	...
	require.Nil(t, events)
	require.ErrorIs(t, err, session.ErrRewindConflict)
	...
	for i := range active.Events {
		if active.Events[i].RequestID == "request-edited" &&
			active.Events[i].Author == authorUser {
			userEvents++
		}
	}
...
func TestRunnerLatestTurnReplacementRestoresUnfinishedTurn(t *testing.T) {
	...
	events, err := r.Run(
		...
		model.NewUserMessage("edited"),
		agent.WithRequestID("request-edited"),
		agent.WithLatestTurnReplacement("request-unfinished"),
	)
	require.NoError(t, err)
	for range events {
	}
...
func TestRunnerLatestTurnReplacementRetainsSharedState(t *testing.T) {
	...
	events, err := r.Run(
		...
		model.NewUserMessage("edited"),
		agent.WithRequestID("request-edited"),
		agent.WithLatestTurnReplacement(start.RequestID),
	)
	require.NoError(t, err)
	for range events {
	}
...
func TestRunnerLatestTurnReplacementReservesNewRequestBeforeMutation(t *testing.T) {
	...
	service := sessioninmemory.NewSessionService()
	t.Cleanup(func() { require.NoError(t, service.Close()) })
	key := session.Key{AppName: "app", UserID: "user", SessionID: "editable"}
	sess, err := service.CreateSession(ctx, key, nil)
	require.NoError(t, err)
	start := event.NewResponseEvent(
		"unfinished-invocation",
	...
	activeEvents, err := r.Run(
		ctx,
		"other-user",
		"other-session",
		model.NewUserMessage("active"),
		agent.WithRequestID("request-edited"),
	)
	require.NoError(t, err)
	...
	require.True(t, r.Cancel("request-edited"))
...
func TestResolveLatestTurnReplacement(t *testing.T) {
	tests := []struct {
		...
		{
			name: "replacement",
			opts: agent.NewRunOptions(agent.WithRequestID("new-request"),
		...
		},
		{
			...
			opts: agent.NewRunOptions(agent.WithRequestID(""),
				agent.WithLatestTurnReplacement("old-request")),
			wantErr: "requires an explicit request id",
		},
		{
			name: "same request",
			opts: agent.NewRunOptions(agent.WithRequestID("request"),
				agent.WithLatestTurnReplacement("request")),
			wantErr: "request ids must differ",
		},
		{
			name: "repeated request option uses latest value",
			opts: agent.NewRunOptions(
...
func TestRunnerRoutedPersistenceMakesLatestTurnUnavailable(t *testing.T) {
	...
	require.ErrorIs(t, err, revision.ErrRewindUnavailable)
}
