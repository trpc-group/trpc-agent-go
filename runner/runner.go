...
func (r *runner) Run(
	...
	execCtx = revision.ContextWithRequestID(execCtx, ro.RequestID)
	...
	ro, awaitUserReplyRootName, awaitUserReplyLookupPath, deferredTurnState, err := r.applyAwaitUserReplyRoute(
		awaitCtx,
		sessionKey,
		sess,
		message,
...
func resolveLatestTurnReplacement(
	...
	if replacement.ExpectedRequestID == ro.RequestID {
		return fmt.Errorf(
...
type turnStateDeltaContextKey struct{}

type turnStateDelta struct {
...
func contextWithTurnStateDelta(
	ctx context.Context,
	state session.StateMap,
) context.Context {
	if ctx == nil {
	...
	return context.WithValue(ctx, turnStateDeltaContextKey{}, &turnStateDelta{
		state: session.StateMap(mergeStateDelta(nil, state)),
	})
}

func takeTurnStateDelta(ctx context.Context) session.StateMap {
	if ctx == nil {
		return nil
	}
	pending, _ := ctx.Value(turnStateDeltaContextKey{}).(*turnStateDelta)
	if pending == nil || pending.consumed {
		return nil
	}
	pending.consumed = true
	return session.StateMap(mergeStateDelta(nil, pending.state))
}
...
func (r *runner) appendMessagesAsSessionEvents(
	...
	turnStarted := false
	_, rewindFenced := revision.RewindHeadFence(sess)
	for _, pending := range messages {
		...
		appendCtx := ctx
		turnDelta := takeTurnStateDelta(ctx)
		if len(turnDelta) > 0 {
			evt.StateDelta = mergeStateDelta(evt.StateDelta, turnDelta)
		}
		if !turnStarted && (pending.currentTurn || rewindFenced) {
		...
		if rewindFenced {
			revision.ClearRewindHeadFence(sess)
			rewindFenced = false
		}
...
func (r *runner) appendIncomingMessage(
	...
	agent.InjectIntoEvent(invocation, evt)
	evt = r.applyEventPlugins(ctx, invocation, evt)
	evt.StateDelta = mergeStateDelta(evt.StateDelta, takeTurnStateDelta(ctx))
...
func (r *runner) resolveSessionForRun(
	...
	result, err := revision.Rewind(
		...
		session.RewindRequest{
			Key:                   key,
			TargetRequestID:       replacement.ExpectedRequestID,
			ExpectedHeadRequestID: replacement.ExpectedRequestID,
			IdempotencyKey:        ro.RequestID,
		},
	)
	if err != nil {
		return nil, err
...
func (r *runner) markRoutedTurnHazard(
	...
	generation, ok := revision.Generation(loop.sess)
	if !ok {
		return nil
	}
	rootKey := session.Key{
...
func (r *runner) persistCurrentTurnMessages(
	...
	ag agent.Agent,
	awaitUserReplyLookupPath string,
	deferredTurnState session.StateMap,
	message model.Message,
	persistedCurrentTurnMessages []model.Message,
	ro agent.RunOptions,
) error {
	ctx = contextWithTurnStateDelta(ctx, deferredTurnState)
	if awaitUserReplyLookupPath != "" {
