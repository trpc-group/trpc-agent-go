...
func New(appName string, opts ...Option) (rootrunner.Runner, error) {
...
func (r *runner) postRun(
	...
	if httpResponse.StatusCode < 200 || httpResponse.StatusCode >= 300 {
		return nil, r.httpStatusError(httpResponse)
	}
...
func (r *runner) httpStatusError(response *http.Response) error {
	body, err := io.ReadAll(response.Body)
	if err != nil {
		return fmt.Errorf("trpcagent runner: run returned %s", response.Status)
	}
	detail := string(body)
	...
	return fmt.Errorf("trpcagent runner: run returned %s: %s", response.Status, detail)
}
