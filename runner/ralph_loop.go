...
var errRalphLoopMissingStopCondition = errors.New(
	"ralph loop: missing completion promise and verify command",
)
...
type RalphLoopConfig struct {
	...
	//
	// The tag strings are configurable via PromiseTagOpen/Close.
...
//   - VerifyCommand exits with code 0 (when set), OR
...
func (a *ralphLoopAgent) runLoop(
	...
	for iter := 1; iter <= max; iter++ {
		...
		done, feedback := a.verifyIteration(ctx, lastFull)
		if done {
			return
		}
...
func (a *ralphLoopAgent) verifyIteration(
	...
	complete := (!wantPromise || promiseOK) && (!wantCmd || cmdOK)
	if complete {
		return true, ""
	}
