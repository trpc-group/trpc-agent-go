...
func TestInitDenialMonitorDegradesWhenCachedLogStreamUnavailable(t *testing.T) {
	resetDiagnosticsCapsCacheForTest()
	t.Cleanup(resetDiagnosticsCapsCacheForTest)
	old := macosLogStreamCommand
	macosLogStreamCommand = filepath.Join(t.TempDir(), "missing-log")
	t.Cleanup(func() { macosLogStreamCommand = old })
	...
	if rt.sandboxDenialCollectingReady() {
		t.Fatal("sandboxDenialCollectingReady = true, want false after log stream failure")
	}
	caps := rt.DiagnosticsCapability()
	if caps.EventStreamAvailable || caps.StrongCorrelation {
		t.Fatalf("caps = %#v, want degraded stream availability", caps)
	}
}

func TestStartMacOSLogStreamMonitorWithPredicateRejectsMissingCommand(t *testing.T) {
	old := macosLogStreamCommand
	macosLogStreamCommand = filepath.Join(t.TempDir(), "missing-log")
	t.Cleanup(func() { macosLogStreamCommand = old })

	monitor, err := startMacOSLogStreamMonitorWithPredicate(`eventMessage CONTAINS "Sandbox:"`)
	if err == nil {
		monitor.stop()
		t.Fatal("startMacOSLogStreamMonitorWithPredicate = nil, want start error")
	}
}

func TestStartMacOSLogStreamMonitorWithPredicateRejectsImmediateExit(t *testing.T) {
	script := filepath.Join(t.TempDir(), "exit-immediately.sh")
	if err := os.WriteFile(script, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	old := macosLogStreamCommand
	macosLogStreamCommand = script
	t.Cleanup(func() { macosLogStreamCommand = old })

	monitor, err := startMacOSLogStreamMonitorWithPredicate(`eventMessage CONTAINS "Sandbox:"`)
	if err == nil {
		monitor.stop()
		t.Fatal("startMacOSLogStreamMonitorWithPredicate = nil, want error before output")
	}
}

func TestRandomHexFallsBackWhenCryptoRandFails(t *testing.T) {
	old := cryptoRandRead
	cryptoRandRead = func([]byte) (int, error) { return 0, os.ErrPermission }
	t.Cleanup(func() { cryptoRandRead = old })

	got := randomHex(8)
	if len(got) != 16 {
		t.Fatalf("randomHex fallback = %q, want 16 hex chars", got)
	}
}

func TestMacOSVersionKeyReturnsUnknownWhenCommandMissing(t *testing.T) {
	old := macOSVersionCommand
	macOSVersionCommand = filepath.Join(t.TempDir(), "missing-sw_vers")
	t.Cleanup(func() { macOSVersionCommand = old })

	if got := macOSVersionKey(); got != "unknown" {
		t.Fatalf("macOSVersionKey() = %q, want unknown", got)
	}
}
