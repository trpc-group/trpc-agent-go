...
const (
	...
	ExitSetupFailed = 75
	// SetupErrorPrefix marks failures emitted before the user command starts.
	...
	// UserExitPrefix marks a user-command exit observed by the trusted relay.
	UserExitPrefix = "egress-relay: user-exit:"
)
...
func (r *Relay) handle(client net.Conn) {
	...
	upstream, err := net.Dial("unix", r.UnixPath)
	if err != nil {
		return
	}
