...
func (r *Runtime) defaultCredentialDenyMaskArgs(
	...
	for _, path := range defaultCredentialDenyPaths() {
		...
		if !profile.exposesHostRoot() && home != "" && sameOrChild(home, abs) {
			continue
		}
		...
		if info.IsDir() {
			args = append(args, "--perms", "000", "--tmpfs", abs)
			...
			args = append(args, "--remount-ro", abs)
			continue
		}
		args = append(args, "--ro-bind", denyReadMaskSource(ws), abs)
	}
	return args, nil
}

// defaultCredentialGrantArgs restores an explicitly granted child of a masked
// credential directory. These binds are inserted after that directory's tmpfs
// and before --remount-ro so granting one file does not expose siblings.
func (r *Runtime) defaultCredentialGrantArgs(
	...
	for _, rule := range profile.fileSystem.Rules {
		...
		target := filepath.Clean(rule.Path)
		if sameOrChild(wsAbs, target) || seen[target] || !isDefaultCredentialChild(target) {
			continue
		}
		rel, err := filepath.Rel(wsAbs, target)
		if err != nil {
...
func isDefaultCredentialChild(target string) bool {
	_, ok := credentialDenyParent(target)
	return ok
}
