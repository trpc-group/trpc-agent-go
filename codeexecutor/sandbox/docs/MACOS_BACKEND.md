...
the guest cannot create pathname or abstract Unix domain sockets even though
host socket paths remain visible under `--ro-bind / /`. Anonymous stream
socketpairs remain available. macOS uses Seatbelt network rules plus Mach
service and Unix socket policy. The cross-platform model remains binary, while
macOS-specific extension fields expose IPC affordances that Linux does not
claim to support through path allowlists.
