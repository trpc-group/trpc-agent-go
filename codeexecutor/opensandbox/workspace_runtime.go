...
//   - workspace_collect.go: Collect (ListDirectoryWithDepth + host-side
//     doublestar matching), readFile, StageInputs, CollectOutputs
//     workspace_run.go:     RunProgram, resolveRunCwd, ExecuteInline,
//     runBash, cappedBuffer/cappedOutputBuffer, shellQuote, sanitize,
//     isTimeoutErr, formatExecutionError
...
func (r *workspaceRuntime) CreateWorkspace(
	...
	// Defense-in-depth: a pre-existing runBase (or a leftover workspace
	// path) may carry skills/work/runs/out symlinks pointing outside the
	// workspace. mkdir -p follows such symlinks and would create content
	// outside runBase. Strip them first.
	for _, d := range dirs {
...
// false promise.
//
// Persist / MaxDiskBytes are not enforced in v1: workspaces are
// per-call, and disk caps are not wired to the OpenSandbox API.
func validateWorkspacePolicy(pol codeexecutor.WorkspacePolicy) error {
	if pol.Persist {
		return errors.New(
			"opensandbox: WorkspacePolicy.Persist is not supported in v1; " +
				"workspaces are per-call and cleaned up automatically",
		)
	}
