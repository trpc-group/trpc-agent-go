...
type mockOpenSandboxServer struct {
	...
	// nil means no exit_code field (test the nil 鈫?-1 fallback).
	exitCode *int
	stdout   string
	stderr   string
	...
	// rm, chmod 鈥?those without `&& cd `) to use the configured exitCode
	...
	// listDirectoryCalls counts GET /directories/list requests so tests
	// can assert Collect enumerates the workspace in a single round
	// trip; listDirectoryDepths records the depth query of each call.
	listDirectoryCalls  int
	listDirectoryDepths []string
...
// containing whitespace (tabs, newlines) 鈥?use parseShellQuotedPaths
// for those.
...
// in shellQuote, so a path appears as a doubly-nested `'\鈥?path>'\鈥漙;
// the parser first unwraps the outer bash -c quoting (reversing
// shellQuote's '\鈥?escapes) and then returns the single-quoted
...
func (m *mockOpenSandboxServer) handle(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodGet && path == "/files/download":
		...
		return
	case r.Method == http.MethodGet && path == "/directories/list":
		m.handleListDirectory(w, r)
		return
	}

	// Unknown route 鈥?return 404 so the test fails loudly.
...
func (m *mockOpenSandboxServer) handleCommand(w http.ResponseWriter, r *http.Request) {
	...
	// "timeout" 鈥?exercising isTimeoutErr in RunProgram. Only apply
	...
	// Handle readlink -f and readlink -m calls from resolveSandboxPath /
	...
	if strings.Contains(req.Command, "readlink -z -f") ||
		...
		stdout = resolveMockSymlink(p, symlinks) + "\x00"
		if !forceInfraExit {
			zero := 0
			exitCode = &zero
		}
	...
	if stdout != "" {
		// Marshal the text with encoding/json: the readlink probes emit
		// NUL-terminated records, which fmt's %q renders as \x00 鈥?an
		// invalid JSON escape. json.Marshal emits the spec-compliant
		// \u0000 instead.
	...
	// them too 鈥?used to test runBash error and incomplete-stream
	// paths.
...
// handleListDirectory serves GET /directories/list, the execd endpoint
// behind the SDK's ListDirectoryWithDepth that Collect uses to
// enumerate the workspace. It mirrors the observed execd behaviour:
//
//   - entries are lstat'ed and carry a "type" of file / directory /
//     symlink / other, so a symlink is reported as such rather than as
//     its target;
//   - a listing root that is itself a symlink is refused with 400
//     ("refusing to traverse") instead of being followed.
//
// Entries come from searchResults (seeded via setSearchResults) joined
// under the requested dir. A spec may carry an explicit "name:type"
// suffix ("dir" is accepted as an alias of "directory"); a path
// registered via setSymlink is reported as a symlink regardless. Sizes
// are looked up from the files map (setDownloadData) so Collect's
// SizeBytes reflects the seeded content, defaulting to the 12-byte
// "mock-content" that handleDownloadFile returns. With no configured
// results a single output.txt is listed so basic Collect tests work.
func (m *mockOpenSandboxServer) handleListDirectory(w http.ResponseWriter, r *http.Request) {
	dir := r.URL.Query().Get("path")
	m.mu.Lock()
	defer m.mu.Unlock()
	m.listDirectoryCalls++
	m.listDirectoryDepths = append(m.listDirectoryDepths, r.URL.Query().Get("depth"))
	w.Header().Set("Content-Type", "application/json")
	if _, isLink := m.symlinks[dir]; isLink {
		w.WriteHeader(http.StatusBadRequest)
		fmt.Fprintf(w, `{"code":"INVALID_REQUEST_BODY","message":%q}`,
			"path is a symbolic link, refusing to traverse: "+dir)
		return
	}
	specs := m.searchResults
	if len(specs) == 0 {
		specs = []string{"output.txt"}
	}
	...
	for _, spec := range specs {
		...
		if _, isLink := m.symlinks[p]; isLink {
			fileType = "symlink"
		}
		size := int64(12)
		...
		if fileType == "untyped" {
			// Simulate an execd build that predates type metadata.
...
func TestWorkspace_StageInputs_NotImplemented(t *testing.T) {
	...
	assert.ErrorIs(t, err, errDeclarativeIONotSupported)
}
...
// TestWorkspace_EngineFS_DeclarativeIOStub verifies that
// Engine().FS() surfaces the declarative-I/O stub error: the minimal
// executor does not advertise SupportsDeclarativeIO (unknown), so the
// raw runtime stubs are reachable through the engine and callers get
// an actionable "not supported" error.
func TestWorkspace_EngineFS_DeclarativeIOStub(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
...
// unique to timeouts 鈥?the OOM killer, manual kills, and sandbox
...
// TestResolveSandboxPath_SSESplitPreservesNULFraming is the regression
// for NUL-framing corruption when the SDK delivers the readlink probe's
// stdout split across multiple SSE OutputMessages. cappedBuffer's line
// semantics inject a newline between messages, which would corrupt the
// record; runBashRaw must accumulate byte-exact so the resolved path
// is not mangled and PutFiles / RunProgram keep working.
func TestResolveSandboxPath_SSESplitPreservesNULFraming(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
	...
	// Deliver the NUL-terminated stdout as many small SSE events.
	m.setStdoutChunkSize(3)

	target := ws.Path + "/sub/dir"
	resolved, err := exec.rt.resolveSandboxAncestor(context.Background(), target, ws.Path)
	require.NoError(t, err)
	assert.Equal(t, target, resolved, "resolved path must survive the SSE split intact")
}
...
// a newline 鈥?all legal in POSIX filenames and producible by user or
// model code 鈥?survive collection end to end. The listing arrives as
// JSON, so no shell framing or trailing-newline stripping can touch the
// names; the test guards against reintroducing a line-oriented path.
func TestWorkspace_Collect_FilenamesWithDelimiters(t *testing.T) {
	...
	// boundary: any TrimSpace / TrimRight on the listed path would make
	// this file fail collection.
	...
	// verifies its size comes from the listing metadata for exactly
	// that path.
...
func TestWorkspace_Collect_Truncation(t *testing.T) {
	...
	// Compute the exact path the mock's handleListDirectory will return
...
// developer mode. When symlink creation fails the test is skipped 鈥?// Linux/macOS CI covers the regression path.
...
// TestWorkspace_Collect_SkipsNonRegularEntries verifies that Collect
// only downloads entries execd reports as regular files: directories,
// symlinks (even ones whose target is inside the workspace) and other
// node types such as FIFOs are all skipped, because DownloadFile would
// follow a symlink and block on a FIFO.
func TestWorkspace_Collect_SkipsNonRegularEntries(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
	...
	m.setSymlink(ws.Path+"/inside_link.txt", ws.Path+"/real.txt")
	m.setSearchResults([]string{
		"subdir:dir", "real.txt", "inside_link.txt", "pipe.txt:other",
	})

	files, err := exec.Collect(context.Background(), ws, []string{"*"})
	require.NoError(t, err)
	require.Len(t, files, 1, "only the regular file should be collected")
	assert.Equal(t, "real.txt", files[0].Name)
	assert.Equal(t, 1, m.listDirectoryCalls, "Collect must enumerate the workspace in one round trip")
}
...
// TestWorkspace_Collect_PathGlobsMatchRelativePath verifies the two
// matching rules Collect applies to workspace-relative paths: a pattern
// without a separator matches by basename at any depth, while a pattern
// with a separator (including interior spaces) is matched against the
// relative path as written and does not match the same basename
// elsewhere.
func TestWorkspace_Collect_PathGlobsMatchRelativePath(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
	...
	m.setSearchResults([]string{
		"out/reports 2026/q1.csv",
		"out/reports 2026/nested/q2.csv",
		"other/q3.csv",
		"notes.txt",
		"deep/a/b/notes.txt",
	})

	files, err := exec.Collect(context.Background(), ws, []string{"out/reports 2026/*.csv"})
	require.NoError(t, err)
	require.Equal(t, []string{"out/reports 2026/q1.csv"}, namesOf(files),
		"a path glob must match the relative path as written, not by basename")

	files, err = exec.Collect(context.Background(), ws, []string{"notes.txt"})
	require.NoError(t, err)
	require.ElementsMatch(t, []string{"notes.txt", "deep/a/b/notes.txt"}, namesOf(files),
		"a bare basename pattern must match at any depth")

	files, err = exec.Collect(context.Background(), ws, []string{"**/*.csv"})
	require.NoError(t, err)
	require.Len(t, files, 3)
}

// TestWorkspace_Collect_InvalidPatternRejected verifies a malformed glob
// fails Collect explicitly instead of silently matching nothing.
func TestWorkspace_Collect_InvalidPatternRejected(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()

	ws, err := exec.CreateWorkspace(context.Background(), "exec-badglob", codeexecutor.WorkspacePolicy{})
	require.NoError(t, err)

	_, err = exec.Collect(context.Background(), ws, []string{"out/[unclosed"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "invalid collect pattern")
	assert.Equal(t, 0, m.listDirectoryCalls, "pattern validation must happen before any remote call")
}

// TestWorkspace_Collect_SymlinkedWorkspaceRootIsAnError verifies that
// when execd refuses to list a workspace root that is a symlink, Collect
// surfaces that error rather than returning an empty result.
func TestWorkspace_Collect_SymlinkedWorkspaceRootIsAnError(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()

	ws, err := exec.CreateWorkspace(context.Background(), "exec-rootlink", codeexecutor.WorkspacePolicy{})
	require.NoError(t, err)
	m.setSymlink(ws.Path, "/tmp/outside")

	_, err = exec.Collect(context.Background(), ws, []string{"*"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "list workspace files")
	assert.Contains(t, err.Error(), "refusing to traverse")
}

// TestWorkspace_Collect_UntypedListingIsAnError verifies Collect refuses
// a listing whose entries carry no type metadata: without lstat types a
// symlink cannot be told apart from a file, so collecting would be
// unsafe. This also documents the minimum execd behaviour Collect needs.
func TestWorkspace_Collect_UntypedListingIsAnError(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()

	ws, err := exec.CreateWorkspace(context.Background(), "exec-untyped", codeexecutor.WorkspacePolicy{})
	require.NoError(t, err)
	m.setSearchResults([]string{"a.txt:untyped"})

	_, err = exec.Collect(context.Background(), ws, []string{"*"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no file type metadata")
}
...
func TestWorkspace_WalkAndUpload_StreamsFiles(t *testing.T) {
	...
	// The open file must still be readable for the full payload 鈥?proves
...
// that was accepted 鈥?enumerated from the pinned parent handle 鈥?is
...
func TestOpenChildNoFollow_RejectsSwappedDirectory(t *testing.T) {
	...
	// Swap AFTER enumeration, BEFORE descent 鈥?the attack window.
...
func TestWorkspace_WalkAndUpload_SkipsNonRegularFiles(t *testing.T) {
	...
	// admin/developer mode 鈥?skip rather than fail the suite.
...
// listing metadata, not just "at least limit+1". Without the
// listed size, a file 3x the cap would report SizeBytes ==
...
func TestWorkspace_Collect_SizeBytesReflectsRealSize(t *testing.T) {
	...
	// Seed a file 3x the cap. The mock's handleListDirectory looks up
	...
	assert.Equal(t, realSize, files[0].SizeBytes,
		"SizeBytes should be the real file size from the listing, not limit+1")
	assert.True(t, files[0].Truncated, "file exceeding maxReadSizeBytes should be truncated")
...
// 鈥?the SDK's HTTP client rejects the request before it reaches the
...
// with cleanupContext(cancelledCtx) DOES send the rm -rf command 鈥?// the detached context is not cancelled, so the SDK's HTTP client
...
// skips a symlink pointing outside the workspace rather than reading
// its target: the listing reports it with type "symlink", and only
...
func TestWorkspace_Collect_SkipsSymlinkEscape(t *testing.T) {
	...
	// The listing reports leak.txt as a symlink; Collect must not
	// download it.
...
// when listing metadata is outdated.
...
// a file shrinks between the listing (knownSize large) and DownloadFile
// (actual content smaller), Truncated is false 鈥?not a false positive
...
func TestPutFiles_RemovesPreExistingSymlink(t *testing.T) {
	...
	// PutFiles should succeed 鈥?the symlink is removed before upload.
	err = exec.PutFiles(context.Background(), ws, []codeexecutor.PutFile{{
		Path:    "output.txt",
...
// --- L2: Collect file-count limit ---

// TestCollect_CountLimit verifies that Collect downloads at most
// maxCollectFiles matches and flags the result as partial, so model-
// generated code that creates thousands of matching files cannot make
// Collect stream them all back.
func TestCollect_CountLimit(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
...
// package-private v1 sentinel is detectable with errors.Is, protecting
// the contract that StageInputs/CollectOutputs return this sentinel
// for v1 protocol incompatibility.
func TestErrDeclarativeIONotSupported_PackageSentinel(t *testing.T) {
	...
	// StageInputs returns errDeclarativeIONotSupported.
	err = exec.StageInputs(context.Background(), ws, nil)
	assert.ErrorIs(t, err, errDeclarativeIONotSupported,
		"StageInputs must return errDeclarativeIONotSupported detectable via errors.Is")

	// CollectOutputs returns errDeclarativeIONotSupported.
	_, err = exec.CollectOutputs(context.Background(), ws, codeexecutor.OutputSpec{})
	assert.ErrorIs(t, err, errDeclarativeIONotSupported,
		"CollectOutputs must return errDeclarativeIONotSupported detectable via errors.Is")
}
...
func TestExecuteCode_AggregateOutputCap(t *testing.T) {
	...
	// plus the truncation marker. Without the cap, 10 blocks 脳 1 MiB
...
// TestCollect_OverlappingPatternsDedupBeforeCounting verifies that a
// file matched by several patterns consumes one unit of the file budget
// and is returned once: exactly maxCollectFiles files matched by two
// overlapping globs must come back complete, with no partial marker.
func TestCollect_OverlappingPatternsDedupBeforeCounting(t *testing.T) {
	m := newMockServer(t)
	defer m.close()
	exec := newTestExecutor(t, m)
	defer exec.Close()
	...
	names := make([]string, maxCollectFiles)
	for i := range names {
	...
	files, err := exec.Collect(context.Background(), ws, []string{"**/*", "**/*.txt", "*.txt"})
	require.NoError(t, err)
	require.Len(t, files, maxCollectFiles, "each file must be returned once")
	for _, f := range files {
		assert.NotEqual(t, collectLimitsHitMarkerName, f.Name,
			"overlapping patterns must not double-spend the file budget")
	}
	assert.Equal(t, 1, m.listDirectoryCalls, "multiple patterns must still cost one listing")
}
...
// --- Collect listing selection (pure functions) ---

func TestCompileCollectGlobs(t *testing.T) {
	globs, err := compileCollectGlobs([]string{
		" *.txt ", "./out/*.csv", "", "**/*.png", "sub/deep/x.log",
	})
	require.NoError(t, err)
	assert.Equal(t, []string{"**/*.txt", "out/*.csv", "**/*.png", "sub/deep/x.log"}, globs,
		"bare basenames get a **/ prefix; ./ is stripped; path globs are kept as written")

	_, err = compileCollectGlobs([]string{"ok.txt", "bad/[x"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), `invalid collect pattern "bad/[x"`)
}

func TestSelectCollectMatches_FiltersTypesPrefixAndMetadata(t *testing.T) {
	ws := "/tmp/run/ws"
	entries := []osb.FileInfo{
		{Path: ws + "/a.txt", Type: "file", Size: 3},
		{Path: ws + "/sub", Type: "directory"},
		{Path: ws + "/sub/b.txt", Type: "file", Size: 5},
		{Path: ws + "/link.txt", Type: "symlink", Size: 20},
		{Path: ws + "/fifo.txt", Type: "other"},
		{Path: ws + "/.metadata.tmp", Type: "file", Size: 1},
		{Path: ws + "/.hidden.txt", Type: "file", Size: 2},
		{Path: ws + "/new\nline.txt", Type: "file", Size: 1},
		{Path: "/etc/passwd.txt", Type: "file", Size: 9},
		{Path: ws + "/a.txt", Type: "file", Size: 3}, // duplicate entry
	}
	matches, capped, err := selectCollectMatches(entries, ws, []string{"**/*.txt"})
	require.NoError(t, err)
	...
	assert.Equal(t, []string{"a.txt", "sub/b.txt", ".hidden.txt", "new\nline.txt"}, rels,
		"only typed regular files under the workspace, deduplicated, metadata temp skipped; "+
			"dotfiles and newline names are ordinary matches")
	assert.Equal(t, ws+"/sub/b.txt", matches[1].path)
	assert.Equal(t, int64(5), matches[1].size)
}

func TestSelectCollectMatches_CapsAndFlagsPartial(t *testing.T) {
	ws := "/tmp/run/ws"
	entries := make([]osb.FileInfo, 0, maxCollectFiles+1)
	for i := 0; i <= maxCollectFiles; i++ {
		entries = append(entries, osb.FileInfo{
			Path: fmt.Sprintf("%s/f%04d.txt", ws, i), Type: "file", Size: 1,
		})
	}
	matches, capped, err := selectCollectMatches(entries, ws, []string{"**/*"})
	require.NoError(t, err)
	assert.Len(t, matches, maxCollectFiles)
	assert.True(t, capped, "one eligible file beyond the budget must flag the result as partial")

	matches, capped, err = selectCollectMatches(entries[:maxCollectFiles], ws, []string{"**/*"})
	require.NoError(t, err)
	assert.Len(t, matches, maxCollectFiles)
	assert.False(t, capped, "filling the budget exactly is a complete result")
}

func TestSelectCollectMatches_UntypedEntryIsAnError(t *testing.T) {
	ws := "/tmp/run/ws"
	_, _, err := selectCollectMatches([]osb.FileInfo{
		{Path: ws + "/a.txt", Type: "file"},
		{Path: ws + "/b.txt"},
	}, ws, []string{"**/*"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no file type metadata")
