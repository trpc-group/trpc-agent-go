...
func (r *workspaceRuntime) RunProgram(
	...
	// before mkdir/upload. Leftover content from earlier calls can leave
...
func (r *workspaceRuntime) ExecuteInline(
	...
	// Each call gets a fresh per-call workspace; clean it up when the
	// call finishes. Use a context detached from the parent's
...
// (resolveSandboxPath / resolveSandboxAncestor), whose records are
...
// resolved path stays under ws.Path. Used by RunProgram as
// defense-in-depth so pre-existing or leftover symlinks cannot
// redirect mkdir/upload via skills/work/runs/out paths.
...
// infrastructure scripts (resolveSandboxPath / resolveSandboxAncestor)
// whose records are delimited by NUL bytes: a newline
