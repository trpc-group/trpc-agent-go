...
//
// Until then, reuse of a runBase directory across calls must only happen
// with trusted in-sandbox code; the default per-call workspace gives
// every execution a fresh directory where no adversary-controlled
// symlink can pre-exist.
...
// resolvedPathB64Prefix marks a readlink result that the sandbox script
// base64-encoded before printing. A canonical path may legally contain
// a newline; the SDK delivers stdout one line per event and drops the
// newline, so the raw path cannot cross the stream intact.
const resolvedPathB64Prefix = "b64:"

// decodeListedPath recovers a sandbox path from a readlink record.
// Live scripts prefix GNU base64 (-w0); the mock server emits raw
// paths, which pass through unchanged because they lack the prefix.
func decodeListedPath(s string) string {
	if strings.HasPrefix(s, resolvedPathB64Prefix) {
		raw, err := base64.StdEncoding.DecodeString(strings.TrimPrefix(s, resolvedPathB64Prefix))
		if err == nil {
			return string(raw)
		}
	}
...
func (r *workspaceRuntime) resolveSandboxPath(
	...
	script := "printf 'b64:%s' \"$(readlink -z -f -- " + shellQuote(target) +
		" | tr -d '\\0' | base64 -w0)\""
	...
	resolved := decodeListedPath(strings.TrimSuffix(out, "\x00"))
	if resolved == "" {
		return "", fmt.Errorf(
...
func (r *workspaceRuntime) resolveSandboxAncestor(
	...
	out, err := r.runBashRaw(ctx, "printf 'b64:%s' \"$(readlink -z -m "+
		shellQuote(target)+" | tr -d '\\0' | base64 -w0)\"", defaultCreateTimeout)
	if err != nil {
		return "", fmt.Errorf(
