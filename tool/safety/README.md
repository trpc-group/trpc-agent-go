...
| 2 | credential_access | `R-CRED-001` | argv/cwd hitting `~/.ssh`, `**/.env`, `**/id_rsa`, credentials; `file:` URIs are decoded to their filesystem path first so `curl file:///etc/shadow` (any RFC 8089 spelling, incl. percent-encoded) cannot hide the path inside a URI. Path-bearing portions embedded in **option values** are extracted before matching, so a forbidden path cannot hide inline: `--upload-file=/etc/shadow`, `-T/etc/shadow`, and curl's `@file` read syntax (`--data-binary=@/etc/shadow`, `-d @/etc/shadow`, `-F name=@/etc/shadow`) are all matched | critical |
