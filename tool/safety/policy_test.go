...
func strictJSONPolicyCases() []invalidPolicyCase {
	return []invalidPolicyCase{
		...
		invalidJSONPolicy("json duplicate command field", `{"version":1,"commands":{"denied":["rm"],"denied":[]}}`),
		invalidJSONPolicy("json case variant command field", `{"version":1,"commands":{"denied":["rm"],"Denied":[]}}`),
		invalidJSONPolicy("json duplicate limits field", `{"version":1,"limits":{"max_timeout":"1s","max_timeout":"2s"}}`),
		invalidJSONPolicy("json case variant limits field", `{"version":1,"limits":{"max_timeout":"1s","Max_Timeout":"2s"}}`),
