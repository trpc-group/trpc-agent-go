...
func (m *mockPostPolicyExtractor) ConfiguredUpdatePolicy() updatepolicy.Value {
	return m.policy
}
...
func TestAutoMemoryWorker_ObservesPostPolicyOperations(t *testing.T) {
	ext := &mockPostPolicyExtractor{
		policy: updatepolicy.Value(extractor.UpdatePolicyAppendOnly),
...
func TestReconcileOps_DoesNotOverwriteAssistantProvenance(t *testing.T) {
	ext := &mockAssistantExtractor{mockExtractor: &mockExtractor{}}
	op := newMockOperator()
	op.searchResults = []*memory.Entry{
		{
