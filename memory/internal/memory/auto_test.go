...
func TestReconcileOps_KeepsDistinctDatedState(t *testing.T) {
	previousTime := time.Date(2023, 8, 11, 0, 0, 0, 0, time.UTC)
	currentTime := time.Date(2023, 11, 30, 0, 0, 0, 0, time.UTC)
	op := newMockOperator()
	op.searchResults = []*memory.Entry{{
		ID:      "mem-negroni-count",
		AppName: "app", UserID: "u1",
		Memory: &memory.Memory{
			Memory:    "Has made Negronis at home 5 times since learning from Emma.",
			Topics:    []string{"Negroni", "Emma", "count"},
	...
	in := []*extractor.Operation{{
		Type:      extractor.OperationAdd,
		Memory:    "Has made Negronis at home 10 times since learning from Emma.",
		Topics:    []string{"Negroni", "Emma", "count"},
...
func TestReconcileOps_DropsDuplicateAtSameEventTime(t *testing.T) {
	eventTime := time.Date(2023, 11, 30, 0, 0, 0, 0, time.UTC)
	op := newMockOperator()
	op.searchResults = []*memory.Entry{{
		ID:      "mem-negroni-count",
		AppName: "app", UserID: "u1",
...
func TestReconcileOps_PrefersSameTimeDuplicateOverPriorState(t *testing.T) {
	previousTime := time.Date(2023, 8, 11, 0, 0, 0, 0, time.UTC)
	currentTime := time.Date(2023, 11, 30, 0, 0, 0, 0, time.UTC)
	op := newMockOperator()
	op.searchResults = []*memory.Entry{
		{
			ID: "prior", Memory: &memory.Memory{
				Memory:    "Has made Negronis at home 5 times since learning from Emma.",
				EventTime: &previousTime,
			},
			Score: 0.999,
		...
		{
			ID: "current", Memory: &memory.Memory{
				Memory:    "Has made Negronis at home 10 times since learning from Emma.",
				EventTime: &currentTime,
			},
			Score: 0.95,
...
func TestIsDistinctDatedState(t *testing.T) {
	previousTime := time.Date(2023, 8, 11, 0, 0, 0, 0, time.UTC)
	currentTime := time.Date(2023, 11, 30, 0, 0, 0, 0, time.UTC)
	tests := []struct {
		name     string
		...
		{name: "nil operation", existing: &memory.Memory{}, want: false},
		{name: "undated operation", op: &extractor.Operation{}, existing: &memory.Memory{}, want: false},
		{name: "nil existing", op: &extractor.Operation{EventTime: &currentTime}, want: false},
		{name: "undated existing", op: &extractor.Operation{EventTime: &currentTime}, existing: &memory.Memory{}, want: true},
		{name: "same time", op: &extractor.Operation{EventTime: &currentTime}, existing: &memory.Memory{EventTime: &currentTime}, want: false},
		{name: "different time", op: &extractor.Operation{EventTime: &currentTime}, existing: &memory.Memory{EventTime: &previousTime}, want: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, isDistinctDatedState(tt.op, tt.existing))
		})
	}
}
