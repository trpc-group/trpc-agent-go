...
func expectUpdateLoad(mock sqlmock.Sqlmock, key memory.Key, softDelete bool) time.Time {
	now := time.Now()
...
func TestService_UpdateMemory(t *testing.T) {
	...
	sourceCreatedAt := expectUpdateLoad(mock, key, false)
	targetID := imemory.GenerateMemoryID(
		&memory.Memory{Memory: "updated", Kind: memory.KindFact},
		key.AppName,
		key.UserID,
	)
...
func TestService_UpdateMemory_IDChanged_RotateMemory(t *testing.T) {
	...
	// Content changed → new ID → rotateMemory: BEGIN + pre-check + INSERT + DELETE + COMMIT.
	mock.ExpectBegin()
	mock.ExpectQuery("SELECT deleted_at IS NULL FROM memories").
		WithArgs(sqlmock.AnyArg(), key.AppName, key.UserID).
		WillReturnError(sql.ErrNoRows)
...
// soft-deleted row, rotateMemory explicitly revives it before deleting A.
...
func TestService_UpdateMemory_SoftDelete_RotateMemory_ReviveDeletedRow(t *testing.T) {
	...
	// BEGIN + pre-check finds B soft-deleted (not active) + revive B +
...
func TestService_UpdateMemory_SoftDelete_RotateMemory_TargetNotExist(t *testing.T) {
	...
	// Content changes → new ID "mem-B" → rotateMemory:
	// BEGIN + pre-check finds B does not exist (sql.ErrNoRows) + INSERT B +
	// soft-delete A + COMMIT.
	mock.ExpectBegin()
	mock.ExpectQuery("SELECT deleted_at IS NULL FROM memories").
		WithArgs(sqlmock.AnyArg(), key.AppName, key.UserID).
...
func TestService_UpdateMemory_RotateMemory_ConcurrentTargetInsertRollsBack(t *testing.T) {
	db, mock := setupMockDB(t)
	defer db.Close()
	svc := setupMockService(t, db, mock, WithSkipDBInit(true))
	defer svc.Close()
...
// the explicit target check returns an error and rolls back before A changes.
...
func TestService_UpdateMemory_HardDelete_RotateMemory_DeletedTargetCleanup(t *testing.T) {
	db, mock := setupMockDB(t)
	defer db.Close()
	svc := setupMockService(t, db, mock, WithSkipDBInit(true))
	defer svc.Close()
...
func TestService_UpdateMemory_HardDelete_RotateMemory_ActiveRowConflict(t *testing.T) {
	db, mock := setupMockDB(t)
	defer db.Close()
	svc := setupMockService(t, db, mock, WithSkipDBInit(true))
	defer svc.Close()
...
func TestService_UpdateMemory_RotatedTargetErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			arrange: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("UPDATE.*deleted_at = NULL").
					WillReturnResult(sqlmock.NewErrorResult(fmt.Errorf("rows failed")))
			},
			want: "revive rotated memory target rows affected",
		},
		{
		...
		},
		{
			...
			want: "delete rotated memory target rows affected",
		},
		{
			name: "delete zero rows",
			arrange: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec("DELETE FROM").
					WillReturnResult(sqlmock.NewResult(0, 0))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			db, mock := setupMockDB(t)
			opts := []ServiceOpt{WithSkipDBInit(true)}
			if tt.softDelete {
				opts = append(opts, WithSoftDelete(true))
			}
			svc := setupMockService(t, db, mock, opts...)
			t.Cleanup(func() {
				_ = svc.Close()
			})

			key := memory.Key{AppName: "app", UserID: "u1", MemoryID: "mem-A"}
			expectUpdateLoad(mock, key, tt.softDelete)
			mock.ExpectBegin()
			mock.ExpectQuery("SELECT deleted_at IS NULL FROM memories").
				WithArgs(sqlmock.AnyArg(), key.AppName, key.UserID).
				WillReturnRows(sqlmock.NewRows([]string{"active"}).AddRow(false))
...
func TestService_SearchMemories_HybridSearchBackfillsTemporalTail(t *testing.T) {
	db, mock := setupMockDB(t)
	defer db.Close()
	svc := setupMockServiceWithVector(
		t,
		db,
		mock,
	...
	const query = "What is the order of the sports events I watched in January?"
	now := time.Now()
	eventTime := time.Date(2023, time.January, 14, 0, 0, 0, 0, time.UTC)
	mock.ExpectQuery("SELECT memory_id.*LIMIT 6").WillReturnRows(
		sqlmock.NewRows(memColsWithSimilarity).
			AddRow(
				"m1", "app", "u1", "Watched an NBA game.", `["sports"]`,
				"episode", now, nil, nil, now, now, 0.95,
			).
			AddRow(
				"m2", "app", "u1", "Other memory.", `["other"]`,
				"fact", nil, nil, nil, now, now, 0.90,
			).
			AddRow(
				"m3", "app", "u1",
				"Watched the College Football National Championship.",
				`["sports"]`, "episode", eventTime, nil, nil, now, now, 0.85,
			),
	)
	mock.ExpectQuery("SELECT memory_id.*LIMIT 6").WillReturnRows(
		sqlmock.NewRows(memColsWithSimilarity),
	...
	require.NoError(t, mock.ExpectationsWereMet())
	require.Len(t, results, 2)
	require.Equal(t, []string{"m1", "m3"}, []string{
		results[0].ID,
		results[1].ID,
	})
}
