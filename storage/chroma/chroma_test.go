...
func TestDefaultClientBuilder(t *testing.T) {
	...
	if c.hc.Transport != custom || c.headers[authTokenHeader] != "token" || c.headers["X-Test"] != "yes" {
	...
	if _, err := defaultClientBuilder(
		WithBaseURL("http://127.0.0.1:1"),
		WithAPIKey("token"),
	); err == nil {
		t.Fatal("API key without explicit scope must resolve identity at construction")
	}
}
...
func TestIdentityScopeInference(t *testing.T) {
	t.Run("infers unique tenant and database", func(t *testing.T) {
		...
		if c.tenant != "cloud-tenant" || c.database != "cloud-db" {
			t.Fatalf("scope resolved at construction = %s/%s", c.tenant, c.database)
		}
	...
	t.Run("rejects missing unique database", func(t *testing.T) {
		...
		defer srv.Close()
		_, err := defaultClientBuilder(WithBaseURL(srv.URL), WithAPIKey("token"))
...
func TestHeartbeatAndClose(t *testing.T) {
	...
	t.Run("injected transport remains caller-owned", func(t *testing.T) {
		custom := &closeTrackingTransport{}
		cli, err := defaultClientBuilder(
			WithBaseURL("http://127.0.0.1:8000"),
			WithTransport(custom),
		)
		...
		if custom.closes != 0 {
			t.Fatalf("injected transport close calls = %d, want 0", custom.closes)
		}
	})
...
func TestCollectionInfoSnapshot(t *testing.T) {
	...
	c := &httpClient{
		collection: CollectionInfo{
			ID:                    "id",
			Name:                  "docs",
			SparseVectorIndexKeys: keys,
		},
	}

	info := c.CollectionInfo()
	info.SparseVectorIndexKeys[0] = "mutated"
	info.SparseVectorIndexKeys = append(info.SparseVectorIndexKeys, "extra")
	got := c.CollectionInfo()
	if got.ID != "id" || got.Name != "docs" {
		t.Fatalf("CollectionInfo() = %#v", got)
	}
	if !reflect.DeepEqual(got.SparseVectorIndexKeys, []string{"lexical"}) {
		t.Fatalf("SparseVectorIndexKeys = %v, mutation leaked into client state", got.SparseVectorIndexKeys)
	}

	unbound := (&httpClient{}).CollectionInfo()
	if unbound.SparseVectorIndexKeys != nil {
		t.Fatalf("unbound SparseVectorIndexKeys = %v, want nil", unbound.SparseVectorIndexKeys)
	}
}
...
// errOn4xxTransport mimics an injected transport that returns non-2xx
// responses as errors instead of exposing the HTTP response.
type errOn4xxTransport struct {
	calls int
}

func (t *errOn4xxTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	t.calls++
...
func TestInjectedTransport(t *testing.T) {
	var auth string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if auth != "token" || custom.calls != 1 {
		t.Fatalf("injected transport calls/auth = %d/%q", custom.calls, auth)
	}
	if err := cli.GetCollection(context.Background(), "missing"); err == nil ||
		errors.Is(err, ErrCollectionNotFound) {
		t.Fatalf("custom transport error = %v, want original error", err)
	}
}
...
// TestRedirectLoopStopsAtLimit verifies that checkRedirect bounds redirect
// loops even though a custom CheckRedirect disables net/http's built-in limit.
func TestRedirectLoopStopsAtLimit(t *testing.T) {
	...
	if err := cli.GetCollection(context.Background(), "docs"); err == nil ||
		...
		t.Fatalf("redirect loop error = %v, want stopped after 10 redirects", err)
	}
	if hops != 10 {
		t.Fatalf("redirect hops = %d, want 10", hops)
	}
}
