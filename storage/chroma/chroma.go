...
//   - dense /query and Cloud /search rank-expression APIs. Self-hosted servers
//     that do not implement /search return ErrSearchNotImplemented.
//   - a ClientBuilder with HTTP URL, tenant/database, Cloud API key, and auth
//     options.
...
type ClientBuilderOpts struct {
	...
	// When Transport resolves a service name, use the scheme it expects,
	// for example polaris://chroma-service.
	BaseURL string

	// Transport sends the requests. Nil uses a clone of
	...
	// Database is empty, the builder resolves them from /auth/identity
	// at construction.
	APIKey string
...
// WithTransport sets the transport used for every request. This can be used
// to customize proxies, service discovery, TLS, timeouts, or tracing. A nil
// transport is ignored and a clone of http.DefaultTransport is kept.
//
// The redirect policy always stays with this client: cross-origin redirects
// strip the API key and custom headers. An injected transport is owned by the
// caller and is never closed by Close.
func WithTransport(rt http.RoundTripper) ClientBuilderOpt {
...
// or database is empty, the builder resolves them from Cloud identity at
// construction.
