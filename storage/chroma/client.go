...
const (
	...
	// maxRedirects mirrors net/http's default redirect limit. A custom
...
// either value is empty; in that case the builder resolves them from
// /auth/identity. APIKey is sent as X-Chroma-Token.
func defaultClientBuilder(opts ...ClientBuilderOpt) (ClientInterface, error) {
	...
	if inferScope {
		...
		defer cancel()
		if err := c.resolveScope(ctx); err != nil {
			_ = c.Close()
			return nil, err
		}
	}
...
// A custom CheckRedirect disables net/http's default redirect limit, so the
// hop count is bounded here to keep redirect loops from spinning until the
// request timeout.
func (c *httpClient) checkRedirect(req *http.Request, via []*http.Request) error {
	if len(via) >= maxRedirects {
		return fmt.Errorf("chroma: stopped after %d redirects", maxRedirects)
	}
	if len(via) > 0 {
		origin := via[0].URL
...
// resolveScope fills tenant and database from /auth/identity when an API key
...
func (c *httpClient) resolveScope(ctx context.Context) error {
...
// CollectionInfo returns the bound collection configuration.
func (c *httpClient) CollectionInfo() CollectionInfo {
	c.bindingMu.RLock()
	defer c.bindingMu.RUnlock()
	info := c.collection
	// Clone to avoid a shallow copy so callers cannot change SparseVectorIndexKeys.
	info.SparseVectorIndexKeys = slices.Clone(info.SparseVectorIndexKeys)
	return info
}
...
// Close releases idle connections owned by this client. A transport supplied
// through WithTransport remains owned by the caller and is not closed.
